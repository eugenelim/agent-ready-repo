...
# Register the `regen` merge driver that .gitattributes declares for self-host
# projections. `git config` set semantics make this idempotent with no guard.
...
bootstrap-git:  ## Register the self-host projection merge driver (once per clone)
	git config merge.regen.driver true
	git config merge.regen.name "keep ours; regenerated by make build-self"
	@echo "bootstrap-git: merge.regen registered; projections now auto-resolve on merge."
