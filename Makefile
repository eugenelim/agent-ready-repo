...
.PHONY: build build-self build-self-dry-run build-check package build-scaffold lint-packs pre-pr sast print-sast-dirs print-sast-config validate clean zipapp release-preflight
...
# The SAST leg below is NOT chained into build_gate_chain — Semgrep has no Windows
# support and the leg is conditional — so it stays appended here, run last.
build-check:
