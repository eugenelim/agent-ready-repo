...
$(PYTHON) -m pytest tools/test_build_gate_chain.py tools/test_journey_editorial_decisions.py tools/test_catalogue_tooling_rewire.py tools/test_catalogue_tooling_docs.py tools/test_stasis_retirement_claims.py tools/test_validate_guides.py tools/test_check_guide_index.py tools/test_catalogue_navigation.py tools/test_documentation_entry_links.py tools/test_build_site_link_rewrites.py tools/test_check_rendered_site_links.py tools/test_build_site_routing.py tools/test_check_docs_contrast.py tools/test_build_site_inventory.py tools/test_build_site_projection.py tools/test_build_site_sidebar.py tools/test_browser_gate_subset.py tools/test_local_ci_shared_test_deduplication.py tools/test_gitattributes_merge_driver.py tools/test_merge_driver_behaviour.py tools/test_pull_request_template_adoption.py tools/test_check_core_release.py tools/test_intent_corpus_gate.py -q
$(3)
...
.PHONY: worktree-doctor bootstrap-web bootstrap-docs-site bootstrap-sites bootstrap-git web-browser-gate
...
# Register the `regen` merge driver that .gitattributes declares. It covers two
# generators: self-host projections (`make build-self`) and the record indexes
# (`index-records.py`). `git config` set semantics make this idempotent with no guard.
# Scope: git config lives in the shared common directory, so one run covers
# every linked worktree -- but NOT another clone, which needs its own run.
# Without it git reports `Unknown merge driver: regen` and falls back to a
# normal conflicting merge, which is degraded rather than wrong.
bootstrap-git:  ## Register the regen merge driver for generated files (once per clone)
	git config merge.regen.driver true
	git config merge.regen.name "keep one side; the generator and its gate settle it"
	@echo "bootstrap-git: merge.regen registered; generated files now auto-resolve on merge."
