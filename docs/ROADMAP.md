...
- **Surface the cleared-gate parallel opportunity to the human.** Today
  `loop-cohort schedule` prints candidate waves with the hint "waves mark
  what *could* parallelize" — but that is a *pre-gate, static* hint. After
  `dispatch-decision` returns `parallel` for a wave, nothing proactively
  tells the human "this wave cleared the gate — opt in?"; the agent decides
  silently. A proactive post-gate surface (or an explicit per-run opt-in
  affordance) is what would make the opt-in actually get exercised rather
  than defaulting to sequential by inertia. **Unblocks when:** the work-loop
  EXECUTE step emits a cleared-gate prompt/summary the human can act on.
