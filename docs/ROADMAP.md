...
## `wave-scheduled-supervisor` — shipped (T1–T7 + all 4 autonomy follow-ons landed)
...
(Accepted 2026-05-29). T1–T7 landed via work-loop in PR #182 (merged `794e013`).
...
- ~~**Surface the cleared-gate parallel opportunity to the human.**~~
  **Shipped 2026-05-29 in PR #182 (T7 / AC10; merged `794e013`).** `dispatch-decision` now emits
...
- **Predict disjointness before dispatch.** The `git merge-tree`
  ...
  touched-file globs.
- ~~**Explicit per-run opt-in flag / state field.**~~ **Shipped 2026-05-29 in
  PR #187** (spec `docs/specs/supervisor-auto-parallel/`). A per-run
  `auto_parallel` `state.json` field (default off; set by `loop-cohort
  auto-parallel` / `--off`) lets an **unattended** run fan out a **gate-cleared**
  wave without the follow-on-1 human opt-in — so a plan can finish while you're
  away from your desk. **GO-approval-only**: never a gate input, never
  parallelizes a non-cleared wave, and a failed parallel wave still Surfaces and
  stops. This was the autonomy capstone — **all four follow-ons now shipped.**
