...
- **Slice 1 started:** 2026-09-24. Slice 1, [`lifecycle-transition-contract`](../../specs/lifecycle-transition-contract/spec.md), started: its T3 corpus migration is delivered in commit `d00f39e34`. A material amendment to that spec — restating AC-0010, and adding no criterion — was reviewed to `Clean` over seven adversarial rounds and re-approved on 2026-09-24, returning the spec to `Implementing` and this brief to `Executing`. The slice cut stays at three slices; slices 2 and 3 are not yet cut.
...
- **Retention or disposal of a closed record**, *provided* `close-work`'s existing disposition contract covers a product bet. Whether it does is open — the parent carries it as A4, untested and never given a kill condition. If it does not cover the case, the extension returns to this brief's scope rather than becoming a second contract. **A4 is therefore settled before the slice-cut confirmation**, so the extension can enter the cut as a candidate rather than arriving after the delivery map is material-locked. The parent's § Validation hook carries settling A4 as its **zeroth** owed activity — the label that hook uses, ahead of the three it numbers First, Second and Third. Corrected 2026-09-24 as nonmaterial: a wording fix to a recording sentence, not to a Ready-gate field, so the bound review result stands and the brief does not return to `Draft`. **It was not settled, and the cut was confirmed anyway on 2026-09-23 with A4 open** — recorded here because this is where the obligation lives, and `docs/specs/lifecycle-transition-contract/spec.md` § Assumptions, which carries the same deviation, is spec prose nothing re-reads to find a missed brief precondition. **A4 settled against coverage on 2026-09-24**, in the parent's § Validation hook, which owns the reading and its evidence: `cool-30-days` is eligible on delivered closed work and `retain-exception` on a continuing obligation, and neither reaches a bet that closed without delivering — the state 6 of today's intents are in. **So this non-goal's condition fails and the retention extension returns to this brief's scope.** Because the cut was confirmed with A4 open, it returns as a later slice rather than as a candidate inside the confirmed cut, which is the cost the deviation above predicted. This is a **material** change to the delivery map, so the Ready review bound above no longer covers this section; the fresh review it owes runs when `author-delivery-brief continue` next presents a cut, which is the first point the new slice is decided on. `Status` does not return to `Draft`, which would assert this brief never passed its Ready gate — it passed on 2026-09-23. It reads `Ready` while slice 1 is parked for an amendment and `Executing` otherwise, both of which § Brief lifecycle derives from child state rather than from review evidence.
...
- **[Killed]** `close-work`'s disposition contract covers a closed intent's record without extension. It never carried a kill condition, but it did carry a line set before reading, and the parent's § Validation hook ran its zeroth activity on 2026-09-24: no eligibility row reaches a bet that closed without delivering. The conditional non-goal above therefore fires, and § Scope / Non-goals records what that costs.
