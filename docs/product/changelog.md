...
- **`queue-add` skill (core pack 0.13.0) — capture session-surfaced work into `workspace.toml`.** Given a bulleted or numbered list of follow-ons, recommendations, or deferrals, `queue-add` derives spec paths, infers dependencies only from explicit sequencing language, prioritizes (advisory — queue order plus a rationale comment, never a schema field), and groups (independent batch, atomic bundle for work that must ship together, or a suggestion to escalate to `author-brief`). It appends to an active initiative's `[work].queue` or the repo-level `[backlog]`, and for items that fit neither it suggests the right home (brief, findings register, RFC, or a new initiative). Each entry carries a comment rich enough for a cold-start session to write the full spec. Writes `workspace.toml` only; never creates spec files; the user reviews before writing. Triggers on "add these to the queue", "capture these as queue items", "add this to the backlog". ([RFC-0064](../rfc/0064-ini-001-ai-native-ecosystem.md))
