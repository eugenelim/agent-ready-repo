...
- **`check-workspace` surfaces findings count (core 0.13.0).** After resolving the queue DAG, `check-workspace` now surfaces a count line — "N rfc candidates · M roadmap intents" — from the findings registers when either count is non-zero. Omitted when both are zero or the files are absent.
