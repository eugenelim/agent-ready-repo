...
- **The reachability check exists.** Full-mode spec review sends each report
  that is not footer-free clean to `finding-adjudicator` — a structurally clean
  return closes the round directly. It checks whether the
