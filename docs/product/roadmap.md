...
**M1 · workspace-mcp Stage 1 (RFC-0078).** Per-session MCP server shipped with the `core` pack (agentbundle 0.28.0). Provides `workspace_status`, `elicit`, `git_status`, `git_branch`, `git_commit`, and `git_push` tools over MCP stdio. Class A adapter (Claude Code via ACP): control harness injects the server at `session/new.mcpServers`; FSM state and queue observable via `workspace_status()` polling; git operations scoped to the dispatched item's output paths. Session instruction (`DEFAULT_SESSION_INSTRUCTION`) shapes agent behaviour for harness-controlled sessions. Operator how-to at `guides/core/how-to/run-headless-session.md`. Stage 2 (Codex Class A, notification relay, permissions projection) deferred. [RFC-0078]
