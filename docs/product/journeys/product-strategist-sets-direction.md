...
| **Opportunities** | `define-ux-strategy` produces a committed `ux-strategy.md` (vision → goals+measures → plan) that `journey-mapping` and `user-flow` can reference as the strategic anchor. `define-content-strategy` produces a committed `content-strategy.md` (Purpose + Process + Structure + Governance — the pack's own composite) that content-design skills consume downstream. |
