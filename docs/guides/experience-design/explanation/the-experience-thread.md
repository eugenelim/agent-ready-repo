...
`experience-design` makes that intent explicit and durable, the way
...
a framework that picks your tech stack," so `experience-design` ships **method, not
...
`experience-design` is **habits, not infrastructure**. No hooks, no engine, no in-pack
