...
Normative. `tools/lint-guidebook-steps.py` reads its obligation identifiers,
label forms, judgement kinds and prohibited vocabulary **out of this file** and
copies none of them, so editing a table here changes what the lint enforces.

Maintainer material, not adopter material: it governs how a guidebook step is
written, and lives in `docs/guides/` for the same reason every other authoring
rule does. [`guides/AGENTS.md`](../../guides/AGENTS.md) points here.

The reasoning behind these obligations, and the parts no lint can decide, are in
[`docs/product/guidebook-methodology.md`](../product/guidebook-methodology.md).
The walk that tests a finished guidebook against a first-time reader is in
[`docs/product/pack-walks/`](../product/pack-walks/README.md).
