...
The `research` pack answers questions at two very different scales, and it's
...
- **Depth** — *how hard you look at a question in one sitting.* This is the
  `/research` dial: `quick` (inline answer, no file), `standard` (cited
...
A one-off `/research` goes straight from sources to a written answer; the
...
saturation*. That's the signal `/research-project-check` reads, by eye, to tell
...
They compose. Inside a project's capture phase, you run *episodic* `/research`
...
The decision tree is short: **one sitting, one answer → one-off `/research`.
...
- [Research pack reference](../reference/research-pack.md) — the catalogue of
  both axes' skills.
