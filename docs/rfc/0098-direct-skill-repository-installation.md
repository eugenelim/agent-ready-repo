...
`.claude/skills/` is the sole hidden path this decision admits; E13's rule that hidden root context is ignored is otherwise unchanged, and no other dot-directory becomes a collection root. Root-level category grouping (`<category>/<name>/SKILL.md` with no collection root) remains refused, because it is structurally indistinguishable from a vendored aggregate of other publishers' collections — the corpus held both shapes and no mechanical test separates them.

The extra level costs depth headroom: the corpus maximum was 7 against a limit of 10 measured from the collection root, so a category-grouped repository with equally deep payloads reaches 8. Depth is the binding budget under AC32's headroom rule.

### E15 — Depth is measured from the skill envelope and bounded at 12

**2026-08-28 — Approver: eugenelim.** E11 bounds path depth at 10 measured from the enumeration root. Two defects follow. First, the origin is layout-dependent: E14's optional category level consumes one level, so an identical skill payload measures one deeper merely because its publisher groups by topic, and the same repository changes verdict when regrouped. Second, the value is too tight against evidence: over the T0d corpus the deepest admitted payload measures 9 of 10 from the collection root — 10% headroom — which trips the AC32 rule requiring 50%.
...
The bound is raised from 10 to **12**, giving 42% headroom against that measured maximum. 14 would satisfy AC32's 50% threshold outright and is deliberately not taken: the evidence does not ask for it, and a bound should be the smallest value the corpus supports. AC32's report obligation is discharged by this erratum rather than by widening until the rule falls silent.
