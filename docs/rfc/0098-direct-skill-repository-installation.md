...
**2026-08-28 — Approver: eugenelim.** The body’s root-single/local-path wording says other regular root entries are ignored, but did not cover hidden root entries, direct-pack and collection roots, or state whether ignored context participates in the direct bounds. That gap made a direct installation of an ordinary repository depend on unrelated repository size and root links. For root-single/local-skill, direct-pack, and collection shapes, root repository context—including hidden files and directories and link-like or special root entries—is outside the direct enumeration: enumeration and Family-2 counting start only at `skills/` for a collection/direct pack, or at root `SKILL.md` plus `scripts/`, `references/`, `assets/`, and `evals/` for a root single/local skill. Context is neither traversed nor counted toward Family-2 entries, depth, or files, and is never normalized, hashed, reported, or projected. The link-like disposition is unconditional refusal when a link-like or special entry is encountered in a candidate path; no root-context traversal occurs.
