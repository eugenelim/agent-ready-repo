...
Family-2 enumeration is one confined traversal: each helper invocation receives only its remaining entry allowance. `packages/agentbundle/agentbundle/catalogue_tooling/file_safety.py` supplies `BoundExceeded(UnsafeContentError)`, carrying the budget that breached, so direct callers attribute a bound refusal without a second traversal or message parsing. Existing catalogue callers continue to catch it as `UnsafeContentError`. A direct source that cannot be traversed, or changes during admission, uses its own registered diagnostic rather than measured-path integrity; no offending-path obligation applies to that distinct condition.

`packages/agentbundle/agentbundle/catalogue_tooling/diagnostics.py` is the sole `DiagnosticCode` registry for direct diagnostics; the four direct modules remain top-level. Direct identity names use the existing `^[a-z0-9][a-z0-9-]*$` grammar and 64-character maximum. Publisher values inserted into printed recovery commands are shell-quoted; human-readable paths are escaped for bidi controls, separators, default-ignorables, and non-graphic code points. The identity name may remain an installed-state key only because that grammar constrains it.

Every injectable acquisition clock or progress/budget seam refuses `None`, `bool`, non-`int`, negative, and non-finite values before resolving as `min(MODULE_CONST, value)`. Clock and progress seams are private, keyword-only, and production-unsupplied; valid progress resets the inactivity timer. `LIST_INSTALLED_CHECK_DEADLINE_SECONDS = 90` and `LIST_INSTALLED_CHECK_MAX_RESOLUTIONS = 25` are E11 constants. `list-installed --check` applies the deadline to in-flight post-dedup acquisitions, never raises for its deadline, cap, or a resolution failure, and fans a canonical source result out to every corresponding row.
...
**2026-08-28 — Approver: eugenelim.** The measured-envelope entry budget is reduced from 5,000 to **2,500**. With all six Family-2 budgets at their limits, the superseded 5,000-entry budget measured 4.72–4.97 s wall-clock; the 2,500-entry budget measured 1.92–2.12 s wall-clock and 26 MiB resident. The recorded corpus maximum is 572 entries, so the reduced limit retains substantial observed headroom while restoring AC36’s single-traversal cost basis. This erratum owns the entry figure; E11 refers to it rather than silently rewriting its approved dated decision.
