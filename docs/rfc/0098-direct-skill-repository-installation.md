...
Family-2 enumeration is one confined traversal: each helper invocation receives only its remaining entry allowance. `packages/agentbundle/agentbundle/catalogue_tooling/file_safety.py` supplies `BoundExceeded(UnsafeContentError)`, carrying the budget that breached, so direct callers attribute a bound refusal without a second traversal or message parsing. Existing catalogue callers continue to catch it as `UnsafeContentError`. The helper's `max_depth` parameter is not used for E15; callers derive envelope-relative depth from its returned paths. A direct source that cannot be traversed, or changes during admission, uses its own registered diagnostic rather than measured-path integrity; no offending-path obligation applies to that distinct condition.

`packages/agentbundle/agentbundle/catalogue_tooling/diagnostics.py` is the sole `DiagnosticCode` registry for direct diagnostics; the four direct modules remain top-level. Direct identity names use the existing `^[a-z0-9][a-z0-9-]*$` grammar and 64-character maximum. Every publisher- or user-supplied value interpolated into a printed recovery command, including a source string or path, is shell-quoted; human-readable paths are escaped for bidi controls, separators, default-ignorables, and non-graphic code points. The identity name may remain an installed-state key only because that grammar constrains it.

Every injectable acquisition clock or progress/budget seam refuses `None`, `bool`, non-`int`, negative, and non-finite values before resolving as `min(MODULE_CONST, value)`. Clock and progress seams are private, keyword-only, and production-unsupplied; valid progress resets the inactivity timer. `LIST_INSTALLED_CHECK_DEADLINE_SECONDS = 90` and `LIST_INSTALLED_CHECK_MAX_RESOLUTIONS = 25` are E11 constants. `list-installed --check` applies the deadline to in-flight post-dedup acquisitions, limits itself to five in-flight resolutions and a 256 MiB aggregate downloaded-byte budget, terminates an acquisition once `pax_global_header` is read, never raises for its deadline, cap, in-flight limit, aggregate-byte budget, or a resolution failure, and fans a canonical source result out to every corresponding row. Every direct state mutation uses `agentbundle.statelock.persist_state_locked`; the 0.5 floor is computed from the state re-read inside that lock, never a pre-lock snapshot.
...
The bound is raised from 10 to **12** so that the admitted maximum of 6 and the widest observed payload of 7 both clear it after the origin change, without inviting arbitrary nesting. The raise is justified by the origin correction and by AC36's cost ceiling: the mutually satisfiable Family-2 budgets at their limits (entries, depth, files, selected skills, and total bytes; per-file bytes measured at total bytes ÷ files) measured 1.92–2.12 s and 26 MiB against the 5 s / 256 MiB ceiling; wall-clock binds and memory does not. The earlier 1.70 s figure omitted depth and selected-skill count, which turn out to dominate; the 4.02 s / 4.97 s six-budget figure was measured at the superseded 5,000-entry budget (see E16).
...
**2026-08-29 — Approver: eugenelim.** The AC36 CI reference measurement on `ubuntu-latest` provisionally confirms the ceiling with the mutually satisfiable Family-2 budgets at their limits; the confined-helper measurement records wall-clock as the binding dimension and a 1.9 MiB admission-region `tracemalloc` peak as a non-binding fixed memory floor, pending T3's direct-admission-entry-point rerun.

### E16 — Entry budget reduction and cost basis
...
### E22 — The repository root may be the collection, and an empty Git placeholder is not a hidden payload

**2026-08-31 — Approver: eugenelim.** E14 recognises `skills/` and `.claude/skills/` as the collection roots, and the envelope rule refuses any hidden entry. Both were measured against the corpus and both were refusing real, unremarkable repositories.

**Root-level collections.** Publishers commonly place skill folders directly at the repository root with no wrapper — `alt-text/SKILL.md`, `lead-research-assistant/SKILL.md`. Under E14 these matched no recognised root and refused with "no supported shape", which is a whole publishing shape left unreachable rather than a judgement about the source. The repository root is therefore admitted as a collection root, reached **only** after every other marker has been ruled out, so a repository carrying `skills/` still uses it and a root-level folder cannot shadow it.

This adds no recursive discovery. It is the same single level E14 already defines: a child directory holding `SKILL.md` is an envelope, exactly as it would be beneath `skills/`. Two constraints make it safe rather than merely permissive. Candidate selection skips dot-prefixed children, and — decisively — enumeration is restricted to the identified skill folders rather than the root. Walking the root wholesale would traverse `.git`, `.github`, and every other repository directory, which E13 says is never inspected or counted, and would spend the entry budget on Git internals before reaching a single skill. Measured on a corpus repository, the restricted walk reports 2 entries and 2 files where the unrestricted one reported over a thousand.

**Empty Git placeholders.** The hidden-entry rule exists because a dotfile inside a skill envelope can carry instructions. A `.gitkeep` cannot: it exists only to make Git track an otherwise empty directory. Refusing an entire repository over a zero-byte placeholder rejected real sources for a file with no content. `.gitkeep` and `.keep` are therefore admitted inside an envelope, **and only when empty** — emptiness is enforced after the bounded read, so a placeholder carrying bytes is still refused as the hidden file with content that it is. That guard earns its place immediately: one surveyed repository ships a 178-byte `.gitkeep`, which continues to refuse.

**Measured effect.** Against the same eighteen-repository corpus, admissions rise from five to ten. The remaining eight refusals are attributable and unremarkable: a file-count budget breach, a non-empty `.gitkeep`, partial catalogue markers, nested envelopes, an identity that fails the slug grammar, and three repositories that carry no skill at any recognised location. The AC35 verdict table is regenerated against this behaviour.

### E23 — The decompressed bound measures reads, and the inactivity timeout is acquisition-only

**2026-08-31 — Approver: eugenelim.** Two corrections to E11's Family-1 acquisition bounds, both found by review of the implementation against E11's own wording.

**The decompressed counter measured the wrong thing.** E11 bounds "1 GiB incrementally measured decompressed bytes on the decompressed side of gzip and tripping mid-read". The implementation accumulated `TarInfo.size` — what each member *declares* in its header, which a hostile archive controls independently of what it ships. The bound now counts bytes actually read off the decompressed stream through a counting wrapper, and the declared-size accumulation is retained only as a cheap pre-check with the authoritative bound named beside it. The difference is not theoretical: tar pads every member to a 512-byte block behind a 512-byte header, so forty members declaring eight bytes each declare 320 bytes and read past 40 KiB. A fixture asserts exactly that shape.

**The inactivity timeout cannot fire where E11 places it.** E11 states that the 90-second inactivity code "is therefore reachable only during extraction", because the 30-second socket timeout necessarily fires first during acquisition stalls. The extraction loop has no timer, so the code is unreachable in both phases. This is recorded rather than repaired: extraction is already bounded on the axis that matters by the corrected decompressed counter above, the member count, and the per-file and total byte budgets, and adding a wall-clock timer beside a byte bound would introduce a second way to fail a slow-but-legitimate large archive without covering a threat the byte bound misses. AC27 lists "inactivity/download limits" among the refusals that must be reachable; the download limit is reachable and tested, and the inactivity limit is hereby scoped to acquisition, where the socket timeout dominates it. `INACTIVITY_TIMEOUT_SECONDS` remains as the acquisition-side stall guard for the case where a response trickles without stalling long enough to trip the socket.

### E24 — E22's measured-effect figures, corrected against the committed table

**2026-08-31 — Approver: eugenelim.** E22's closing paragraph reported figures that the regenerated AC35 table does not carry. The table was regenerated after E22 was written and the paragraph was not re-checked against it.

**The corrected figures.** Against the same eighteen-repository corpus the committed table records **nine admitted and nine refused**, not "five to ten" with "the remaining eight refusals". Four repositories — `Microck/ordinary-claude-skills`, `abubakarsiddik31/claude-skills-collection`, `borghei/Claude-Skills`, and `simonw/claude-skills` — carry no skill at any recognised location, not three. The attribution E22 gives for the rest stands unchanged: a file-count budget breach, a non-empty `.gitkeep`, partial catalogue markers, nested envelopes, and an identity that fails the slug grammar. The direction of E22's argument is unaffected — root-level collections and empty placeholders together move the corpus from five admissions to nine — and the table, not this paragraph, is the artifact AC35 is judged against.

**Two later rules were re-measured against the same corpus.** The hidden-entry rule now also covers the segments between a collection root and an envelope, so `skills/.hidden/backdoor/SKILL.md` refuses where it was previously admitted and reported as selectable; and the root candidate scan's entries are now carried into the walk's allowance rather than bounded separately. **No repository's verdict changed** under either. The three root-level collections' recorded `admitted_entries` rose by exactly their root-child count — `ComposioHQ/awesome-claude-skills` by 35, `alirezarezvani/claude-skills` by 59, `posit-dev/skills` by 20 — and the wrapped shape is unchanged, because a repository whose skills sit under `skills/` enumerates nothing in that scan.
