...
The bound is raised from 10 to **12** so that the admitted maximum of 6 and the widest observed payload of 7 both clear it after the origin change, without inviting arbitrary nesting. The raise is justified by the origin correction and by AC32's cost ceiling — the whole Family-2 envelope at every limit measures 1.70 s and 28 MiB, far inside 5 s and 256 MiB — not by any headroom threshold. An earlier draft of this erratum justified the raise by a corpus-headroom rule computed from a **refusing** repository; that reasoning is withdrawn.
