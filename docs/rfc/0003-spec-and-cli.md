...
Lift the per-IDE adapter contract from `contracts/adapter.toml`
...
The adapter contract — currently at `contracts/adapter.toml`
...
1. **Stay in this repo, treat the path as canonical.** URL is
   `github.com/<org>/agent-ready-repo/blob/main/contracts/adapter.toml`.
...
This repo's adapter contract — currently `contracts/adapter.toml`
...
`contracts/adapter.toml`; no versioning, no
...
- **F-spec-publish.** Move `contracts/adapter.toml`
...
- 2026-05-22 (post-acceptance): adapter contract files relocated to
  `contracts/`; this RFC's path references updated (the sibling-repo
