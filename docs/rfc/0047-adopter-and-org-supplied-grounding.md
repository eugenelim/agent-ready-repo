...
- **RFC (backlog): org-owned detached-fork distribution** — how an organization forks the catalogue into one it owns outright and distributes internally with no upstream dependency (the systematic counterpart to the editable-install path that exists today). RFC-0047's org-pack leg authors the pack; this follow-on RFC owns the detached-fork distribution mechanism.
