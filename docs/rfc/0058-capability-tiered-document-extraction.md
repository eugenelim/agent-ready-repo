...
- **ADR** — record the capability-tiering doctrine for extraction (peer to ADR-0037 for grounding).
- **Spec:** `docs/specs/extraction-tier0-and-output-contract/` — the no-ML floor (D2), unified contract (D3), and D7 formats.
- **Spec:** `docs/specs/extraction-general-image-mode/` — the general text/table mode (D4), Tier-1 PDF-page rasterization.
- **Spec:** `docs/specs/extraction-higher-tiers/` — opt-in enrichment/vision/managed-API (D5) and structure+chunking (D6), with a `security-reviewer` pass on the egress tier.
- **Erratum** on RFC-0007 recording that its locked-down-dependency drawback is addressed here.
