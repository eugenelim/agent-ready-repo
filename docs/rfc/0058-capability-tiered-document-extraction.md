...
- **Spec:** [`docs/specs/extraction-general-image-mode/`](../specs/extraction-general-image-mode/spec.md) — the general text/table mode (D4), Tier-1 PDF-page rasterization. ✅ authored (second slice); plan approved, spec-stage adversarial + `security-reviewer` passes Clean. Rasterizer resolved to **`pdf2image` (MIT)** — `pymupdf` rejected (AGPL). Implementation is the next slice.
