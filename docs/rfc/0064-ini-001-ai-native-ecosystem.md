...
| 10 | 2026-08-31 | workspace-status integrity fixes shipped | Supersedes Erratum #2's open state: both skill fixes are shipped (`new-rfc-followon-queue-write` and `workspace-status-queue-reconciliation`). Canonical `workspace-status` reconciliation now surfaces the drift Erratum #2 described, so its manual post-session-gap sweep is retired and no longer the standing instruction. |
| 11 | 2026-09-21 | `["ini-002".shaping_queue]`'s three shaping slugs are retired | ADR-0119 retires `Initiative` as a canonical repository artifact and as a workspace ownership boundary, making the recursive intent graph the single hierarchy. The three shaping-queue slugs below — `ini-002-initiative-brief`, `opp-assessment-pe-pack` and `capability-map-ini-002` — each existed to produce a living reconciliation artifact for that retired container, so each was deleted on 2026-09-21 along with its `workspace.toml` entry. The frozen queue above is historical; no work remains behind those slugs. `docs/product/workspace-toml-deps.md` previously used two of them as its only worked examples of `shape:<slug>` and `strategy:<slug>` and now cites live slugs instead. |
