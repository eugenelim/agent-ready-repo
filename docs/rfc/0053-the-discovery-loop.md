...
- **Related:**
  ...
  - **Sibling loop:** [RFC-0049](0049-the-release-loop-and-company-os.md) — the *downstream* outer loop (`release-lead` + `release-loop`); same agent-def + skill + harness pattern this RFC establishes upstream; both build on the RFC-0048 substrate (the sidecar + the gate arc), and this RFC specifies the sidecar schema RFC-0049's release loop also consumes.
  - **Self-coverage:** [RFC-0051](0051-the-self-coverage-gate.md) — owns the goal + seam and the full seven-module design-convergence instantiation; `discovery-loop` is that instantiation's **primary home** (carries its own co-scoped copy of all seven modules as its pre-G2 phase, wired here), whereas `work-loop` adopts only the net-new slice.
  - **Lens pack:** [RFC-0050](0050-the-experience-pack.md) — the `experience` lens this loop detect-and-degrades on.
  - **Precedent:** [RFC-0041](0041-infra-aware-work-loop.md) + ADR-0031 — the *doctrine + reference library + reuse, no engine / no new reviewer* precedent the framing rests on.
  - **Mechanisms:** RFC-0025 (`work-loop` light/full + the iteration cap this loop's outer cap mirrors) · RFC-0040 (the three-tier layout resolution the sidecar paths obey) · RFC-0019 (`receive-brief` — the brief→spec join at G3; its coverage lint child-4's traceability lint generalizes) · [ADR-0022](../adr/0022-value-stream-meta-repo-cross-component-layer.md) (the cross-repo reference-by-version mechanism the traceability slot reuses).
  - **Child-4 lint:** [`docs/specs/traceability-lint/`](../specs/traceability-lint/spec.md) — the lint that consumes the traceability slot.
  - **Notes:** promoted research + the empirical prototype in [`0053-notes/`](0053-notes/); [`0048-notes/09`](0048-notes/09-gap-resolutions.md) (the paper resolutions this spike confirms) and [`0048-notes/02`](0048-notes/02-worked-example-flow-trace.md) (the worked example it was run against).
