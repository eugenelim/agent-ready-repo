...
- **`pack.toml` `[pack.links].documentation`** → the per-pack **guide home** (`guides/catalogue-curation/`), following the repo's Diátaxis convention. Because per-pack guides **ship with the pack** (playbook Module 7 keeps them; only catalogue-governance docs are stripped), a fork's operators get the same upskilling material.
...
- **Per-pack guide:** `guides/catalogue-curation/` (Diátaxis: tutorial + how-tos + reference + explanation) authored via `new-guide` — the maintainer discovery & upskilling surface, and the one that ships to forks (playbook Module 7). Pack `README.md` + `pack.toml` `[pack.links].documentation` point to it.
