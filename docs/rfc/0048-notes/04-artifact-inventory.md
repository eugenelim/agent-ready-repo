...
them agent-producible and build-consumable (the experience pack's own framing).
...
| **Persona** | who it's for; the referent taste + UX fit derive from | *elicited inline by the first consumer* (not a separate produced artifact in the current child set — RFC-0048 § Amendments DRIFT-F; the frame-domain spec excludes it; promoting it to a produced artifact is a deferred option) | aesthetic-direction; map-customer-journey |
...
| Artifact | What it is | Skill | Consumed by |
| --- | --- | --- | --- |
| **Journey map** | stages × actions / emotions / pains / opportunities (frontstage) | map-customer-journey ✦ | map-screen-flow; blueprint-service |
...
| **Screen inventory + state matrix** | the screen list; per-screen states (empty/loading/error/success/permission) | map-screen-flow ✦ (states defer to handle-all-states ✓) | experience skills; voice-and-microcopy |

**Craft (existing experience-pack skills):**
...
| **Quality-floor checklist** | handle-all-states · accessibility floor · reduced-motion | experience shared ✓ | every screen design |
