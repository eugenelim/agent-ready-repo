...
behind, and this evidence base has now been corrected 23 times in one round
...
| R9-22 | The privacy gate added in R9-13 can fail | **Not in the shell it would normally be run in.** The gate piped the sweep into `tail -1` and tested `${PIPESTATUS[0]}`, which is correct in bash and broken in zsh: zsh indexes `PIPESTATUS` from 1, so the subscript expands to empty, the test is malformed, `fail` is never set, and the script exits **0 with a real leak present**. Measured, not reasoned: with a planted `/Users/<name>` path, bash exited 1 and zsh exited 0. The default shell on the host is zsh. The control written *this round* to close a privacy hole therefore could not fail in ordinary use — the same shape as R9-15, one layer down | **Fixed by removing the pipe**, not by picking a shell-specific subscript: the sweep's output is captured and its own exit status is used, which behaves identically under both shells. All four cells of the truth table are verified — clean and leaking, bash and zsh |
| R9-23 | The claim-accounting tool's identifier heuristic distinguishes check ids from other capitalised tokens | It does not, and it **blocked this promotion to say so**: writing R9-22 introduced a backticked shell array name, which the "uppercase inside backticks is a check id" rule reported as a named check no artifact carries. The tool was right that it could not tell the difference, and the build stopped — which is R9-16's exit-non-zero change doing exactly its job on its author | **Named as a literal, not designed around.** The alternative was rewording the note so the regex would be satisfied, which is R9-10. Exact literals only: the *prefix* form of this skip rule is what made the identifier check unfailable (R9-15), and an exact literal cannot generalise past itself |
