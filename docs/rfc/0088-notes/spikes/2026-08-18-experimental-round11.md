...
**A caution a reader needs before trusting the published digest**: the archive
SHA-256 quoted in the RFC is *not* reproducible by re-running the promote. The
accounting tool records the archive's own size, and its output is a member of that
archive, so the digest alternates between two values rather than converging
(R11-6). The manifest is the verification handle that holds; the digest is not,
until that self-reference is broken. Two
