...
- **Profiles on the Claude-plugin `/plugin` and APM routes.** Neither route has a *profile/bundle* concept — a marketplace *lists* plugins and users install them individually. Claude plugins *do* compose, but only via a plugin-level `dependencies` array (the **meta-plugin** pattern), and APM via per-package `apm.yml` dependencies — both the meta-pack shape this catalogue rejected (see Options → meta-pack). Mapping profiles onto either is out of scope for v1; see Open question 2 for why it is not a free win.
...
- **Claude-plugin and APM routes** have no *profile* concept (a marketplace lists plugins; users install them one at a time), and their only native composition is plugin-to-plugin `dependencies` — the meta-pack pattern this catalogue rejected (see Options → meta-pack). Profiles stay CLI-only in v1, consistent with [RFC-0003](0003-spec-and-cli.md) framing the CLI as complementary to, not a replacement for, the install routes.
...
| **A1b — A1, also surfaced on the plugin/APM routes** (e.g. emit a meta-plugin per profile so `/plugin install` pulls the set via plugin `dependencies`) | A1 + per-route meta-plugin synthesis | Reaches plugin/APM users, but the only native composition there is plugin `dependencies` — the rejected meta-pack shape — and those dependencies install at the same scope, with no notion of our user/repo pack scope, so it can't honor scope-homogeneity. Deferred to OQ2. |
...
**Distinguished — meta-pack / meta-plugin (not on the axes; a different thing that looks similar).** [ADR-0003](../adr/0003-credential-broker-contract.md) / [RFC-0013](0013-credential-broker-contract.md) option F ("Pack bundling": a pack that depends on its consumer packs) was **rejected** for portability coupling. A profile is *not* a meta-pack: it is an external manifest with no pack→pack hard dependency and no pack identity, so the coupling objection does not transfer.

This distinction is load-bearing for the plugin route. **Claude Code's only native composition is exactly this rejected shape:** a plugin declares a `dependencies` array and installing it auto-resolves and installs those plugins ([Claude Code plugin dependencies](https://code.claude.com/docs/en/plugin-dependencies)) — a meta-plugin. There is *no* profile/bundle primitive; a marketplace lists plugins that users install individually ([Claude Code plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces)). So to give plugin-route users one-command install we would have to publish a meta-plugin per profile, re-introducing the pack→pack coupling RFC-0013 F rejected — and Claude's plugin dependencies enable "at the same scope" with no notion of our user-vs-repo pack scope, so a meta-plugin could not enforce the scope-homogeneity rule (D4) either. A catalogue-owned, decoupled, scope-aware `profiles/<name>.toml` read by the CLI sidesteps both problems; that is the core reason for the CLI-route-only decision (D3), not a mere convenience.
...
- **Claude Code plugins (the target platform itself)** — confirms the *gap* this RFC fills: there is **no** profile/bundle primitive; a marketplace lists plugins installed individually ([plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces)). Its only composition is a plugin `dependencies` array that auto-installs the named plugins on install ([plugin dependencies](https://code.claude.com/docs/en/plugin-dependencies)) — the meta-plugin/meta-pack shape this catalogue rejected, and one that installs deps at the same scope with no notion of our user/repo pack scope. Empty prior art for "a curated, scope-aware, decoupled bundle" on this platform is itself the finding: it does not exist natively, which is why a catalogue-owned CLI manifest is the right home (see Options → meta-pack).

Takeaway: "a named set of units installed in one command" is well-established across three independent ecosystems, each a thin composition layer over an existing per-unit installer — none required new runtime infrastructure. The target platform (Claude Code) offers only the *coupled* meta-plugin form, which is why this RFC keeps profiles a decoupled, CLI-owned manifest. This supports the A1 "hygiene, not infrastructure" framing.
