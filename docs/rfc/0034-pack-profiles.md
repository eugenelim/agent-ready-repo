# RFC-0034: Pack profiles — one-command install of a curated pack set
...
**Recommendation (BLUF):** Add **pack profiles** — a first-party-curated, named set of packs an adopter installs in one command (`agentbundle install --profile solution-architect <catalogue>` pulls `core` + `architect` + `research` + `contracts`). A profile is a hand-authored `profiles/<name>.toml` file at the catalogue root, read **only** by the `agentbundle` CLI; it adds zero primitives, zero runtime, and zero adapter-contract surface. This is distribution hygiene in the [RFC-0031](0031-catalogue-package-manager-posture.md) lineage, not package-manager infrastructure.

**Why now (SCQA):** *Situation* — the catalogue is a collection of independently-installable packs, consumed à la carte ([RFC-0001](0001-bundle-distribution-by-adapter-spec.md)). *Complication* — a role like "solution architect" needs four packs that span install scopes (`core` is repo-scope; `architect`/`research`/`contracts` default to user-scope), so onboarding is four invocations *and* requires the adopter to know which pack lands at which scope, or they fight `allowed-scopes` refusals. RFC-0001 already blesses these combinations — but only as a prose table no tool can execute. *Question* — should the catalogue promote those blessed combinations from prose into a one-command installable unit, and if so, in the thinnest form that respects the charter?

**Decisions requested:**

1. **Adopt pack profiles at all** (vs. do-nothing / keep prose) — *and with it, the explicit charter call:* does one-command role install clear Principle 4 ("used often enough to stick")? This is the proposal's weakest pillar and the Approver must rule on it, not wave it through. · *Recommended:* yes — the substantive kernel is encoding mixed-scope role setup once, and onboarding recurs per repo *and* per new team member (see frequency argument under Risks). · decide-by: RFC acceptance; default if no objection: **adopt**.
...
4. **Resolution contract** — ordered, deps-first; one adapter pins the whole batch; all-pre-flight-before-any-write; skip-already-installed; a lint enforces dep-completeness + order validity. · *Recommended:* yes. · decide-by: acceptance.
5. **First-party-curated only; no profile entity in state; no schema or contract bump.** · *Recommended:* yes; adopter-authoring is a named non-goal / follow-on. · decide-by: acceptance.

## Problem & goals

**Diagnosis.** Installing a role's worth of packs today is N invocations of a single-pack command (`install` takes exactly one `--pack`, no `nargs` — `cli.py:218`), and the adopter must independently know each pack's default install scope. The knowledge "which packs make up the solution-architect role, and which scope each lands at" exists only in a human's head or in RFC-0001's prose "Common adoption patterns" table (`0001…:668-677`) — which no tool can execute. The friction is highest at the catalogue's single most frequent event: a new adopter (or a new repo, or a person taking on a new role) setting themselves up.

This is a real but **bounded** problem: it is convenience over an existing capability, not a missing capability. The honest case for acting is that the *mixed-scope* dimension makes the manual path genuinely error-prone, and a profile encodes that scope knowledge once, declaratively.

**Goals.**

- One command installs a curated, named set of packs for a role/audience.
- The set may span install scopes; the adopter does not need to know each pack's scope.
...
The sections below are keyed to the Decisions requested: **D2–D5** map to Decisions 2–5. Decision 1 (adopt at all / charter gate) has no design section by nature — it is the go/no-go on everything here.

### D2 — where profiles are defined: `profiles/<name>.toml` at the catalogue root

Profiles live in a new top-level `profiles/` directory, a sibling of `packs/`, one file per profile:
...
```toml
# profiles/solution-architect.toml
description = "Solution architect: governance core plus architecture, research, and contract authoring."

# Ordered, deps-first. Each entry names a pack; `scope` is optional and
# defaults to that pack's own [pack.install] default-scope.
packs = [
  { pack = "core" },                  # repo-scope default
  { pack = "architect" },             # user-scope default
  { pack = "research" },              # user-scope default
  { pack = "contracts" },             # user-scope default
]
```
...
- **It belongs to the catalogue, not to any pack.** `pack.toml` is per-pack and cannot own a cross-pack set; the only existing cross-pack primitive, `[pack.dependencies]`, is a *needs* graph, not a *role* set. A profile is catalogue-level metadata, so it sits at the catalogue root next to `packs/` and `Makefile`.
...
- **Flat file, not a dir-per-profile.** A profile is an ordered pack list plus a description; `profiles/<name>/…` is a trivial future migration if a profile ever needs companion files. v1 doesn't pay for it.
...
- **`agentbundle install --profile <name> <catalogue>`** reads `catalogue_dir/profiles/<name>.toml`, expands it to its ordered pack list, and installs them (see D4). `--profile` and `--pack` are mutually exclusive — a small but real argparse change: `--pack` is today a bare `required=True` arg (`cli.py:218`) with no mutex group, so this converts it into a member of a **required mutually-exclusive group** with `--profile`.
- **`agentbundle list-profiles <catalogue>`** globs `catalogue_dir/profiles/*.toml` and prints each profile's id + description (parallel to the existing `list-packs`, [RFC-0003](0003-spec-and-cli.md)).

The CLI delta is therefore three small items: the `--pack`/`--profile` required-mutex conversion, a `list-profiles` subparser, and the `--profile` reader/orchestrator. No build-pipeline, marketplace, or self-host change.
...
### D4 — resolution contract: ordered, one-adapter, all-pre-flight-before-any-write

A profile install is a **thin orchestrator** over the existing single-pack `install.run` path. It inherits install's documented dual-scope contract — *"every scope's preconditions run before any write to either scope"* (`install.py:1-26`) — and extends the same guarantee from cross-scope to **cross-pack and cross-adapter**:

1. **Expand & filter.** Read the profile; resolve each entry's scope (the entry's optional pin, else the pack's `[pack.install] default-scope` via `scope.resolve`). Drop packs already installed at their resolved scope, reporting `already present, skipped` — so single-pack install's refuse-on-reinstall (`install.py:426`) is never triggered and stays untouched. A profile is declarative "ensure present."
2. **Pin one adapter for the whole batch.** Resolve a single target adapter — explicit `--adapter`, else `DEFAULT_ADAPTER` — and **assert every remaining pack's `allowed-adapters` includes it**. Because `_resolve_target_adapter` falls back per-pack to `allowed_adapters[0]` when the default isn't allowed, *not* pinning one adapter could silently split a profile across adapters. If any pack disallows the resolved adapter, refuse before any write, naming the pack and suggesting a compatible `--adapter`. Every pack in a profile install therefore lands on one consistent adapter target.
3. **Validate deps against the batch.** install's required-dep gate checks `[[pack.dependencies.required]]` against union state (`install.py:3203`) — but a pack listed later in the batch isn't in state yet. The orchestrator validates each pack's required-deps against *(current union state ∪ the profile's pack set)*. This is the **one minimal `install.py` change**: the dep gate gains an "also-being-installed in this batch" set parameter. All of this is pre-flight — before any byte is written.
...
**Order is load-bearing, and authored, not computed.** Profiles are an ordered list authored deps-first. A **lint** enforces two invariants: *dependency-completeness* (the set is closed under each pack's required-deps — no "install X first" surprise) and *order validity* (each pack's required-deps appear earlier in the list). Correctness is pushed to author-time lint; the orchestrator stays a dumb iterator with no runtime graph logic.

### D5 — curated-only; no state entity; no schema or contract bump

- **First-party-curated.** The catalogue ships a small starter set (at minimum `solution-architect`); adding a profile is a normal PR adding one `profiles/<name>.toml`.
- **No profile in state.** Each pack writes its normal row in repo `.agentbundle-state.toml` / user `~/.agentbundle/state.toml`; no profile membership is recorded. The state schema stays at v0.3 — **no bump**. (Optional provenance: the existing per-row `install_route` field — written for every scope's row, repo and user — could carry `"profile"` instead of `"cli"`. This is zero-schema-change but not free: `install_route` is a hardcoded literal at `install.py:851`, so it requires parameterizing that write-site. Recommended but severable — see Open question 3.)
...
None — this is additive. No existing state, file, or manifest changes shape. The first profile (`profiles/solution-architect.toml`) and the CLI surface land together; existing per-pack install/upgrade/uninstall is untouched.
...
|---|---|---|
| **A0 — Do nothing** (keep RFC-0001 prose patterns) | none | Cost of delay: the highest-frequency onboarding moment stays N error-prone, scope-aware commands. Honest contender; loses on the mixed-scope kernel. |
...
| **C3 — No order guarantee** | Breaks intra-batch deps and partial-failure consistency. Rejected. |
...
- *It ships and is just sugar no one reaches for.* Mitigation: scope the v1 surface to one proven role profile and the CLI verbs; adding a profile is a one-file PR, so the cost of being wrong is near-zero and removal is `rm profiles/*.toml` + the flag. If after a release no second profile earns authoring, that is the signal it was sugar — and it cost almost nothing to learn.
- *Profiles drift from reality* (a profile lists a pack that changed scope or was renamed). Mitigation: the dep-completeness/order lint runs in CI against the live `packs/` tree; a profile naming a non-existent pack or a broken dep order fails the build.
- *Silent adapter split.* Mitigated by D4 step 2 — one adapter pins the whole batch, refusing before any write if a pack disallows it.
- *Partial install on I/O failure.* Mitigated by deps-first write order (consistent prefix) + per-pack summary; this is the same residual single-pack install already carries, not a new exposure.

**Key assumptions (falsifiable).**

- *The mixed-scope multi-pack install is fiddly enough to warrant encoding.* If an adopter could trivially install the set by hand without scope knowledge, the substantive kernel evaporates and this is pure sugar (→ A0). The spike (below) supports the assumption but it is the crux a reviewer should attack.
- *Role onboarding is frequent enough to clear Principle 4.* This is the weakest pillar and the explicit charter call in Decision 1. The frequency argument *for*: onboarding fires per repo **and** per new team member — for the charter's stated "team of fifty," a role profile is reached every time someone joins or starts a new project, not once a year. The argument *against* (a fair reviewer position): for a solo adopter it is closer to once-per-repo, and "document the patterns better in prose" (A0) might suffice. There is **no spike for this** — it is an unfalsified frequency assumption, called out plainly rather than dressed up.
...
**Drawbacks.** It is convenience over an existing capability, not new capability — it adds a (small) CLI surface and a lint to maintain, and it introduces a second way to express "these packs go together" alongside `[pack.dependencies]` (orthogonal in intent — *role* vs. *needs* — but a reader must learn the distinction). The first-party-only constraint means an adopter who wants their own role bundle still loops `--pack` until the follow-on lands.

## Evidence & prior art

**Spike / de-risk result.** The spike targets **Principle 2** (substantive, not duplicative) — the falsifiable claim that the manual path is genuinely fiddly. (Principle 4, frequency, has no spike and is flagged as an open assumption above and in Decision 1.) I composed the proposed `solution-architect` set by hand against the live catalogue: today it is **four invocations** — `install --pack core` (resolves repo-scope), then `--pack architect`, `--pack research`, `--pack contracts` (each resolves user-scope by its `[pack.install] default-scope`). The adopter must know `core` lands repo-scope while the other three land user-scope, or hit `allowed-scopes` refusals (`scope.resolve` → `ScopeRefused`, `install.py:237-243`). **That scope knowledge is the substantive kernel** a profile encodes once — prose cannot execute it. Verdict: clears Principle 2; Principle 4 remains the borderline call surfaced above rather than papered over.

**Repo precedent.**

- [RFC-0001](0001-bundle-distribution-by-adapter-spec.md) "Common adoption patterns" (`:668-677`) blesses the *pattern* of named pack combinations *as prose* (it lists `core` + governance-extras / user-guide-diataxis / monorepo-extras); the `solution-architect` set is a new instance of that blessed pattern. This RFC promotes the pattern from prose to an executable unit. RFC-0001's anti-mega-bundle reasoning (`:388-392`) — packs stay independently installable so downstream can depend on specifics — is honored: a profile is a thin pointer list, not a pack, and creates no new dependency edges.
...
- [RFC-0004](0004-install-scope-per-pack.md) (per-pack `default-scope`/`allowed-scopes`) and [RFC-0011](0011-pack-allowed-adapters.md) (`allowed-adapters`) supply the per-pack scope and adapter resolution a profile reuses verbatim.
...
- ADR — record the "profiles are catalogue-level, CLI-route-only, curated source files" decision and the meta-pack distinction.
- Spec — `docs/specs/pack-profiles/` for the `profiles/<name>.toml` schema, the `install --profile` / `list-profiles` CLI surface, the dep-gate batch parameter, and the dep-completeness/order lint.
