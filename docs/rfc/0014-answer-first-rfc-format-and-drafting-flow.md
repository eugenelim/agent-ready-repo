...
- **Why now (SCQA):**
  ...
  - *Complication* — three recurring failures: (1) the decision a reviewer must make is buried inside "Proposal", tangled with design detail; (2) open questions pile up unowned (RFC `0005` carries **11**, `0010` **8**, `0008`/`0003` **5**); (3) drafts miss obvious things, so the reviewer does the catching.
...
- **Decisions requested:**
  ...
  2. **Upgrade the drafting flow** — Self-Ask on research-resolvable questions, widen to ≥2–3 genuine options, spike the riskiest assumption, cap + own open questions (~≤3), and a pre-handoff gate: a **citation-integrity protocol** + a **verify-before-you-assert** check + a self-scored YES/NO completeness checklist + a **mandatory** `adversarial-reviewer` dispatch in fresh context? — *Recommended: yes; `adversarial-reviewer` is mandatory (resolved this review).* *Decide-by: this review.*
...
## Options considered   — ≥2 genuinely distinct incl. do-nothing (+ cost of delay); trade-offs stated up front; starred-recommended table encouraged
...
2. **Research + de-risk checkpoint** (extends today's step 3): repo + external sweep *(unchanged)*; **Self-Ask** to resolve research-answerable questions so they never reach the human; **widen options** to ≥2–3 genuinely distinct incl. do-nothing; **spike the riskiest assumption** or state why none is needed; **cite as you go** — when a sweep (or a research subagent) surfaces a source, fetch it and confirm it resolves *and* contains the borrowed claim before that claim enters the draft; **recommend** per remaining decision (default + reasoning + owner + decide-by), capping open questions at ~3. Emit in chat; wait for steer *(gate — unchanged mechanism)*.
...
- **Option A — do nothing.** Keep the current template and flow. *Cost of delay:* the three failures above recur on every RFC; reviewer time keeps being spent catching omissions and chasing unowned questions. Rejected.
- **Option B — answer-first template + upgraded flow, reusing existing machinery (recommended).** Targets all three failures directly; net-new surface is small because `adversarial-reviewer` and the `work-loop` trio already exist. Trade-off: slightly more structure up front; mitigated by right-sizing small RFCs to one-liners.
...
- *Widen options before converging:* single-option decisions failed ~50% in [Nutt 1999](https://cebma.org/assets/Uploads/Nutt-1999-gecomprimeerd.pdf).
