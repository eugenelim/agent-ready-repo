...
- **Related:**
  ...
  - [`tools/hooks/session-start.py`](../../tools/hooks/session-start.py) — explicitly renders `patterns.jsonl` with `--show-knowledge` for curation; normal session start does not load entries

> **Research alignment note (2026-08-12):** This Draft predates the lifecycle
> and topic model in
> [`docs/architecture/knowledge-capture.md`](../architecture/knowledge-capture.md).
> Its security controls and operator-reviewed promotion remain relevant, but
> its assumptions that capture belongs only to `work-loop` and that flat
> `patterns.jsonl` is the permanent canonical model are no longer recommended.
> Amend this RFC around shared candidates, stable topics, occurrences,
> freshness, and explicit enquiry before acceptance or implementation.
...
*Situation:* `work-loop` (its enhanced capture step, landing in a parallel change) now filters practitioner lessons into `docs/knowledge/patterns.jsonl` at loop end using an editorial gate ("would a senior engineer put this in an onboarding note?"). The session-start hook does not load these observations automatically; it can render them explicitly with `--show-knowledge` for curation. The schema and curation convention are established in `docs/knowledge/README.md`.

*Complication:* Append-only growth without periodic curation produces well-known decay: entries that say the same thing differently accumulate; scopes that no longer point at real paths mislead curators and future retrieval; patterns that have fired repeatedly enough to warrant a skill or convention stay buried in the JSONL without escalation. As the number of entries grows from tens to hundreds, curation cost and any future task-scoped retrieval noise compound.
...
| D2 | Curation write rights — should `distill-knowledge` be append-only (new superseding entries only, no edits/deletes) or should it act as a mutation gate (edit in place, remove) with operator approval? | **Mutation gate with living contract** | Append-only keeps the file growing with noise indefinitely — superseded entries conflict during curation and future retrieval; stale scopes mislead agents. `distill-knowledge` is the correct owner of mutations: it runs at a checkpoint, proposes changes, and requires operator approval before writing. New additions from `work-loop` and contributors remain append-only; only distill-knowledge may edit or remove. Gaps in IDs are valid; the linter enforces only uniqueness. `docs/knowledge/README.md` will be updated to reflect this contract in the implementing PR. | This review | Confirm or restrict to append-only |
| D3 | Escalation thresholds: what makes an entry a migration candidate? | **Four tiers** (see § Proposal — Escalation tiers) | Graduated tiers encode different cost/value signals: high recurrence → skill; universal applicability → governed canonical context; convention gap → CONVENTIONS.md; enforceable mechanically → lint. "3+ distinct source entries" is the recurring bar — a single occurrence isn't a pattern; two could be coincidence; three across distinct PRs/initiatives is an empirical signal of recurrence. | This review | Confirm thresholds or adjust |
...
1. **Semantic duplication.** Two loops independently capture "always run `make lint-ruff` before opening a PR" as separate entries. Both appear during curation or future task-scoped retrieval; the second adds no signal unless its independent provenance is retained as recurrence evidence.
...
3. **Supersession opacity.** An entry says "use `WidgetV1`." A later entry says "use `WidgetV2` (supersedes K-0012)." Curation and any future retrieval see both; the agent sees a contradiction unless it reads the body carefully.
...
| **Universal** | Entry's `scope` is `"*"` (or near-`*`), `kind: "pattern"`, no qualifier limiting it to a specific file path or tech stack | Propose promotion to the governed canonical surface that owns it (for example AGENTS.md, CONVENTIONS.md, or a skill); never hardcode observational prose into automatic session-start output |
...
| Option | What it is | Trade-offs | Selected? |
|--------|------------|------------|-----------|
| **Append-only (original convention)** | distill-knowledge appends a new superseding entry; never edits or removes | Zero mutation risk; history is always preserved. Cost: superseded and stale-scope entries keep misleading curation and future retrieval; the file grows indefinitely; a 500-entry file creates substantially more curation noise than a curated 80-entry one. | No |
| **Mutation gate with living contract (recommended)** | distill-knowledge may edit in place and remove — all operator-approved; additions from work-loop remain append-only; git commit is the audit trail; IDs are immutable and gaps are valid | Keeps the observational corpus accurate for curation and future task-scoped retrieval. Cost: requires operator to review and approve each mutation. | **Yes** |
| **`supersedes` field + soft-deletion flag** | Add `"supersedes"` and `"retired": true` fields to the schema | Machine-checkable by linter; an explicit retriever can filter retired entries without removing them. Cost: schema bump; linter and consumer updates; two new field decisions (required vs. optional, behavior on absent flag); the file still grows; reindexing is not addressed. | No |
...
- *Explicit curation and future task-scoped retrieval surface all matching active entries, including contradictory superseded ones.* The current hook renders on explicit `--show-knowledge` only and has no supersession lifecycle. If a future retriever filters an enforced lifecycle correctly, the retrieval-noise motivation weakens, though deduplication and graduation remain valuable.
...
- `tools/hooks/session-start.py`: the explicit curation renderer for matching entries; normal session start does not load knowledge.
...
1. **Stale-scope matching — five sub-decisions for the implementing spec.** The explicit renderer already supports comma-separated scopes by trying the raw glob and then each trimmed segment; it does not enumerate the working tree. Stale-scope retirement still needs a separate protocol: **(a) Placement:** describe the installed skill's matching behavior in the skill rather than depending on repo-local `tools/` imports. **(b) Semantics:** match the renderer's `fnmatch.fnmatch` behavior, including `*` crossing path separators and treating a multi-scope entry as live when any segment matches. **(c) Performance:** enumerate the working tree once and reuse the path set for all topics or entries. **(d) Confinement and cycles:** resolve every traversed directory, prune targets outside the repository, reject reparse-point escapes, and keep a visited-resolved-directory set to prevent junction cycles. **(e) Gitignored files:** derive the path set from tracked files plus non-ignored untracked files so build residue cannot keep a dead scope alive. **Owner:** implementing spec. **Decide by:** spec authoring.
