...
| `make build-check` never reaches green: `run-semgrep-gate` fails under `--strict` | Six runs at host load 39-140. The timing-out file set **varies**: run 1 gave `atlassian/_client.py` + `linear/linear.py`, run 3 `core/loop-cohort.py` + `linear/linear.py`, run 6 `loop-cohort.py` alone; run 2 passed semgrep outright. Decisive test: `loop-cohort.py` (2704 lines, the largest script in the tree) passes the identical gate invocation **in isolation with exit 0** at load 46, and it is not in this branch's diff (`git diff --name-only 236ae549c..HEAD -- .../scripts/` is empty). The one Python file this branch does add, `tests/roster/test_agent_skill_engineering_consumer_integrations.py`, also exits 0 under the same invocation. | `environmental`, **not carried**. Per-file rule timeouts under contention on a shared host, which the gate's own message describes: "a target it could not fully parse, or a rule that exceeded its per-file time budget… not findings in your code". Every timing-out rule is an `insecure-transport` requests rule; the added file imports no HTTP client. No `SEMGREP_EXCLUDE` was widened — ADR-0102 requires a stated residual and a retirement trigger for that, and a load artifact is neither. |
| `audit-npm` exited 2 on two of six runs | `npm audit --json` in `docs-site/` returned `npm warn audit network timeout at: https://registry.npmjs.org/-/npm/v1/security/advisories/bulk`. On the next attempt `tools/audit-npm.py --root .` exited **0**: "docs-site: no blocking advisories", "web: no blocking advisories", 2 lockfiles audited at `--audit-level=moderate`, canary probe live. | `environmental`, **not carried**. The advisory endpoint intermittently times out. The tool exits 2 by design for this: its docstring separates "could not run" from exit 1 "found advisories", precisely so a transient is not reported as a finding. Neither committed lockfile is in this branch's diff. |
