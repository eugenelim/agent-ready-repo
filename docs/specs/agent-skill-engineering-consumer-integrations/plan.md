...
| Which target primitive the declaration's `providers` names | `packs/architect/pack.toml:88` — `providers = ["skill:project-knowledge"]`, naming the primitive that actually serves the seam in the target pack rather than a wrapper around it | **Match: `providers = ["skill:ase-okf-reference"]`.** `packs/agent-skill-engineering/.apm/skills/ase-okf-reference/SKILL.md` is the only primitive in the target pack carrying `agent-skill-engineering-reference/v1`, and CAT-V-019 resolves each `providers` ref against `<target>/.apm/skills/<name>/` when the target is in the catalogue (`verify.py:2165`), so the ref must name a real primitive. Naming the generated router *here* is build-time declaration, which ADR-0097:173-175 admits as exactly that — "*build-time declaration, provenance, and bounded-traversal precedents*". ADR-0097:97-99 constrains what a **consumer body** may rely on, and AC5 checks that separately over the two `SKILL.md` bodies |
