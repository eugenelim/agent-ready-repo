...
- **Status:** Shipped (§ Follow-ons' three consumer-side security-review controls all closed 2026-09-08; their dispositions are recorded in [`notes/consumer-security-review-closures.md`](notes/consumer-security-review-closures.md), because `workspace.toml [backlog].closed` no longer carries closure records. The fourth follow-on, RFC-0097:189's behavioural half, is unaffected: it remains open under its original slug, which AC16 and its roster test pin. No body line changed. Not a supersession — every decision here stands. Work-loop's handshake rules moved to `packs/core/.apm/skills/work-loop/references/skill-engineering-provider.md` on 2026-09-22; AC2-AC5 are unaffected and unedited, because the consumer declaration they read — the contract version, both task kinds and the absent-case diagnostic — stays in `SKILL.md`, which additionally forbids reaching any provider before that reference is loaded) <!-- Draft | Approved | Implementing | Shipped | Archived -->
- **Owner:** eugenelim
