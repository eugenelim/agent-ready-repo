...
- **Status:** Shipped (§ Follow-ons' three consumer-side security-review controls all closed 2026-09-08; their dispositions are recorded in [`notes/consumer-security-review-closures.md`](notes/consumer-security-review-closures.md), because `workspace.toml [backlog].closed` no longer carries closure records. The fourth follow-on, RFC-0097:189's behavioural half, remains open under `docs/product/intents/agent-skill-consumer-boundary-hardening.md`. No body line changed. Not a supersession — every decision here stands) <!-- Draft | Approved | Implementing | Shipped | Archived -->
- **Owner:** eugenelim
