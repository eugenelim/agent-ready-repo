...
- **Documentation, execution-pointer, and projection wiring: goal-based check.** The roster guard verifies the five closed rule-bearing sources, the how-to's two clauses, and `work-loop/SKILL.md`'s pointer; `FORCE=1 make build-self` regenerates projections after source changes, and existing pack/repository gates validate the generated and release surfaces.
