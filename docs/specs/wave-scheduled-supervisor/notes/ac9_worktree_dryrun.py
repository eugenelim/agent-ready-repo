...
run(["git", "config", "user.email", "a@x"], repo)
run(["git", "config", "user.name", "a"], repo)
...
d_safe_disjoint = lc.dispatch_decision(
    ["cannot-collide", "cannot-collide"], merge_tree_clean=disjoint
)
d_safe_overlap = lc.dispatch_decision(
    ["cannot-collide", "cannot-collide"], merge_tree_clean=overlap
)
