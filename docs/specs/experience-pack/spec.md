...
- [x] `tools/lint-design-craft-agnostic.py` → `tools/lint-experience-agnostic.py` and `tools/test-lint-design-craft-agnostic.py` → `tools/test-lint-experience-agnostic.py`; the scan root is retargeted to `packs/experience/` and the override env var `DESIGN_CRAFT_ROOT` → `EXPERIENCE_ROOT` (the self-test sets it).
- [x] Both CI steps (`build-check.yml`, `build-check-windows.yml`) name the renamed tool; the lint runs clean against `packs/experience/`.
- [x] The two **provenance pointers stay pinned, not retargeted**: the RFC-0033 citation in the lint docstring and the `(design-craft-pack AC8)` tag in the CI step name (both point at frozen sources with no `experience` equivalent).

### D2 — the connective skills + shared quality floor

- [x] Three new skills exist as pure-markdown `SKILL.md` + `references/` + `assets/`: `map-customer-journey`, `blueprint-service`, `map-screen-flow`; each projects to all seven allowed adapters.
...
- [x] Each connective skill is standalone-useful (elicits inputs inline when persona/outcomes/intents are absent) and detect-and-degrades (names services/components textually when `architect`/`contracts` are absent).
- [x] Each connective skill's `SKILL.md` **declares its inputs and a `Consumed by:` seam** (the downstream artifact/skill it feeds), so the flow thread is readable end-to-end (the verification target for the design-flow-completeness AC above).
- [x] `quality-floor.md` is the **single pack-shared floor**, with **all three sections intact** — handle-all-states, the accessibility floor (WCAG pointed-to), and reduced-motion; `map-screen-flow` and `interaction-design` reference it **sibling-relative** (`../design-critique/references/quality-floor.md`) and `voice-and-microcopy` defers **by-name** (cross-pack), with no private sibling copy. **Build-time amendment (RFC-0050 § Amendments, Accepted 2026-06-29):** the floor stays resident at `design-critique/references/quality-floor.md` rather than moving to a "pack-level `references/` location" — a pack-level `references/` dir **does not project** (the build projects only `skills`/`agents`/`hooks`/`hook-wiring`/`commands`), so the resident file referenced sibling-relative is the buildable realization of the RFC's pack-shared intent.
- [x] `permission/denied` is documented as an *additional* gated-screen state extending the floor, not a replacement list.

### D3 — the platform/surface axis

- [x] `map-customer-journey`, `map-screen-flow`, and `aesthetic-direction` each carry a `surface` value (`responsive-web | iOS | Android | cross-platform`) that changes *what the method asks* (consult Apple HIG / Material 3 / MDN responsive / shared-then-adapt), and **points to** those standards — grep-confirmed: no reprinted breakpoint/HIG/Material values.
- [x] `map-internal-process` does **not** carry the platform/surface axis.

### D4 — the per-screen brief

- [x] `map-screen-flow` emits **one per-screen brief per screen** using the `0048-notes/07` brief format carried in its `assets/`, split into a **shared design contract** (referenced, not copied, by every brief) and a **per-screen spec**.
...
- [x] `aesthetic-direction` grounds each named goal in a stable referent (persona + precedent + standards + platform conventions) and records *what grounds each goal*; it stays method-not-values (no palette/font/value printed).
- [x] `design-critique` gains a **taste mode** (evidence-grounded critique against the grounded aesthetic reference + platform fit) and stays an **interactive authoring-time skill** — not a reviewer agent, and explicitly not "fresh-context".
- [x] `voice-and-microcopy` (resident in `product-engineering`) consumes the screen flow's per-screen state matrix when present (copy per screen × state, keyed to it) and behaves as today when absent; the `experience` and `product-engineering` READMEs cross-link each other.
- [x] `product-engineering` takes a minor version bump (**`0.7.0 → 0.8.0`** — its current version, the RFC's literal `0.6.0 → 0.7.0` predates an intervening PE bump).

### D6 — path resolution

- [x] `pack.toml` ships `[pack.layout.repo] parent = "docs/design"`; there is a new `[experience]` table in the adopter-owned `agentbundle-layout.toml` contract, resolved config → default → discover-by-marker (frontmatter `type:` `customer-journey`/`service-blueprint`/`screen-flow`/`process-flow`).
- [x] Each new artifact-writing skill ships a `references/agentbundle-layout.md` documenting the table; file-per-slug shapes match the RFC (`journeys/`, `blueprints/`, `screens/<slug>-flow.md` + `screens/<slug>/<screen>.md`, `processes/<slug>.md`); each skill surfaces the resolved path before its first write and creates its dir lazily.

### D7 — the `experience-reviewer` agent

- [x] An `experience-reviewer` agent exists under `packs/experience/.apm/agents/`: forked-context, read-only, flags-never-rewrites; its name is **not** a substring of `design-reviewer` or any core/discovery reviewer.
- [x] Its `description:` **leads with a design-time-only cue** (reviews journey / screen-flow / per-screen-brief / aesthetic artifacts; never code diffs; never architecture design docs).
- [x] Its review list carries the grounded aesthetic reference (D4) + platform fit (D3) + cross-brief coherence (the D4 consistency pass) + the **full quality floor — handle-all-states, accessibility, and reduced-motion** (the promoted three-part floor; accessibility is the only independent a11y check between human-value-add gates).
- [x] The agent ships its eval surface. **Build-time amendment (RFC-0050 § Amendments, Accepted 2026-06-29):** there is no measured *agent*-eval shape in the catalogue — agents are direct-file primitives dispatched by **role-match** (not Skill-router activation), and `[pack.evals]`/`run-pack-evals.py` are **skills-only** (the model agent, architect's `design-reviewer`, ships no eval files either). So the agent's eval surface is its **gradable review-list output contract** (the four lenses + the verdict/severity block in its `.md`) exercised by the **manual-QA dispatch** the Testing Strategy names (dispatch against a sample journey + screen flow; observe severity-tagged findings across all four lenses incl. accessibility) — not a `eval_queries.json`/`evals.json` pair, which would be an orphan (no projecting home under a flat-file agent, nothing to measure it). Wiring a measured agent-eval harness is a future tooling RFC.
- [x] RFC-0048's lens-team roster (line 236) reflects UX/design as *skill + an opt-in forked `experience-reviewer`* — a tracked amendment **within RFC-0048, which is still Open**; this spec verifies the reconciliation holds at build time (verified: RFC-0048:236 records `experience-reviewer` + the `experience` pack — see the RFC-0048-Open assumption below for D7's load-bearing-vs-optional conditional).
...
- [x] The handover is **instructions, never pixels/values**; it names tool *categories*, endorses/requires none — grep-confirmed clean under the agnosticism lint.
...
- [x] It writes to `<parent>/processes/<slug>.md` (frontmatter `type: process-flow`), cross-references the service blueprint by-name when a process is customer-triggered, and is the producer of `frame-intent`'s "current-state process map" input.
- [x] The journey skill is named `map-customer-journey` (customer/end-user-scoped; employee-journey mapping is out of v1).

### D10 — the `interaction-design` craft skill

- [x] An `interaction-design` craft skill exists (pure-markdown, agnostic) — **one coarse skill covering the pillars**, not a fan-out: feedback & response (incl. the Doherty-Threshold perceived-performance lens, design-time only), input & forms, **component/screen state machines** (statechart-modeled, authored as a mermaid `stateDiagram-v2` in the brief — **no state-management library reprinted**), **motion & micro-animations** (purposeful motion that communicates state, pointing to Material/HIG motion guidance, **honoring the quality floor's reduced-motion rule** and **reprinting no durations/easing/curves**), navigation-as-behavior, gesture & pointer affordances (on the D3 platform axis), and cognitive-law fit (cross-referencing `design-critique`'s `heuristics.md` + Laws of UX, not duplicating).
- [x] It **references** (points to, does not author as standalone skills) recognized **onboarding** (progressive onboarding, empty-state-as-onboarding, coachmarks) and **search-interaction** (typeahead, faceted filtering, zero/error results) pattern families a per-screen brief can invoke.
- [x] It **enriches the per-screen brief** (the interaction/behavior section) and **defers to the shared quality floor** rather than emitting its own file-per-slug artifact, so it needs **no new D6 layout entry**.
- [x] It is demonstrably distinct from `layout-and-information-architecture` (structure/wayfinding), `aesthetic-direction` (visual taste), and — the load-bearing carves — the **quality floor** and **`map-screen-flow`**: the three state homes are the quality floor (the state *set* / reduced-motion *rule*), `map-screen-flow` (the *cross-screen* routing), and `interaction-design` (the *in-component* state machine + motion + feedback). Macro-flow (`map-screen-flow`) vs micro-behavior (`interaction-design`), since both touch states/transitions/errors.

### Packaging, evals & governance

- [x] `[pack.evals].skills` gains the new authoring skills (`map-customer-journey`, `blueprint-service`, `map-screen-flow`, `map-internal-process`, `interaction-design`); each ships `evals/eval_queries.json` (trigger) + `evals/evals.json` (Tier-4 judge). `run-pack-evals.py` measures them.
- [x] `[pack.install]` is unchanged (user-scope default; same `allowed-adapters`; declared order load-bearing).
- [x] A `docs/product/changelog.md` `[Unreleased]` entry records the rename + the new skills/agent/handover + the PE wiring.
- [x] `docs/specs/README.md` lists `experience-pack` in the active list.
...
- Technical: the canonical quality floor lives at `packs/experience/.apm/skills/design-critique/references/quality-floor.md` (post-rename) and is three-part (states + accessibility + reduced-motion); it stays resident there as the single pack-shared file (see the D2 floor AC build-time amendment — a pack-level `references/` dir does not project) (source: that file; RFC-0050 § D2 / § Amendments).
