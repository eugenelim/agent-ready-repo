...
- [ ] **AC1.** `discovery-lead` ships as an **agent definition** and `discovery-loop` as a **skill**, both
...
- [ ] **AC2.** The recursion is **data, not runtime**: the plan-tree's `intent` slots nest via `parent_id` and
...
- [ ] **AC3.** The sidecar schema **definition** ships as a `references/sidecar-schema.md` file **carried in
...
- [ ] **AC4.** The schema defines the slots with their field-sets: **blackboard** (`{id, type, lens, status ∈
...
- [ ] **AC5.** The **three status namespaces** are partitioned onto distinct fields, not one drifting enum: a
...
- [ ] **AC6.** A **plan-tree template** ships as a carried **`assets/` file** (not just a field-list): a node =
...
- [ ] **AC7.** **One plan-tree per initiative** (a forest, not a master tree): one directory per initiative
  ...
  roadmap).
- [ ] **AC8.** The discovery-workspace is **durably checkpointed at each round and each consent gate** (not per
...
- [ ] **AC9.** Anti-drift is structural: the **controller is the principal slot-writer** (cross-pack lenses emit
...
- [ ] **AC10.** A consent gate (G0, G1.5, G2) is a **pause, not a runtime**: `discovery-lead` writes the
  ...
  fires.
- [ ] **AC11.** **Rejection/recovery with cascade-invalidation** runs in the controller's own context: emit
...
- [ ] **AC12.** The verdict is a **typed enum**, each row with its own transition: **approve** /
...
- [ ] **AC13.** **Two integrity guards bind every row**: (1) **impact-before-blast** — any verdict that would
...
- [ ] **AC14.** **Persistence is two-tier**: Tier 1 — the working `_state/` store is durable while the
...
- [ ] **AC15.** **Resume is specified** (a load + a status read, no runtime): (a) **entry** — invoke
...
- [ ] **AC16.** The `meta` block and each plan-tree node carry `round`/`round_cap` and
  ...
  sub-ideas).
- [ ] **AC17.** **Every bound is a pause-and-confirm/override gate, not an auto-terminal**: hitting a bound sets
...
- [ ] **AC18.** `discovery-lead` right-sizes **solo** (holds the blackboard in one context, switches lenses
...
- [ ] **AC19.** The discovery roster is **loop-scoped** per RFC-0048's roster table:
...
- [ ] **AC20.** The CHARTER's "three reviewers is the ceiling" stays a **`work-loop`/code-review cap**; the
...
- [ ] **AC21.** Two thin PE skills ship: **`explore-options`** (divergence — *generate* N candidate product
...
- [ ] **AC22.** `de-risk-intent` gains a **validation-hook field**; `decompose-intent` optionally gains a
...
- [ ] **AC23.** **Provisional-spine emission at G2**: the decision brief is emitted as a **connected
...
- [ ] **AC24.** The **decision-brief template carries a required, structured success-metrics / North-Star slot**
...
- [ ] **AC25.** **Verdict write-authority (no forged consent).** The human verdict is written through a
...
- [ ] **AC26.** **Decision-log as a real audit trail.** The decision-log slot is **append-only** with **per-row
...
- [ ] **AC27.** **Non-degradable security lens on a boundary.** The `discovery-threat-reviewer`'s *depth* is tied
...
- [ ] **AC28.** **Lens-write integrity (no blackboard poisoning).** A lens may only **propose**
...
- [ ] **AC29.** **Cascade-invalidation circuit-breaker.** Cascade re-runs **count against the cost budget**, and
...
- [ ] **AC30.** **`reversibility-class` is an enumeration** (`reversible` / `costly-to-reverse` /
  `one-way-door`), not free text; `one-way-door` binds to a **mandatory consent gate** regardless of
  which gate it arose at.
- [ ] **AC31.** **Sidecar data-handling / classification.** The spec **classifies each slot** (`public` /
...
- [ ] **AC32.** **G3 handoff** to `work-loop` is wired: `discovery-loop` emits a brief → `new-spec` →
  `work-loop` (unchanged; different inputs, verifier, autonomy posture).
- [ ] **AC33.** **The self-coverage gate** (RFC-0051) is wired as `discovery-loop`'s **pre-G2 phase**, and this
...
- [ ] **AC34.** **The traceability slot** this spec defines is consumed by **child-4's lint**
  (`docs/specs/traceability-lint/`), and the D3 cascade transition walks the **same edges**. **Child-4
  dependency:** the shipped lint performs a **root→leaf reachability** pass (not just edge-presence), so
  the backstop can detect the disconnected-subtree / fabricated-edge failure it is invoked against —
  named here so this spec's backstop claim is not load-bearing on the weaker presence check.
- [ ] **AC35.** **The backlog bridge**: the decision brief decomposes into an **ordered, dependency-aware
...
- [ ] **AC36.** This spec **owns** the `new-spec` **`Discovery:` up-edge header** + the discovery-artifact
...
- [ ] **AC37.** **Producer-before-consumer sequencing**: `new-spec` emits the header + markers **first**, and
...
- [ ] **AC38.** The **BRD/PRD/SRS/FRD/use-case/RTM crosswalk** ships as **guidance in the `discovery-loop`
...
- [ ] **AC39.** A **coordinator ADR** is recorded (the sibling of RFC-0041's ADR-0031 / RFC-0049's coordinator
...
- [ ] **AC40.** The **Diátaxis guide set** is authored (via `new-guide`, under `docs/guides/product-engineering/…`)
...
- [ ] **AC41.** The **loop-skill doctrine** — the two-loop split (discovery vs delivery) + the surfacing
...
- [ ] **AC42.** **`product-engineering` is version-bumped** (`pack.toml` **and** `.claude-plugin/plugin.json`,
...
- [ ] **AC43.** **`make build-self`** projects the new agent + skill + references + assets to every adapter and
  the dry-run drift gate is clean.

**Validation run (the empirical spec gate).**

- [ ] **AC44.** The implementing PR runs **one structurally-different second example** that **forces a cap hit**
