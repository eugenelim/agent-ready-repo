...
incomplete reading. The agent-rendered closeout gate reads the same facts the
JSON does, so the rendered surface and the projection cannot disagree either.
...
| --- | --- | --- | --- | --- | --- |
| `decision-record` | Applicable: RFC-0096 §9 scopes Wave 7, this delivery ships its first slice, and §9's objective does not name either follow-on closing here | [`docs/rfc/0096-portable-delivery-artifact-lifecycle.md`](../../rfc/0096-portable-delivery-artifact-lifecycle.md) § Errata | Approver | AC28-AC30: a dated, Approver-signed erratum carrying the four-slice split with what each slice owns, the three follow-on slugs with owners, and the corrected `cooling-brief-child-scope` basis | Closeout verifies all three and that §9's body digest is unchanged |
| `current-architecture` | Applicable: the wave-ownership statement gains the slice split | [`docs/architecture/work-intake-and-artifact-routing.md`](../../architecture/work-intake-and-artifact-routing.md) | maintainer | AC24 and AC25 | Closeout verifies the three pinned strings, the absent string, and the four named slices |
| `user-documentation` (reference) | Applicable: the closeout derivation a reader relies on changes | [`guides/core/reference/work-intake-routing-and-lifecycle.md`](../../../guides/core/reference/work-intake-routing-and-lifecycle.md) | maintainer | AC26 | Closeout verifies the literal statement |
| `user-documentation` (workflow instructions) | Applicable: the agent-rendered closeout gate reads the changed facts | [`packs/core/.apm/skills/workspace-status/SKILL.md`](../../../packs/core/.apm/skills/workspace-status/SKILL.md) | maintainer | AC14, AC15, AC16 | Closeout verifies the exclusion statement, the removed rationale, and the withholding rule |
...
- Every criterion in "The shared cooled-exclusion derivation" and "The
  agent-rendered closeout gate" is measured on both the `status` and the
  `reconcile` invocation, which are the two that emit the `closeout` block.
...
- **Preservation criteria carry a mutation, not a red stub.** AC2, AC5, AC7,
  AC11, AC13, AC17 through AC24, and AC25 assert that something already true
...
assertion recorded, not inferred from an exit status. Compiled red stubs, 13:
AC1, AC3, AC4, AC6, AC8, AC9, AC10, AC12, AC14, AC15, AC16, AC26, AC27.
Preservation with a mutation row, 14: AC2, AC5, AC7, AC11, AC13, AC17-AC24,
AC25. `no stub (mode)`, 4: AC28-AC31. Total 31; uncovered none.

An earlier run of this suite mislabelled a criterion red on a
`FileNotFoundError` raised inside the fixture helper rather than a failing
assertion, and an earlier draft's tally was wrong by two. Every verdict is now
recorded in the plan with the assertion that failed, because a red for the wrong
reason is as useless as a green for the wrong reason.
...
- [ ] **AC13 — The rest of that file is undisturbed.** That file defines exactly
  67 functions whose name begins `test_`.
...
- [ ] **AC26 — The architecture surface names the four slices and what each
  ...
  `docs/architecture/work-intake-and-artifact-routing.md` contains `Wave 7a-i`,
  `Wave 7a-ii`, `Wave 7b`, and `Wave 7c`, and states for each the objective it
  owns.
...
- [ ] **AC30 — The erratum registers the three open follow-ons and corrects the
  ...
  entry misattributes its constraint to AC46's pinned pair when the finding-code
...
- **Process: a shipped spec directory is frozen as a unit, and this delivery
  ...
  missing anchor and no Status edit is licensed. The erratum is the durable
  record of closure. (source: `docs/CONVENTIONS.md:183-199` and its § "A spec
  directory freezes as a unit".)
