...
- **Agreement and prohibition over shipped content (AC-0009, AC-0010, AC-0011, AC-0012, AC-0013, AC-0020):** TDD across `test_rendered_page_capture_contract.py`, `test_rendered_page_shipped_content_limits.py`, and `test_rendered_page_reviewer_sight.py`. AC-0012's scope is fixed by the criterion; its vocabulary is read from the reference's `channel-name-forbids` row, so removing that row reds the guard instead of silently emptying it.
...
- [ ] **AC-0011.** The evidence manifest's viewports field records the channels covered, stated as width predicates, in both `SKILL.md` and the journey's manifest inventory. Construction test: `test_manifest_viewports_field_records_channels`; fixture: both shipped inventories.
- [ ] **AC-0020.** The `SKILL.md` and journey manifest inventories state the same channel set, and the check fails when they differ. Construction test: `test_both_manifest_inventories_state_one_channel_set`; fixture: both inventories with one perturbed.
