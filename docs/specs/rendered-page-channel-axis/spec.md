...
- [x] **AC-0016.** The `rendered-page-inspection` case's `assertions` array carries a channel-coverage assertion. Construction test: `test_the_harness_expects_channel_coverage`, which checks that an assertion names channel coverage; its sibling `test_the_harness_no_longer_grades_against_a_height_only_floor` pins the absence of the two superseded strings. Neither instantiates a pre-change case: the first is satisfied incidentally by the reworded first assertion, so its must-red half is the second test's absence pin rather than a fixture.
