...
- [x] **AC-0003.** A run records which of the two channel bases it used, taken from the input rather than inferred from the capture set, and carries it to the surface AC-0011 names. Construction test: `test_channel_basis_is_recorded_not_inferred`; fixture: a run supplying no breakpoints and a run supplying them.
- [x] **AC-0004.** The breakpoints a run derives channels from are supplied by the adopter; the step discovers none. Construction test: `test_breakpoints_are_adopter_supplied`; fixture: the shipped channel rule rows.
