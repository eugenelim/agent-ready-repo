...
- **Eval harness (AC-0016):** TDD in `test_rendered_page_reviewer_sight.py` — a parse of `evals/evals.json` asserting channel coverage among the expected behaviours. The delivery checks below read manifests and changelogs and would pass over a harness that never mentioned the axis.
- **Adopter guidance (AC-0018):** goal-based check — the guide is prose walked against a non-repository surface, so the repository's documentation gates are the one-liner that proves it is well-formed and its links resolve.
...
<!-- Channels -->
- [ ] **AC-0001.** When the adopter declares breakpoints `b1 < ... < bn` for a surface, the required channels are exactly the bands those breakpoints bound, with each boundary value belonging to the wider band: `<b1`, then `>=bk` and `<bk+1` for each adjacent pair, then `>=bn`. Origin: the breakpoint values as the adopter declares them, in CSS pixels; the boundary convention follows the mobile-first `min-width` semantics the pack's own `responsive-layout` skill states. Construction test: `test_declared_breakpoints_bound_the_required_channels`; fixtures: a one-breakpoint list, a three-breakpoint list, and a capture taken at a breakpoint value exactly.
- [ ] **AC-0002.** When the adopter declares no breakpoints, the required channels are exactly two — a narrow channel at a viewport width of at most 480 CSS pixels and a wide channel at a viewport width of at least 1024 CSS pixels. Origin: the pack's own `--breakpoint-sm` and `--breakpoint-lg` tokens, both compared against the browser viewport's width in CSS pixels. A width between those bounds satisfies neither channel. Construction test: `test_fallback_channels_are_the_two_shipped_bands`; fixture: the shipped channel table, plus a width of 768 satisfying neither band.
...
- [ ] **AC-0011.** The evidence manifest's viewports field records the channels covered, stated as width predicates, in both `SKILL.md` and the journey's manifest inventory. Construction test: `test_manifest_viewports_field_records_channels`; fixture: both shipped manifest inventories.
- [ ] **AC-0012.** No channel name in the shipped channel table, and no value in either manifest viewports field, contains any of `mobile`, `tablet`, `desktop`, `phone`, `laptop`, `iphone`, `ipad`, or `android`. Construction test: `test_no_channel_is_named_for_a_device`; fixtures: the shipped channel table and both manifest fields, plus a seeded device-named channel that must red.
- [ ] **AC-0013.** Lens 6 of the reviewer agent names `viewport-width` among the recorded fields the reviewer must use, in the sentence that today names `scroll-position`. Construction test: `test_reviewer_lens_reads_the_width_axis`; fixture: the lens section alone, not the whole agent file.
...
- Technical: the contract is markdown tables read by
  ...
  whose `satisfies()` already parses `<=`, `>=`, `<`, `>` and `=` predicate cells,
  so a width predicate needs no new parser
  (`frontend_engineering_rendered_page_rules.py:141-153`).
