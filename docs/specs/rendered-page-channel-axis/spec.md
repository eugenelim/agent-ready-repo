...
- [ ] **AC-0008.** Removing the channel table, or the generalized pair rule row, from the reference makes the pack's capture-set checks fail rather than skip the rule the removed row governs. Construction test: `test_the_channel_requirement_is_shipped_content` and `test_every_rule_row_the_walk_reads_raises_when_deleted`; fixtures: the reference with each rule row the completeness walk reads removed in turn, and the reference with the whole `## Channels` section removed. Deleting a row must raise, not skip the rule it governs; a row present and set to anything other than `required` is a stated decision and is honoured.
