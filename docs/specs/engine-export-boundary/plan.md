...
(spec and plan) is a Draft sibling carrying four `build/tests/` references that
...
- Add the case this task exists for: the **relocated** shape
  `…/tests/build/fixtures/packs`, which the old substring guard misses and the
...
The engine copy's exclusion value is `("tests/",)`, **not**
`("agentbundle/build/tests/",)`: that call collects
`cfg.source / "packages" / "agentbundle"`, so after T1 the relocated suite sits
directly under the vendored root.
