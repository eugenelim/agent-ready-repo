...
- **`check-workspace` routing fix: goal-based.** `grep "frame-situation" packs/core/.apm/skills/check-workspace/SKILL.md` returns a hit on the `strategy` row (line 98 or equivalent); the old text "run product-strategy pack skill" is replaced with the `frame-situation` routing note.
- **Manual QA — skill invocation.** Cold-prompt each of the 9 skills with a minimal context. Record that the output contains:
  ...
  - For `run-okr-cascade`: at least one `{type = "strategy"}` shaping-queue entry in the output, and the PE pack absent diagnostic when no `frame-situation` is installed.
...
- [ ] **`run-okr-cascade`** names and implements the cross-pack routing contract (RFC-0063 §Cross-pack routing contract): commits `okr-cascade.md` to `docs/product/shaping/`; appends gap entries as `{slug = "<gap-slug>", type = "strategy"}` (no `needs` field — no-dependency entries omit it per `workspace.toml` format) to the appropriate initiative's `[shaping_queue].backlog` in `workspace.toml` (resolves the single active `["ini-NNN"]` section automatically; if multiple are active, elicits from the user); emits `"frame-situation not found — install PE pack to route OKR gaps into the shaping sequence"` when `frame-situation` is absent; documents the full contract in `references/cross-pack-routing.md`. Note: `frame-situation` is an M2 deliverable and does not exist yet; only the absent-diagnostic branch can be exercised at M4 ship time.
- [ ] **`check-workspace` line 98 is updated** to fix the routing for `type = "strategy"` entries. The current text ("run product-strategy pack skill — requires product-strategy pack") is correct for unprocessed items awaiting a strategy skill, but incorrect for OKR gap entries that have already been produced by the strategy phase and are ready for PE framing. The fix changes the routing to: *"route through `frame-situation` (PE pack — M2); if not yet available, run `frame-intent` as interim."* This mirrors the existing `shape`-type pattern ("`frame-intent` (available now); `frame-situation` (M2, when available)"). Zero existing `type = "strategy"` entries in `workspace.toml` are affected — the type was defined in anticipation of M4. The edit is in `packs/core/.apm/skills/check-workspace/SKILL.md` line 98 only.
