...
`.github/workflows/docs.yml`, `packs/core/pack.toml`,
`packs/core/plugin.json`, `marketplace.json`, `CHANGELOG.md`, and the
...
| Serialize appends under a file lock | Cross-platform locking is disproportionate for a single-session authoring tool; the atomic replace means no writer can corrupt the file, and the duplicate-id lint catches a collision at the next gate. Recorded as spec Assumption 3, not silently dropped. |
