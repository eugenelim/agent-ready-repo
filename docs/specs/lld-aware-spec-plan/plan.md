...
**Depends on:** T1, T2, T3
**Touches:** guides/core/reference/spec-shape-and-lld.md, guides/core/explanation/why-the-plan-owns-the-lld.md

**Tests:**
- Goal-based: a reference guide file and an explanation guide file exist under
  `guides/` at their Diátaxis paths. *(verifies AC: guide files exist)*
...
- Author **two new files** (the LLD additions are a distinct concern from the
  ...
  - `guides/core/reference/spec-shape-and-lld.md` — the `Shape:` vocabulary, the
    nine `## Design (LLD)` categories + the Rollout-owned tenth, and the
    stack-derivation present/absent branch.
  - `guides/core/explanation/why-the-plan-owns-the-lld.md` — why the design lives
