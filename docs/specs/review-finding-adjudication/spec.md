...
- **Status:** Shipped (superseded in part by [`adjudicator-evidence-and-remedy-predicate`](../adjudicator-evidence-and-remedy-predicate/spec.md) — AC3's five-predicate set now has a bounded sixth remedy-mechanism predicate, AC10's terminal indeterminate path now admits a guarded machine-evidence retry, and AC11's exact three eval cases are now four; every other decision stands; and superseded in part by [`[core][2.17.1]`](../../product/changelog.md) — the exact-clean fast path narrows the Boundaries *Always do* rail at line 52, AC6, and AC8 from "every reviewer report, including reports that claim to be clean" to every report except a raw return whose bytes equal `Clean — ready to commit.`; that return is still persisted and validated, only the adjudicator dispatch is skipped; every other decision stands; and superseded in part by [`[core][2.26.23]`](../../product/changelog.md) — the *Always do* rail at line 52, AC6, AC7, and AC8 gain a work-risk condition alongside the existing content conditions: the adjudicator dispatch is required when the change warrants at least one `operational-safety` module, is structural per `work-loop` SKILL.md's pre-EXECUTE footnote 1, or is explicitly requested, and is otherwise available but not dispatched; every report is still persisted and validated, only the dispatch narrows, and the adjudicator additionally refutes on the Authority predicate any finding that names no violated acceptance criterion, repository rule, security property, or concrete defect; every other decision stands) <!-- Draft | Approved | Implementing | Shipped | Archived -->
- **Owner:** eugenelim
