...
- [ ] **AC-0021.** No shipped surface states a band enumeration or a per-route capture floor without naming the declared minimum. Construction test: `test_no_shipped_surface_states_a_superseded_capture_claim`, sweeping every `.md` and `.json` file under `packs/frontend-engineering/.apm/**` plus `guides/frontend-engineering/how-to/inspect-the-rendered-page.md`. For each anchor phrase below, the enclosing paragraph — or for JSON, the enclosing string value — must also name the minimum; the comparison normalizes runs of whitespace first, as the shipped rate guard does, because two of these anchors wrap across a line break in the files that carry them. The anchor set, which is the control's entire reach and is stated here so its blind spot is visible:
  ...
  4. `four times *n + 1* where you declare *n* breakpoints` — § 5a and the how-to, both of which wrap it mid-phrase.
  5. `the narrow and wide fallback bands when none are declared` — the eval case's coverage expectation, which a judge grades against.

  Fixture: the pre-change tree, where all five anchors sit in paragraphs that never mention a minimum. One criterion rather than one clause per surface: four surfaces state some form of this claim, and three consecutive review rounds each found the next instance after the last was closed per-surface. An anchor list is a thing a reviewer inspects; "did the author think of the fourth surface" is not. Adding a surface later means adding an anchor.
