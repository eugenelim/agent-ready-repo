...
- Name no channel in shipped content outside the fallback band names the
  reference declares, and state a clamped band's bounds without the
  `` `name` at <operator> `` shape. A shipped sweep asserts set *equality*
  between the channel names it harvests across `.apm/**` and those two declared
  names, so a worked example in a three-cell table or prose in that shape reds
  it. Widening the sweep's shapes or its expected set to accommodate new prose
  would narrow the guard that also carries the device-name prohibition.
...
- **The reference's prose (AC-0021):** TDD in the capture-contract module, asserting the conditional form of the sentence the rule rows would otherwise contradict. Prose rather than a row, because the rows are already covered and a contradiction between them is invisible to a row assertion.
...
- [ ] **AC-0021.** The reference's `## Channels` prose states the minimum's effect on which bands apply, so the rule rows and the prose above them state one contract. Construction test: `test_the_channels_prose_states_the_minimum`; fixture: the shipped sentence "When they declare none, these two apply", which a rule row reading `drop-bands-below-clamp-lowest-survivor` directly contradicts. Rows alone do not discharge it: every row criterion asserts rows, so the reference can ship the new rule immediately beneath prose saying both fallback bands always apply, with every other criterion green.
...
<!-- Adopter surfaces -->
- [ ] **AC-0008.** `SKILL.md` § 5a states that a surface may declare a supported minimum width and that bands below it stop being required, **and its two superseded claims are conditioned rather than left standing**: the channel count is no longer unconditional, and the per-route floor no longer reads `4(n + 1)` for *n* declared breakpoints. Construction test: `test_the_skill_states_the_minimum_input`; fixtures: the two shipped sentences — "Declare none and two apply", which is false for a fallback surface with a minimum, and the `4(n + 1)` floor, which is false whenever a minimum discards a declared breakpoint. Presence of a new sentence does not discharge this: appending one leaves both false claims in the text an agent performs the step from, and this is the adapter-projected surface, so it outranks the guide's identical sentence in AC-0010. The edit keeps the literal phrase `` `narrow` at ≤480 `` in that paragraph, because a shipped per-shape control matches that exact site.
...
- [ ] **AC-0010.** The shipped how-to walks a surface that declares a minimum and states the required set that minimum produces. Construction test: `test_the_how_to_walks_a_single_channel_surface`, asserting the guide contains the minimum-width input token, a per-route floor of four captures for a single channel, and a floor formula that accounts for breakpoints the minimum discards; fixture: the pre-change guide's two sentences stating that the set is eight captures per route *on the fallback bands*, and four times *n + 1* where *n* breakpoints are declared. The eight is already conditioned on the fallback bands and needs no change; the `4(n + 1)` formula is the sentence this delivery falsifies, because a minimum that discards a declared breakpoint leaves fewer channels than *n + 1*.
