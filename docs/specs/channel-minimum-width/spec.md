...
- [ ] **AC-0021.** No shipped **sentence** states a band enumeration or a per-route capture floor without naming the declared minimum. Construction test: `test_no_shipped_surface_states_a_superseded_capture_claim`, sweeping every `.md` and `.json` file under `packs/frontend-engineering/.apm/**` plus `guides/frontend-engineering/how-to/inspect-the-rendered-page.md`.
  ...
  1. `these two apply` — the reference. Its fallback sentence.
  2. ``For breakpoints `b1 < ... < bn`` — the reference. The unconditional band enumeration, which AC-0002's `[400, 800]` fixture contradicts.
  3. `Declare none and two apply` — `SKILL.md` § 5a and the how-to.
  4. `eight captures per route` — the reference, `SKILL.md` § 5a and the how-to, all three.
  5. `four times *n + 1* where you declare *n* breakpoints` — `SKILL.md` § 5a and the how-to, both of which wrap it mid-phrase.
  6. `needs four times` — the reference. Its variant of the same floor in `## Required captures`, which anchor 5 does not match because it is backticked and omits "where you declare".
  7. `the narrow and wide fallback bands when none are declared` — `evals.json`. A judge grades against it.
  8. `Two here because no breakpoints were declared` — `SKILL.md` § 5a's JS worked example, the block an agent performing the step copies. Its own paragraph unit, so conditioning the prose around it does not reach it, and it captures at 480 — the width below what the surface supports that this delivery exists to stop requiring.

  Fixture: the pre-change tree, where every anchor matches in exactly its listed files and no anchor's sentence names a minimum. One criterion rather than one clause per surface: eight carriers state some form of this claim, and five consecutive review rounds each found the next one after the last was closed per-surface. An anchor-and-carrier map is a thing a reviewer inspects; "did the author think of the eighth carrier" is not.
