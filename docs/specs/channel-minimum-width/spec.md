...
- **The verdict (AC-0019):** TDD in the same module. It asserts `evaluate_capture_set`, not `required_channels`, because the walk is what an adopter's run returns and the derivation is only its input. Paired fixtures, with and without the minimum, so the criterion measures the minimum's effect rather than the walk's baseline.
