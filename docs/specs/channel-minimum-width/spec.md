...
being required, and the lowest surviving band starts at the minimum, so a
1280-minimum surface needs four captures in one channel rather than eight across
two. A breakpoint above the minimum still carries its own band, so declaring a
...
- Technical: adding a rule row trips the `REQUIRED_RULE_ROWS["Channels"]`
  equality control at `test_rendered_page_capture_contract.py:811`, so the
...
- Process: a *primitive* is an adapter-projected artifact — `skills/`, `agents/`,
  `hooks/`, `commands/`, `hook-wiring/` (`docs/CONVENTIONS.md:860-862`). This
