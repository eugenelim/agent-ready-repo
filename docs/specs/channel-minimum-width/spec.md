...
- Let the shipped tables state every rule, and let the rule-reading module read
  ...
  print, not a rule an adopter follows. This delivery keeps them where they are
  — adding a worked clamping example to the reference would red the channel-name
  sweep, which asserts set equality against the two declared fallback names.
