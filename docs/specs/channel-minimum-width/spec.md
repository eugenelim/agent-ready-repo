...
- [ ] **AC-0021.** No shipped surface states a band enumeration or a per-route capture floor without naming the declared minimum. Construction test: `test_no_shipped_surface_states_a_superseded_capture_claim`, sweeping every `.md` and `.json` file under `packs/frontend-engineering/.apm/**` plus `guides/frontend-engineering/how-to/inspect-the-rendered-page.md`.

  Mechanization, in this order, because the order decides whether the check can fail: split each file on blank lines into paragraph units — for JSON, take each string value as one unit — **then** normalize runs of whitespace *within* a unit, then require the unit containing an anchor to also name the minimum. Normalizing before splitting would erase every paragraph boundary and collapse the predicate to "this file mentions the minimum somewhere", which every file this delivery touches satisfies regardless of what its anchor paragraphs say.

  The anchor set, which is the control's entire reach and is stated here so its blind spot is visible. **Every anchor must match at least once across the swept files, asserted per anchor**, so rephrasing a claim instead of conditioning it reds rather than silently vacating the sweep:
  1. `these two apply` — the reference's fallback sentence.
  2. ``For breakpoints `b1 < ... < bn`` — the reference's unconditional band enumeration, which AC-0002's `[400, 800]` fixture contradicts directly.
  3. `Declare none and two apply` — `SKILL.md` § 5a and the how-to.
  4. `eight captures per route` — all three of the reference's `## Required captures` paragraph, § 5a, and the how-to.
  5. `four times *n + 1* where you declare *n* breakpoints` — § 5a and the how-to, both of which wrap it mid-phrase.
  6. ``needs four times `n + 1``` — the reference's variant of the same floor, which anchor 5 does not match: it is backticked and omits "where you declare".
  7. `the narrow and wide fallback bands when none are declared` — the eval case's coverage expectation, which a judge grades against.

  Fixture: the pre-change tree, where every anchor matches and no anchor's unit names a minimum. One criterion rather than one clause per surface: six surfaces state some form of this claim, and four consecutive review rounds each found the next instance after the last was closed per-surface. An anchor list is a thing a reviewer inspects; "did the author think of the sixth surface" is not.
...
- [ ] **AC-0011.** The `rendered-page-inspection` eval case names the declared minimum among the behaviours it expects of a completed inspection. Construction test: `test_the_harness_expects_the_minimum`; fixture: the pre-change case. Its superseded coverage expectation is anchor 5 of AC-0021's sweep — appending an expectation here would otherwise leave a judge grading a correct single-channel run as failing coverage.
