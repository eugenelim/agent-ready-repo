...
**Tests (goal-based):**
- `guides/_shared/reference/adapter-support.md` Gemini row corrected from "Universal layer" to its true tier (skill/subagent/command/hook native). [AC15]
- `gemini` **absent** from `SELF_HOST_ADAPTERS`; `make build-self` produces **no** `.gemini/` tree in this repo. [AC12]

**Approach:**
- Edit `guides/_shared/reference/adapter-support.md` (repo-owned / `EXCLUDED_PATTERNS` — direct edit).
...
**Touches:** guides/_shared/reference/adapter-support.md, AGENTS.md, packs/core/seeds/AGENTS.md
