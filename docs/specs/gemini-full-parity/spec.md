...
- **Dual-copy every contract edit** — `_data/adapter.toml` and `_data/adapter.schema.json` edits are mirrored byte-for-byte into `contracts/` (guarded by `test_contract_files_byte_identical`).
...
- [x] **AC4.** `gemini-command-toml` is added to the `mode` enum at **every** enumerating site in `_data/adapter.schema.json` and its `contracts/` mirror.
...
- Technical: the contract is `_data/adapter.toml` + `_data/adapter.schema.json`, byte-mirrored into `contracts/` (`test_contract_files_byte_identical`). (source: `docs/specs/copilot-full-parity/spec.md` Constrained-by)
