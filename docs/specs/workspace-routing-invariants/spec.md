...
- [x] **AC1.** The engine consumes the accepted Group 2 schemas and exposes typed normalized-intake, workspace-entry, source, and dependency validation without a runtime `jsonschema` dependency.
- [x] **AC2.** Every target entry parses to `path`, `kind`, `source`, `summary`, and `needs`; every supported legacy shape parses to an explicit compatibility record that is never dispatchable.
- [x] **AC3.** Malformed entries, unknown kinds, unsafe paths, and unsupported legacy extensions produce the exact codes in the canonical finding table and do not produce partially usable entries.
- [x] **AC4.** A work entry is dispatchable if and only if it appears exactly once in `work.queue` under an active `ini-NNN`, has `kind = "spec"`, resolves to `docs/specs/<slug>/spec.md`, has a sibling `plan.md`, has spec status `Approved`, has matching workspace/artifact provenance, has no unresolved refresh conflict, has every hard dependency satisfied, and has no reconciliation finding.
- [x] **AC5.** A missing spec produces `missing_artifact`; a missing sibling plan produces `missing_plan`. Both entries are non-dispatchable in status, CLI JSON, MCP output, and work-loop preflight.
- [x] **AC6.** Every artifact-like path, including target, parent, dependency, receipt, refresh-target, brief, and routing-layout paths, is lexically validated and then resolved through the established repository-confinement helper before any read. Absolute, drive-qualified, backslash-based, traversing, noncanonical, symlink-escaped, or symlink-looping paths produce `invalid_artifact_path`; containment is Windows-clean and catches `OSError`, `RuntimeError`, and `ValueError` fail closed.
- [x] **AC7.** Duplicate lifecycle membership produces `duplicate_membership`; no duplicate entry appears in ready, blocked-as-ordinary-dependency, or active output.
- [x] **AC8.** Draft spec in `work.active`, Ready brief closed while scope remains, and every other RFC-fixed impossible membership/status pairing produce `impossible_transition`.
- [x] **AC9.** A brief-derived spec whose `Brief:` metadata differs from canonical `source.parent` produces `provenance_mismatch` and is non-dispatchable.
- [x] **AC10.** Tracker-origin entries whose mirrored `source.ref` or `source.revision` differs from canonical artifact provenance produce `provenance_mismatch`; an unresolved refresh conflict produces `refresh_conflict`. Both fail closed.
- [x] **AC11.** Dependency satisfaction is positive and kind-specific: spec `Shipped`; defect closed with `fixed`; brief `Ready | Executing | Shipped`; intent `Accepted | Fulfilled`; research `Complete`; design `Approved`.
- [x] **AC12.** Missing dependency targets produce `missing_dependency`; dependency cycles produce `dependency_cycle`; unknown or superseded states remain unsatisfied.
- [x] **AC13.** A cross-repository dependency is satisfied only by the exact coordination-receipt encoding above: `path` and `containing_brief` name the same confined local brief; `kind`, `receipt_id`, and pinned `accepted_revision` match the strict receipt; both statuses are `Shipped`; reviewer and timezone-bearing timestamp are present; and `refresh_conflict` is false. Omitting, duplicating, or mismatching any field or block produces `invalid_receipt`; reconciliation performs no network call.
- [x] **AC14.** A Ready brief with no materialized specs is valid and visible in brief status but never appears as dispatchable work.
- [x] **AC15.** Paused or closed initiatives do not dispatch. Their malformed references remain visible to full reconciliation.
- [x] **AC16.** Changing only comments, `summary`, or list order leaves classification, dependency satisfaction, findings, dispatch eligibility, and next action unchanged.
- [x] **AC17.** Two clean subprocesses given the same artifacts, TOML, schema identities, adapter/profile identity, and routing configuration identity emit byte-identical normalized classification/finding output after volatile timing and absolute-root fields are excluded.
- [x] **AC18.** The determinism input identity includes schema ids, contract versions and content digests; the semantic workspace snapshot; canonical artifact status and provenance fingerprints; adapter-contract version; tracker-profile id/version when present; and routing-configuration version. Mutating each field independently changes the identity without changing unrelated classification fields.
- [x] **AC19.** CLI `status`, `reconcile`, and `explain` expose stable finding codes, repository-relative affected paths, dispatchability, and the smallest safe next action. TOML parse, filesystem, provenance, dependency, receipt, configuration, and reconciliation exceptions map to the canonical table, deny dispatch, and never expose an absolute root, raw source payload, traceback, or embedded instruction text.
- [x] **AC20.** `workspace-mcp` includes only AC4-eligible entries in `ready`; every ineligible work entry is absent from `ready` and represented through blocked/finding data without leaking absolute paths.
- [x] **AC21.** `work-loop` preflight refuses missing, unapproved, unregistered, legacy, duplicate, provenance-mismatched, or dependency-blocked work and never reconstructs a contract from workspace comments.
- [x] **AC22.** Active work is resumable only when the entry occurs once in `work.active`, the spec status is `Implementing`, the plan exists, provenance matches, and no fail-closed finding applies; active work is never advertised as queue-ready.
- [x] **AC23.** Repair planning never automatically repairs provenance, authority, missing artifacts, duplicate membership, dependencies, or impossible transitions.
- [x] **AC24.** A queue-to-shipped repair preserves the complete structured entry and its provenance/summary; it removes only dependencies that are no longer live under the Group 2 contract and never writes a bare string.
- [x] **AC25.** `workspace-status` documentation names every implemented finding, explains why it is non-dispatchable, and gives one smallest safe next action.
- [x] **AC26.** ADR-0077, ADR-0078, and `normalized-intake-workspace-contracts` are Approved before this spec is approved.
- [x] **AC27.** Adopter-visible `workspace-status` and `work-loop` changes preserve their existing least-privilege tool authority, treat loaded prose as instruction-inert data, contain reads to the project root, carry no repository-internal RFC/ADR/AC/spec citations, and remain byte-consistent with every generated adapter projection and declared pack/package version.
