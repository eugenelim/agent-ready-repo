...
| Release history | Applicable — the lint, the templates and the reviewer condition ship inside `packs/core` and `packs/product-engineering` | `docs/product/changelog.md` | eugenelim | One free-standing `## [<pack>][<version>] — YYYY-MM-DD` entry per pack, directly beneath `[Unreleased]`, each carrying its own `Highlights` | Both entries present under the versions the change sets |
