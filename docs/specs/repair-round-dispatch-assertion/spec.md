...
- [ ] Over that domain every level is exercised at both outcomes, and at least one of them by a superseded record specifically: the shipped `accounts_for_task` returns false for a record carrying `superseded: true` and true for one carrying no `superseded` member, the shipped `unaccounted_wave_tasks` returns both an empty and a non-empty list, and the shipped repair-round verdict both refuses and passes. A domain that decides one way at any level, or that never carries a superseded record, cannot discharge the criterion above.
...
- [ ] `state.json` is byte-identical before and after a `check --phase wave-reopen` invocation, for every state in that domain that has one. This is the criterion that check discharges; the unit suites' single-invocation byte checks answer to the reopen verb's criteria instead.
...
- [ ] Every name and comment describing that check — its step label in `.github/workflows/build-check.yml` and both `tools/lint-ci-parity.py` entries that pin the label byte for byte — describes what the check actually compares.
