...
- [ ] This spec's oracle transcribes the predicates from this contract's words and imports nothing from the implementation, as the frozen walk does and for the reason that walk states — a notes script under `docs/` does not import `_loop_guards`. The coupling to the shipped code is carried instead by an assertion in `packs/core/tests/skills/work-loop/` that the transcribed superseded rule and `unaccounted_wave_tasks` agree on every state in the oracle's domain.
