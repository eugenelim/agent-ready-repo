...
- [ ] **AC6a — fixture-`$HOME` install/uninstall test (local + per-PR pre-merge).** A test at `packages/agentbundle/tests/integration/test_install_converters_user_scope.py` matching the project's canonical sibling-test idiom (verified: `unittest.TestCase` + `unittest.mock.patch.dict(os.environ, {"HOME": str(tmp_home)})` + **in-process** `install.run(argparse.Namespace(...))` via `from agentbundle.commands import install` — *not* a subprocess shell-out; see `test_install_user_hooks.py` for the reference shape):
  ...
  - Asserts `self.home / ".agent-ready" / "state.toml"` schema-version equals whatever `agentbundle.config.STATE_SCHEMA_VERSION` currently exports — the constant is the contract; a hard-coded literal is a snapshot that rots when a future RFC bumps the schema. The `converters` entry is recorded with `scope = "user"` and `len(state.packs["converters"].files) >= 3` (a floor of one file-tracking entry per shipped skill — the install→state→uninstall data flow runs through this dict; uninstall reads it to know what to remove, and a regression where only one skill's files were recorded would silently orphan the other two on uninstall).
