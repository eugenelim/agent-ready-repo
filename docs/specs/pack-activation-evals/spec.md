...
- [x] The pack-authoring guide (`guides/_shared/how-to/author-a-skill.md`) documents how to **write** activation evals — `evals/eval_queries.json`, the trigger-eval convention (flat `[{query, should_trigger}]`, ~8–10 each way, near-miss negatives), **distinct** from the output-quality `evals/evals.json` — how to declare `[pack.evals].skills`, and how to **run** them locally (`python tools/run-pack-evals.py --pack <name>`); its existing `evals/` description no longer implies `evals.json` is the only accepted layout, and the per-pack reference guides touched by the first cut (`guides/core/`, `guides/converters/`) carry no stale `evals.json`-only layout claim.
