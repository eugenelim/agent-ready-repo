...
- [ ] **Lightweight behavior/output check (`tier: B-lite`).** For each eval in a covered skill's existing `evals/evals.json`, the in-harness driver has the host agent **run the skill** on the eval's `prompt` and grades by (a) **deterministic post-conditions** — the eval's `files`/`expected_output`-described artifacts exist and literal expected/forbidden substrings are present/absent in the produced output (the backbone) — and (b) the host agent's **per-`assertion` pass/fail self-attestation** for the semantic assertions a string check can't cover. No separate judge model, no `benchmark.json` deltas, no with/without-skill baseline. It reuses `evals/evals.json` (no new file), writes pass/fail-per-eval + counts to the eval-workspace, and is labelled `mode: in-harness`, `tier: B-lite` so it is never conflated with the Tier-A activation number or a full Tier-B grade. **Fidelity:** the deterministic post-conditions are **observed** (real run, real artifacts); the semantic-`assertion` verdicts are **self-attested** (lightweight, not an independent judge).
- [ ] **Sandbox containment control (replaces E2's "no execution" for the behavior check).** The behavior check **must** execute the skill body, so E2's read-only/no-execution control is replaced by **isolated-workspace execution**: the skill runs in an **ephemeral temp workspace** seeded only with the eval's `evals/files/` fixtures — **no access to the real repo tree, no secrets / process environment, no network** (network only on an explicit per-eval opt-in for skills that genuinely require it) — and the workspace is **gitignored** and **torn down** after the run. The eval `prompt`/fixtures are author-authored (trusted for a catalogue-internal dev tool); the sandbox confines the *skill body's* tool use. A **`security-reviewer` spec-stage pass signs off this sandbox design before any execution code ships**.
