...
> fail-closed) is unit-tested, and the driver loop is validated live — but against
> a **proxy** skill (a synthetic writer), since no committed `evals/evals.json`
> yet ships an `expect` block. A real end-to-end B-lite run against a covered
> skill (with its Tier-1 deps installed + author opt-in) is the remaining
> exercise (`notes/manual-qa.md` §5).
