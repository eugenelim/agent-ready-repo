...
- [x] **Anti-pattern detection + steering (don't launder misuse into the catalogue).** Assimilation detects known misuse patterns in the ingested primitive and either **reshapes to the repo's right shape or rejects** — naming the anti-pattern and its correction. At minimum it catches:
...
- [x] Given a proposed pack area, it tests additivity + fit against the *local* CHARTER coverage model and the four CHARTER principles; on pass it scaffolds the pack shell and emits an RFC with a per-primitive inventory + verdicts; it can reject the area as non-additive.
...
- [x] **Ingested code is a higher-scrutiny class.** An ingested **hook or script** (executable code, not prose) is flagged distinctly and requires an explicit human confirm before it lands — it runs on the operator's machine on git/session events.
...
| 2026-08-02 | `assimilate-repo` | Re-sync against adopter-repo RFC-0001 (2026-07-22 source). Case 1 — RFC Open: delta (1 changed, 1 new candidate) routed as Amendment in-place on RFC-0001; no new RFC authored. Case 2 — RFC Frozen + typo: query-planner verdict corrected; skill paused for Approver sign-off; Erratum appended; no new RFC. Case 3 — RFC Frozen + new decisions: 2 new candidates + 1 reversed verdict routed to new RFC-0002; skill paused for Approver sign-off; supersession Erratum appended to RFC-0001; skill stated new decisions not appended to Frozen RFC. All three routing paths confirmed correct per `re-sync.md` and RFC-0055 forms. | PASS |
| 2026-08-02 | `assimilate-primitive` | Anti-pattern steering (AC5): ingested `agent-reviews-own-output.md` fixture. AST01–AST10 security review: clean. Anti-pattern detected: self-review (agent marking its own homework). Disposition: reshaped (not rejected) — Step 4 self-check removed; stdlib filtering repositioned as a forward data-operation in the procedure. Shaped target presented; no blocker. | PASS |
| 2026-08-02 | `propose-catalogue-pack` | AC6 — proposal: "a pack for managing Notion workspaces". Domain check: PKM explicitly out of scope in `docs/CHARTER.md`. Principle 3 (habit, not a tool) fails: "managing Notion workspaces" is tool-operation with no SDLC anchor. Rejected with both disqualifiers named. Narrow counter-path offered: notion-brief-intake scoped to a single SDLC-anchored intake skill. No shell scaffolded; no RFC emitted. | PASS |
| 2026-08-02 | `assimilate-primitive` | Hook-confirm (AC7): ingested `sample-hook.py` fixture. Flagged as executable code (higher-scrutiny class); raw body shown verbatim. Confirm prompt issued: "Type yes, land this code to proceed, or no to abort." Operator confirmed. Security review: clean (no injection, no network, no writes). Anti-pattern check: purely deterministic — no skill/agent triggering. Landed at `tools/hooks/pre-commit.py`; docstring reshaped to match directory convention; syntax verified. | PASS |
