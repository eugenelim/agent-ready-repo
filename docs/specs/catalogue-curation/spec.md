...
- [x] Given a local path or URL to a single skill/agent/hook (or a small bundle), the skill fetches it, diagnoses a destination pack + lifecycle from the *local* CHARTER coverage model, and migrates it to `.apm`/pack convention — or rejects with a stated reason.
- [x] It previews the plan (files created/moved) before writing, and prompts `make build-self` after writing.
...
- [x] Given a repo/catalogue path or URL, it inventories candidates and assigns each a verdict (`assimilate` | `reject` | `needs-new-pack`), processing iteratively (one reviewable verdict at a time).
...
- [x] It emits the inventory + verdicts as an RFC (a reviewable proposal), and when any `needs-new-pack` verdict appears it *offers* — never auto-invokes — a hand-off to `propose-catalogue-pack`.
...
- [x] When migration hits a judgment only the operator can make (destination pack, naming, splitting a bundle, an activation collision), the skill **prepares the elicitation context** — what it found, the options, its recommendation — rather than asking a bare question; it guides, it doesn't flood.
- [ ] **Anti-pattern detection + steering (don't launder misuse into the catalogue).** (deferred: catalogue-curation-antipattern-steering) Assimilation detects known misuse patterns in the ingested primitive and either **reshapes to the repo's right shape or rejects** — naming the anti-pattern and its correction. At minimum it catches:
...
- [ ] Given a proposed pack area, it tests additivity + fit against the *local* CHARTER coverage model and the four CHARTER principles; on pass it scaffolds the pack shell and emits an RFC with a per-primitive inventory + verdicts; it can reject the area as non-additive. (deferred: catalogue-curation-propose-pack)
...
- [x] Substitutes the four anchors from their declared sources (URL + slug + owner from `.adapt-discovery.toml`; email from `pack.toml` maintainer + git config); slug and owner are path/glob-scoped, URL and email blanket.
- [x] Persists the fork's chosen defaults into the **target copy only**: `scope.py DEFAULT_ADAPTER`, `self-host.toml` `[recipe.adapters].targets` + `[recipe.packs].include`, and blanks `install-defaults.toml` `source` — a bounded re-home transform on declared anchors, never touching this repo's engine.
...
- [x] **Untrusted-content review before write, then a visible shape transform.** Two sequential steps, not one: **(1) security inspection** — the *raw fetched body* is shown **verbatim** (nothing hidden behind reformatting) for the operator to judge the untrusted content; **(2) craft-shaping** — only after the content is accepted as safe is the target-state reshaped to the repo's skill-authoring craft (see *Target-state craft conformance*), as a **visible transform the operator approves**. The operator sees both what was ingested (raw) and what will ship (shaped). (Supply-chain / LLM-injection: an assimilated skill/agent is instruction prose that projects into the operator's and downstream users' agents — so the raw content is judged before it is trusted, and the shaped result is judged before it lands.)
- [ ] **Ingested code is a higher-scrutiny class.** An ingested **hook or script** (executable code, not prose) is flagged distinctly and requires an explicit human confirm before it lands — it runs on the operator's machine on git/session events. _(Hook-ingest path not yet exercised — `shared/ingest` QA was a skill, not a hook.)_ (deferred: catalogue-curation-hook-confirm)
...
| Date | Skill | Exercise | Outcome |
|---|---|---|---|
| prior session | `assimilate-primitive` | Ingested `shared/ingest` from llm-wiki-kit into `agent-commander` pack; raw body reviewed; shaped to craft (terse description, routing tables, progressive disclosure); landed via `write_jailed`; `build-self` prompted. | PASS |
| 2026-07-22 | `assimilate-repo` | Surveyed llm-wiki-kit (57 candidates) anchored against agent-commander PKM charter; iterative verdicts (53 assimilate / 3 reject / 1 needs-new-pack); RFC-0001 produced in agent-commander; `team-onboarding` `needs-new-pack` offered to `propose-catalogue-pack` — not auto-invoked. | PASS |
| 2026-07-22 | `export-catalogue` | White-label export to `/tmp/catalogue-fork-qa`; seeds planted (core + governance-extras); mission captured (PKM); guides staged + de-identified; four-anchor verify passed (zero upstream hits); CLAUDE.md symlink resolved; fork defaults persisted. | PASS |
| 2026-08-03 | `assimilate-primitive` (AC5 — antipattern-steering) | Agent-run (source: skilldrop, PR #849). Detection against all 3 fixtures — `script-triggers-skill.sh`, `agent-reviews-own-output.md`, `flooding-prompt.md` — matched oracle exactly. False-positive check on real skilldrop primitives (terse skill + deterministic script): clean. | PASS |
| 2026-08-03 | `assimilate-primitive` (AC7 — hook-confirm) | Agent-run. Detection + confirm gate correct: shebang-triggered higher-scrutiny class, raw-body shown before prompt, exact phrase `yes, land this code` required, AST01–AST10 correctly scoped to SKILL.md not raw scripts. Landing mechanics (SAST tool list, git-hook vs agent-hook distinction, inventory bumps, lint→build-self→verify order) were oracle-only — fixed in 0.2.2. | PARTIAL → fixed |
| 2026-08-03 | `assimilate-repo` (AC4 — resync-rfc-routing) | Agent-run. Routing logic correct: Amendment/Erratum/new-RFC forms matched RFC-0055. Runnability partial: no self-contained fixture (AC4 depends on external prior-session RFC-0001); Approver-sign-off gate was oracle-only, operator-flagged nature of corrections was ambiguous — both fixed in 0.2.2 `re-sync.md`. | PARTIAL → fixed |
| 2026-08-03 | `propose-catalogue-pack` (AC6) | Agent-run. Reject-on-duplication correct (overlapping packs named). Accelerator-pack routing entirely absent — a tech-stack proposal would wrongly fail Principle 1 and never collect the three extra gates — fixed in 0.2.2 SKILL.md. `pack-shell.md` falsely claimed an empty `.apm/` won't validate — corrected. | PARTIAL → fixed |

**Open paths (human-gated):** AC4–AC7 agent-run results are above; live human-operator sign-off sessions for each are still pending (AC4–AC7 per `catalogue-curation-qa-coverage` spec). AC5 logic is PASS from agent run; AC4/AC6/AC7 partial findings are fixed in 0.2.2 and need re-verification in live sessions.
...
- Technical: the repo's Python SAST toolchain is **bandit** (`bandit.yaml`) and **semgrep** (`tools/semgrep/`, `SEMGREP_EXCLUDE` in `Makefile`); CodeQL (`.github/workflows/codeql.yml`) and Snyk (`.snyk`) run on CI / PRs. The prior probe finding ("no bandit config present") was incorrect — corrected by agent QA run 2026-08-03. Assimilation invokes these gates on ingested content and adds no scanner dependency.
