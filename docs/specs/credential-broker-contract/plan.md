...
**Approach:**
- Edit `packages/agentbundle/agentbundle/_data/adapter.toml` and `contracts/adapter.toml`: bump `[contract] version` from `"0.6"` to `"0.7"`; extend header comment in both files to name RFC-0013 alongside existing RFC pointers.
...
- `guides/credential-brokers/how-to/add-a-credentialed-skill.md` "pick a broker" section replaces the old "pick a primitive class" section (verifies AC43).
...
- Rewrite the relevant section of `guides/credential-brokers/how-to/add-a-credentialed-skill.md`.
