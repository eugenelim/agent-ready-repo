...
> **Scope: one PR.** The validate.py swallow, the install-time event-level enumeration, the formatter extension, the regression tests, and the migration-guide update all land in a single PR. Splitting risks the validate exits 0 but install still refuses (or vice versa) — opposite signals reaching adopters from the same condition.
...
- **Touch `architect`'s or `credential-brokers`'s pack version.** The shipped migration guide (`docs/guides/how-to/v07-to-v08-pack-upgrade.md:45-49`) explicitly documents both as intentional defers from PR #156. This spec doesn't re-litigate that call.
...
| Migration guide update reads cleanly end-to-end | **Manual QA** — read the new section in `docs/guides/how-to/v07-to-v08-pack-upgrade.md` against AC11. | Adopter-facing prose; correctness is judgment-based. |
...
- **AC6c.** When a hook-wiring TOML fails to parse: validate-time refuses with exit 1 per AC4 (correctness rail); install-time emits the per-file drop entry with reason `hook-wiring TOML failed to parse`. The asymmetry is **intentional and documented** — validate is the development-time gate where parse failures are loud bugs; install is the adopter-time degradation rail where the parse failure becomes one more reason a file won't project. A migration-guide bullet (AC11) names the asymmetry so adopters running install without validate don't silently miss a broken pack file.
...
- **AC11.** A new section in `docs/guides/how-to/v07-to-v08-pack-upgrade.md` covers the per-file drop semantics — when it fires, what adopters see, the warning wording. Cross-references the parent spec (`dropped-primitives-coverage`). The section is named with adopter-facing language ("Per-file hook-wiring drops on incompatible adapters" or similar; final wording is the implementer's call).
