...
- `grep -rn -E "does not declare 'attach-to-agent'|not in adapter.*agent-event-vocabulary" tools/ .github/ guides/ docs/architecture/ docs/product/` returns empty (AC14). The whitelisted-directories command sidesteps the spec/RFC self-match documented in spec § Boundaries.
