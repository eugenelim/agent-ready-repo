...
| `test_local_ci_shared_test_deduplication.py::test_core_pytest_semantic_node_contracts_are_exact` | `python3 -m pytest tools/test_local_ci_shared_test_deduplication.py -q` → 1 failed, 25 passed; `assert 85 == 78` on `packs/core/tests/skills/work-loop/test_lint_spec_status.py` | `owned-elsewhere`. Reproduces identically in a clean `origin/main` worktree, so it is not caused here. `main` commits `d5192da9a` and `487b298c4` added tests to that file without re-pinning the guard's node count and digest. Not re-pinned here: the guard's own comment requires a rebase delta to be dispositioned rather than resolved by taking either side, and that disposition belongs to whoever owns the core change. | Claude, this session |
| `make build-check` → `CAT-V-015 self-host projection is out of date` in CI | `gh run view --job 99693050549 --log-failed` → `.claude-plugin/marketplace.json` content differs | `caused-here`, **fixed**. The pack version bump moves the aggregated marketplace manifest, which is regenerated by `catalogue self-host --write` and never hand-edited. The regenerated diff is one line, `0.3.0` to `0.4.0`. Local gates missed it because `make build-check` was not in the local set. | Claude, this session |
