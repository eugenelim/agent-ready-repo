...
- [ ] AC0 **loop-engine events.jsonl append.** `loop-engine.py` `cmd_transition` appends one JSON line per FSM transition to `.loop-run/events.jsonl` (repo-root-relative, gitignored, ephemeral). Each line contains: `{"seq": <int>, "run_id": "<uuid>", "spec": "<spec-dir>", "from": "<state>", "event": "<event>", "to": "<state>", "at": "<iso8601>"}` — matching the canonical schema in design.md:317 and the notification-contract table at design.md:342. `cmd_init` initializes `.loop-run/` (creates dir; creates `events.jsonl` as an empty file — no header line; every line in the file is a schema-conforming event). `cmd_reset` also removes `.loop-run/` (in addition to removing `engine-state.json`). If `.loop-run/` is absent from `.gitignore`, `cmd_init` appends it. The append follows the outbox protocol: write pending line to `.loop-run/events.pending`, write `engine-state.json` atomically (rename), append pending line to `events.jsonl`, delete pending. **Graceful degradation (universal rule):** all events.jsonl and events.pending I/O (create dir, create file, write, append, delete) is wrapped in a try/except; any I/O failure is caught and emits a logged warning; the FSM state write (`engine-state.json` atomic rename) proceeds uninterrupted regardless. events.jsonl I/O errors never abort a transition. This rule applies to `cmd_init`, `cmd_transition`, and `cmd_reset`.
...
- [ ] AC5 **Event bridge — inode/truncation reset.** When `.loop-run/events.jsonl` is deleted and recreated (via `loop-engine reset`, which now removes `.loop-run/` per AC0), workspace-mcp detects the inode change or size-less-than-offset condition, resets offset, buffer, run_id, `current_state`, `gate_pending`, `gate`, `gate_question`, and `review_findings` (all FSM/gate fields cleared to null/false), and reattaches correctly without missing events from the new file.
...
- [ ] AC10 **Slug safety guard — regex and containment.** Before formatting an `ini_slug`, `type`, or `slug` value into an output_pattern glob, workspace-mcp validates each segment against `^[a-zA-Z0-9._-]+$` AND explicitly rejects any segment that is `.`, `..`, or starts with `-`. After formatting, the resolved directory is verified to remain under the type's static output base via `safety.assert_under(static_output_base, resolved_dir)` — not a bespoke `startswith` comparison. An item failing either check is excluded from the result (logged as a warning).
