...
| `adapter.toml` ↔ `contracts/adapter.toml` byte-identical | **Goal-based check** — `test_contract_files_byte_identical` (existing). | Dual-copy gate. |
...
- [x] **AC1a.** `adapter.toml` is **dual-copy**: the edits land byte-identically in
  `packages/agentbundle/agentbundle/_data/adapter.toml` **and** `contracts/adapter.toml`.
...
- Technical: `adapter.toml` is dual-copy (`_data/` + `contracts/`);
