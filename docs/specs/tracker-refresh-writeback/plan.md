...
Implement one shared authority coordinator behind Group 4's `work-intake`
refresh interface, then compose it with the four Group 5 tracker profiles.
...
- `work-intake-surface` owns the public router and configured
  refresh-processor interface.
...
workspace mutation to Group 3 and never edits TOML independently.
...
Each tracker processor implements the Group 4 configured-processor interface:
...
Traces to: AC1-AC13, AC17-AC18, AC23 · implements
`contracts/jsonschema/normalized-intake.schema.json` and
...
guarded reconciliation/mutation and repair readiness → Group 4 public refresh
seam → Group 5 profile registry/acquisition → this coordinator and processors.
...
**Touches:** packs/core/.apm/skills/work-intake/SKILL.md, packs/core/.apm/skills/work-intake/scripts/*.py, packs/core/tests/skills/work-intake/**

**Verification mode:** TDD

**Tests:**

**Stub:** draft (uncompiled) — the Group 2 schema bindings, Group 4 processor
interface, Group 5 profile registry, and shared refresh module are created by
upstream or this RFC-0083 work and are unavailable at PLAN. First EXECUTE
materializes the listed pytest cases as compilable failing tests before any
production edit.
...
- Add comparison-result tests proving completed comparison advances the
  compared revision while acquisition/comparison failure does not. Verifies
  AC6-AC7.
...
- Add authorized, missing, ambiguous, stale, and unauthorized approver cases;
  ...
  Add a distinct confirmation check for each queued remote mutation. Verifies
  AC5, AC12.
...
- Reuse Group 3's guarded mutation operation for the artifact/workspace pair.
...
**Touches:** packs/core/.apm/skills/workspace-status/SKILL.md, packs/core/.apm/skills/workspace-status/scripts/workspace_status_engine.py, packs/core/.apm/skills/workspace-status/scripts/workspace_status.py, packs/core/.apm/skills/workspace-status/evals/**, packs/core/tests/skills/workspace-status/**, tools/test_workspace_status.py, tools/test_workspace_status_cli.py

**Verification mode:** TDD

**Tests:**

**Stub:** draft (uncompiled) — the Group 3 authority result fields and T1
refresh coordinator module are created by upstream or this RFC-0083 work and
are unavailable at PLAN. First EXECUTE materializes the listed pytest cases as
...
**Stub:** draft (uncompiled) — the T1 coordinator and Group 5 Linear profile
module are created by this RFC-0083 work and are unavailable at PLAN.
...
- Assert the Linear destination passes scheme/host/address/redirect/DNS
  confinement before the fake GraphQL transport records a request. Verifies
  AC21-AC22.
...
**Stub:** draft (uncompiled) — the T1 coordinator and Group 5 Jira/Jira Align
profile modules are created by this RFC-0083 work and are unavailable at PLAN.
First EXECUTE materializes the listed pytest cases as compilable failing tests
before any production edit.
...
**Stub:** draft (uncompiled) — the T1 coordinator and Group 5 GitHub profile
module are created by this RFC-0083 work and are unavailable at PLAN.
First EXECUTE materializes the listed pytest cases as compilable failing tests
before any production edit.
...
- Assert the GitHub refresh action declares only the `gh`/filesystem tools and
  boundaries it uses and carries accurate approved-CLI/auth metadata. Verifies
  AC23.
