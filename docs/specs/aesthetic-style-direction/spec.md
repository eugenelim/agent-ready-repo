...
- [x] `creative-direction/assets/creative-direction-template.md` carries a
...
- [x] Each axis row states a token vocabulary, and every vocabulary includes
      `[platform-default]`. *(goal-based)*
- [x] Each axis cell opens with one or more tokens in square brackets drawn
      from that row's vocabulary, followed by prose. An axis carrying two
      independent parameters carries two tokens. *(goal-based)*
- [x] Every axis cell ships filled with its `[platform-default]` token rather
      than a placeholder, so an unedited copy is syntactically valid and
      visibly undecided. *(goal-based)*
- [x] No axis cell matches any pattern in `tools/lint-experience-agnostic.py`.
      This is the literal-and-stack floor only. *(goal-based)*
- [x] Each axis cell names a role, classification, or relationship rather than
      a value. *(manual QA)*

### The counterfactual gate

- [x] `creative-direction/SKILL.md` carries a numbered procedure step directing
      the author to fill the direction sheet before capturing the doc.
      *(goal-based)*
- [x] `creative-direction/SKILL.md` carries a numbered procedure step requiring
      ...
      why. *(goal-based)*
- [x] That step names the revision record as a required field of the direction
      doc, and the template carries a section that holds it. *(goal-based)*

### The presets

- [x] `creative-direction/assets/presets/` contains exactly three files, for
      Swiss / International Typographic, editorial broadsheet, and Bauhaus.
      *(goal-based)*
- [x] Each preset carries a formal-commitments section, a line naming what a
      direction borrowing it leaves behind, a `Best used for:` line, and all
      fifteen axis rows filled. *(goal-based)*
- [x] Every bracketed token used in a preset's axis row belongs to that row's
      vocabulary in the template, verified by extracting and comparing the two
      token sets per row. *(goal-based)*
- [x] Each preset's formal commitments are those its style carries in
      `docs/product/research/aesthetic-style-blueprint.md` §F2.2, which is the
      closed source. *(manual QA)*
- [x] `creative-direction/SKILL.md` names the preset set and states that a
      preset supplies precedent only and is never a finished direction.
      *(goal-based)*

### The divergence audit

- [x] `creative-direction/references/divergence-audit.md` exists and specifies
      comparing N candidate directions pairwise across the fifteen axes.
      *(goal-based)*
- [x] It reports the minimum pairwise distance across the candidate set and
      states that the mean is not the reported figure. *(goal-based)*
- [x] It treats a pair as distinct when at least **six of the fifteen** axes
      ...
      *(goal-based)*
- [x] Its comparison rules are stated and unambiguous for the cases that arise:
...
- [x] `design-review/references/taste-critique.md` carries the four VisAWI-S
      items, each labelled with its facet. *(goal-based)*
- [x] It carries the four UEQ Novelty word pairs. *(goal-based)*
- [x] It states the response format for each instrument, attributes each to its
...
- [x] `packs/experience-design/pack.toml` and
...
- [x] `.claude-plugin/marketplace.json` reads `2.0.6` for `experience-design`,
...
- [x] That entry carries one `Highlights` subsection one heading level below
      it, because the release changes what a consumer can do. *(goal-based)*
- [x] Every heading added to `docs/product/changelog.md` has exactly one blank
      line above and below it. *(goal-based)*
- [x] `creative-direction/evals/evals.json` carries an assertion naming the
...
- [x] `design-review/evals/evals.json` carries an assertion naming the two
...
- [x] `tools/lint-experience-agnostic.py` exits 0 over `packs/experience-design/`.
      *(goal-based)*
- [x] The three `tests/roster/test_experience_design_*.py` suites pass.
      *(goal-based)*
- [x] The five commands in `guides/AGENTS.md` § Essential commands each exit 0:
      ...
      *(goal-based)*
- [x] `catalogue lint --root . --deep` and `catalogue verify --root .` each exit
      0, run after the projection is regenerated. *(goal-based)*
- [x] `docs/specs/aesthetic-style-direction/notes/verification-ledger.md`
      records all four manual-QA verdicts with the reviewer's name and date,
      and is committed in the same change. *(goal-based)*

### Documentation

- [x] A guide under `guides/experience-design/` covers the direction sheet, the
