...
**Tests:** goal-based check, run **differentially** per arm (AC-0016). For each arm
named in AC-0001 through AC-0010: remove that arm from `tools/lint-ci-parity.py`,
...
**Done when:** AC-0016 holds — every arm's removal produced a non-zero exit — and
...
- the entry whose `path` is `tools/repo/build_gate_chain.py` and whose `kind` is
  `defect` appears once, under `[backlog].closed` (AC-0017)
...
- 2026-09-16: Round 2 reviewed the repairs and found five of them had left drift.
  ...
  stated once. AC-0011 and AC-0012 could both pass with the whole change absent, so
  they now read off the roster and lean on AC-0006 for the step. AC-0014 was
...
- 2026-09-16: Contract amendment, owner-authorised. T1's probe found AC-0006
  ...
  approved rejected five correct `PR_GATED` entries and made AC-0015
