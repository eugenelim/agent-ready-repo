...
| Maintainer-facing residual | Applicable: the first draft made the docstring wording and the `tools/AGENTS.md` wording acceptance criteria, but an obligation whose only check is that a sentence exists is not a criterion, so both are owned here instead. Named rather than numbered on purpose: a criterion identifier in the body reads as a live local reference, so a historical number would either dangle or silently re-point after a renumber | `tools/lint-ci-parity.py` docstring; [`tools/AGENTS.md`](../../../tools/AGENTS.md) | Repository maintainer | Docstring names the `PR_GATED_IF` condition, the reason-validity human control, and the spelling-mismatch residual; `tools/AGENTS.md` names `SUITE_DISPOSITION` | Both surfaces name the real symbol and it resolves |
