...
`sha256("<file>|<line>|<title>")` per finding, and notes that the comparison
reads false through most real recurrence because the preimage carries the
finding's position. Three parts of that preimage move while the finding does not:
