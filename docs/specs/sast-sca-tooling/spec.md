...
- **Status:** Shipped (superseded in part by [ADR-0085](../../adr/0085-split-the-sast-gate-into-its-own-ci-job.md) — this spec **teaches** that SAST is chained into `make build-check` in CI; the leg is now its own `gate-sast` job. The Makefile chain it dogfoods is deliberately intact, so its Process point holds locally; everything else stands) (superseded in part by [ADR-0084](../../adr/0084-nosec-reason-delimiter-and-stderr-as-a-gate.md) — the Bandit suppression-comment form this spec teaches was reversed; everything else stands) <!-- Draft | Approved | Implementing | Shipped | Archived -->
- **Owner:** eugenelim
