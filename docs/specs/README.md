...
| [`apm-install-route-parity/`](apm-install-route-parity/spec.md) | Draft | RFC-0010 | F-apm-derivation + F-apm-install-marker: per-pack `SessionStart` writer drops `.adapt-install-marker.toml` on first install via APM's HookIntegrator, projected through `dist/apm/<pack>/.apm/hooks/install-marker.{json,py}`. Reuses the canonical writer template from `claude-plugins-install-route` with two additive changes: a `--install-route {cli,claude-plugins,apm}` argparse flag (default `claude-plugins` for back-compat) and a data-directory portability shim (`${CLAUDE_PLUGIN_DATA}` → `${PLUGIN_ROOT}/.data` → `${CURSOR_PLUGIN_ROOT}/.data` → exit 0). Scope detection under APM is by writer's own projected path (cwd → repo; `$HOME` → user). Adapter contract v0.4 → v0.5 (`"apm"` appended to `install-routes`); marker schema gains `"apm"` as a permitted `install-route` value. Coverage is four-of-seven APM targets (Claude Code, Copilot, Cursor, Gemini); three uncovered targets (Codex, OpenCode, Windsurf) use the documented `agentbundle adapt --scope <project\|user>` manual fallback. Amends `claude-plugins-install-route/spec.md` (AC1 + AC9 in-PR edits), `adapt-to-project/spec.md` (1 new AC + proactive cache-scan branch extends to `apm_modules/`), and `distribution-adapters/spec.md` (APM-route conformance cases + v0.4→v0.5 Changelog). Depends on `claude-plugins-install-route`, `distribution-adapters`, `adapt-to-project`. |
