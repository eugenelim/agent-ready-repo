...
| [`agentbundle-wheel-release/`](agentbundle-wheel-release/spec.md) | Draft | RFC-0003 | Publishes the `agentbundle` Python wheel to a registry so adopters can `pip install agentbundle` without cloning. Closes RFC-0003 §F-cli-dist path #2 in two variants — PyPI (Trusted Publisher OIDC, secrets-free) and corp Artifactory (auto-skipped when secret absent; ships dormant). One workflow file, three jobs (build-and-smoke, publish-pypi, publish-artifactory). 14 ACs, 7 tasks across PR-A (T1–T6, infrastructure) and PR-B (T7, README route-3 update sequenced after first publish). Out-of-band: PyPI account + Pending Publisher (~10 min) before first tag. Amends RFC-0003 §F-cli-dist (path #2 Shipped; paths #1 zipapp + #3 Homebrew Deferred with one-line reasons). Depends on `agent-spec-cli`. |
