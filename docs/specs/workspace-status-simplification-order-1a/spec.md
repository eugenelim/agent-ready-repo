...
- [x] AC5. The CLI emits deterministic versioned JSON with `"schema_version": 1`. The JSON provides ready, blocked, shaping, brief, and reconciliation data (including `type2_cleanup_ops` per Type 2 finding) so the skill does not independently recompute the DAG or reconciliation. `[backlog].open` typed entries are surfaced under `shaping.*` in the JSON (the engine's `extract_top_level_backlog` feeds the shaping pipeline); the JSON intentionally has no separate top-level `backlog` key. The skill may still read external files the engine does not own: `docs/product/findings/*.md` (findings-register tables) and per-backlog-entry `# ` comment lines from workspace.toml text for the backlog section summary.
...
- [x] AC7. Type 2 confirmation and cleanup behavior is unchanged in its interactive flow; the exact cleanup operation metadata (`type2_cleanup_ops`) comes from backend output rather than a second reconciliation. One deliberate correctness refinement: when the user confirms a path is still active, both the active-list op and the queue-list op for that `(ini_slug, path)` pair are excluded from the confirmed set — not just the active-list op — to prevent partial cleanup that would leave the path simultaneously in `active` and `shipped`.
