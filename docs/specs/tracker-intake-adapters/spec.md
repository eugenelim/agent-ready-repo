...
- [x] **AC1.** Jira, Jira Align, Linear, and GitHub each emit valid normalized intake and delegate to `work-intake`.
- [x] **AC2.** Every record declares profile ID/version, durable locator, comparable revision/fingerprint, object hint, action, required content, constraints, and proposed authority. Locator and revision/fingerprint come from trusted acquisition metadata cross-checked against the requested object, never tracker-authored text.
- [x] **AC3.** Equivalent fixtures produce identical artifact, membership, processor, and authority across profiles.
- [x] **AC4.** One shippable/verifiable issue routes to a spec and `new-spec`, regardless of tracker type.
- [x] **AC5.** One coherent multi-spec outcome routes to a Draft brief and shared brief processors.
- [x] **AC6.** One cross-repository outcome routes to linked repo briefs with required parent and coordination provenance.
- [x] **AC7.** An incoherent board, sprint, cycle, milestone, project, view, or query produces separate units or view-only output, never a brief.
- [x] **AC8.** A regression with durable expected-behavior evidence routes to defect context and `bug-fix`, regardless of tracker label.
- [x] **AC9.** A claimed defect without durable evidence remains unresolved or becomes a Draft spec; the adapter does not infer a contract.
- [x] **AC10.** Profile hints cannot override content, altitude, coherence, shippability, verifiability, defect evidence, or cross-repo rules.
- [x] **AC11.** Jira/Jira Align/Linear use only sibling acquisition skills; GitHub uses only approved `gh` reads.
- [x] **AC12.** No intake path invokes tracker writes, refresh conflict resolution, or post-execution write-back.
- [x] **AC13.** Embedded instructions or shell metacharacters in tracker-authored fields cannot change routing, tools, scope, destination, or command execution. Every tracker-derived CLI value is passed as a discrete argument or schema-validated data-file value with shell execution disabled.
- [x] **AC14.** Secrets, credentials, unnecessary sensitive data, and full payloads are absent from normalized records, skill output, stdout, stderr, logs, artifacts, and entries.
- [x] **AC15.** Confidentiality mismatch, unsafe redaction, non-RFC-8259 JSON (`NaN`, `Infinity`, or malformed encoding), malformed payload, absent profile version, missing locator, spoofed provenance, or non-comparable revision fails before repository writes.
- [x] **AC16.** Missing `work-intake` produces a named dependency diagnostic and no local fallback write.
- [x] **AC17.** Tracker guides, shared vocabulary, selection guidance, and journeys use content-based classification and match common fixtures.
- [x] **AC18.** Each pack has Tier-A coverage and `[pack.evals]` declarations for user-triggered intake skills.
- [x] **AC19.** Pack tests, routing evals, catalogue lint/verify, self-host projection, guide validation, site build, and links pass.
- [x] **AC20.** Every credential-bearing adapter-controlled HTTP destination accepts only `https` and hosts from a profile-scoped allowlist; non-HTTPS input fails before credentials attach. GitHub instead uses a documented fixed-host approved-`gh` boundary: host selection comes only from trusted repository or administrator configuration, never tracker payload or source locator; credentials remain bound to that configured host; and a mismatch or untrusted `--hostname`/URL is rejected before `gh` is invoked.
- [x] **AC21.** For adapter-controlled HTTP destinations, validation blocks loopback, private, link-local, and cloud-metadata addresses; redirects are disabled or every hop is fully revalidated; and DNS resolution is pinned or rechecked at connect time so rebinding and DNS time-of-check/time-of-use changes fail before a credential-bearing request. For GitHub, redirect and DNS guarantees belong to the approved `gh` transport boundary and are not claimed as locally enforced; fake-argv tests prove the adapter supplies only the trusted configured host and rejects payload-derived or mismatched host/URL arguments before invocation.
- [x] **AC22.** Each tracker profile declares maximum pages, items, and response bytes plus timeout, retry, and bounded-backoff policy; exceeding a limit produces deterministic truncation explicitly marked incomplete or a view-only refusal, never an unbounded or silently partial intake.
- [x] **AC23.** Every changed tracker skill action declares minimal `metadata.boundaries` and allowed tools for `network_fetch` and `filesystem_read_untrusted`, plus `filesystem_write` only when used; projection tests prove the declarations survive every supported adapter without broadening.
...
- Process: The four adapter tasks are dependency-independent after shared Group 2/4 prerequisites; the installed Phase-1 supervisor executes their wave sequentially in task-ID order because parallel write fan-out is disabled. (source: `docs/rfc/0083-work-intake-and-artifact-routing.md` § Group 5; execution constraint confirmed by the installed `work-loop`)
