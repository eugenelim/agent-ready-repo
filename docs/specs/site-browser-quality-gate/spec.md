...
- **Status:** Shipped (§ Acceptance Criteria's former register anchors `print-audit-page-break-quality` and `print-chrome-paint-inventory` were both closed on 2026-08-25; their dispositions are recorded in [`notes/print-audit.md`](notes/print-audit.md) and the tech-site completion brief. The first closed on owner review of regenerated print evidence; the second was retired won't-do. AC12's `(deferred:)` marker was removed under an explicit owner ruling — the only body edit made, licensed because `docs/CONVENTIONS.md:435` and `lint-spec-status.py` invariant (iv) admit no path between them. AC12 is closed, not delivered: the navigation-visibility axis was never measured. Not a supersession — every decision here stands)
- **Owner:** eugenelim
...
- [x] The accepted tap-target audit classifies every candidate as conforming,
  ...
  this criterion's original enumeration omitted Spacing. The candidates in the
  tap-target audit's two Spacing groups conform through it and had nowhere valid
  to be recorded. The omission was in the spec, not in the measurement. The count
  is deliberately not repeated here — it lives in that audit, where a test keeps
  its groups, cells and total in agreement.
