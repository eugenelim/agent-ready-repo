...
- [x] A release workflow builds + validates the `credbroker` wheel/sdist and is wired for OIDC Trusted Publishing to PyPI, **gated** so the first real publish + name claim is a manual maintainer action (not automatic on merge). *(Layer 3 / C — code wired this spec; the publish itself is the maintainer action)*
