...
- **The press ground on an unpadded inline link reads as a text highlight, not a control press.** Sustained as a taste finding and left for the owner. The idiom is right — press as a further step on the axis hover already moved is what Adobe Spectrum, Microsoft Fluent 2, IBM Carbon and Google Material 3 all do, and Spectrum states it as a rule. But none of those systems puts a ground behind an unpadded inline link; their press grounds sit on padded controls. On this surface `.nav__link` and `.now-index__link` carry `padding-block` only, `.footer__list a` carries `padding: 0`, and the catalogue record names are bare spans, so the fill paints a rectangle tight to the type — which on a paper-and-serif register is the signature of a selection or a `<mark>`. The two remedies are inline padding, which is a layout change, and a second press idiom for text carriers, which the owner decision of 2026-09-23 explicitly refused. Neither is this change's to take.
- **The press ground animates at three different speeds.** Sustained on the facts, refuted as this change's defect. Measured: the ground transitions over 200ms on `.btn-primary`, `.btn-ghost` and the `PageHero` actions, 120ms on `.decision-band__action--primary`, and snaps on the remaining controls. Those durations are declared on the base rules and govern hover identically; this change added no transition and retuning them is the hover work the spec scoped out.
