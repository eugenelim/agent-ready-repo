...
| Do my own new assertions actually discriminate? | Checked by mutation, and **four** did not, across two rounds. Round 2: a summary assertion was `all()` over an empty sequence, and a Windows-path assertion could not tell `str` from `as_posix` on POSIX. Round 3 found two more: a "trusted runner path" check compared a hand-copied literal against the constant it was copied from, and erasing the failure tally discarded a signal that was exactly recomputable. All four replaced with assertions proven to fail under mutation. The lesson recorded here is that writing an assertion is not evidence it can fail. |
