...
- `test_contract_v08.py::test_schema_loads_v08_contract` — load `adapter.schema.json` via `jsonschema` and validate the v0.8 `adapter.toml` against it; assert no validation errors. This is the load-bearing schema-integration test.
...
- Create `packages/agentbundle/agentbundle/build/projections/__init__.py` (empty; namespace anchor) if no `build/projections/` subdir exists, else add the helper alongside the existing `direct_directory.py`.
...
- Hook the warning into the install handler at the **pre-write barrier point**: after Step 5's `scopes_to_install` is finalised AND each scope's target adapter is resolved (`repo_target_adapter` and/or `user_target_adapter` depending on which scopes are in the plan), AND after the RFC-0005 user-scope hook-wiring rail. Before Step 6's plan loop builds writes. (Name the symbolic Step boundary in code comments, not line numbers — line numbers drift with neighbour edits.)
