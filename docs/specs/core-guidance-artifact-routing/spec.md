...
- [x] The RFC-0099-authorized charter clause, two ADRs, frozen-document
...
- [x] `packs/core/seeds/AGENTS.md` and the curated root `AGENTS.md` state the
  ...
  set.
- [x] A construction test fails on reordering, omission, a dynamic rule-loader
  dependency, or a second full-ladder copy in a changed primitive.
- [x] Core guidance leads with outcomes, omits routine tool narration, and ends
  receipts with changed state, verification, and remaining work without
  constraining required interactive host updates.
- [x] Core guidance deletes claims unnecessary to the accepted outcome and
...
- [x] Chat-only or personal/vault input requires a human-confirmed repository
  ...
  dispatchable work.
- [x] Intent rendering moves to the intent owner without inventing a shared
  helper unless a second runtime caller demonstrably needs one.

### AC4 — One delivery-brief owner with two modes

- [x] `author-delivery-brief create` authors a Draft from a direct request or
...
- [x] `author-delivery-brief continue` alone reviews an existing repository
  brief for Ready and may change status only after human confirmation.
- [x] Ready permits zero specs; selecting the minimum delivery slice is a
  separate confirmation before `new-spec` invocation.
- [x] The brief template and coverage validator separate governance references
  from delivery slices; only specs affect execution and closure rollups.
- [x] Project-knowledge receipts, when their existing semantic gate fires, name
  `author-delivery-brief` as producer rather than either alias.

### AC5 — Bounded aliases and write-new migration

- [x] `author-brief` delegates only to `author-delivery-brief create`, and
...
- [x] Each alias emits one concise deprecation notice, cannot broaden the
  target's tools or boundaries, and records itself only as `invoked_alias` on a
  canonical new-name receipt.
- [x] Alias activation and behavior fixtures cover the compatibility window,
...
- [x] Workspace MCP lifecycle metadata, `workspace-status`, Core consumers,
  ...
  through aliases.
- [x] `kind = "intent"`, `kind = "brief"`, `shaping_queue`, `brief_queue`,
...
- [x] Any AgentBundle package change carries its matching package version and
  package tests rather than leaving MCP metadata pointed at the alias.

### AC7 — Trust and confinement stay fail-closed

- [x] Every new or changed skill and alias in this slice declares its exact
...
- [x] Repository access uses the blessed confined file-safety helpers or a
  tested equivalent and rejects every unsafe path/identity shape named by
  RFC-0099.
- [x] A raw external body never enters a committed artifact or workspace
  metadata.
- [x] Only selected, minimized requirement fields may cross into those
  surfaces after structural neutralization and redaction; uncertainty refuses.
- [x] An external locator remains opaque provenance: recording it performs no
  filesystem stat, resolution, listing, read, or write; only a human-confirmed
  repository destination may enter the confined file-access path.
- [x] Persisted locators strip all query and fragment data, credentials, tokens,
  personal absolute-home paths, private paths, and personal data; admission
  refuses when minimization would destroy source identity.
- [x] Prompt-like source text is delimited as data and cannot change identity,
  scope, tools, permissions, lifecycle status, reviewer routing or verdict,
  write targets, or normative ownership.
- [x] Hostile brief, tracker, and personal/vault intent fixtures prove passive
...
- [x] The existing Core system explanation names the Razor product principle,
...
- [x] Existing guide URLs explain neutral intake, direct-owner precedence,
...
- [x] Journey, how-to, explanation, reference, pack README/DESIGN/JOURNEY,
