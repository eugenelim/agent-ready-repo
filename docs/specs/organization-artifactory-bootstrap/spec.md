...
- [ ] AC17: A valid `enabled = true` config produces a `catalogue+https://` URL that `_is_valid_source` accepts (call `_is_valid_source` on the constructed URL and assert `True`). **Deferred until `spec/https-catalogue-channels` lands** `(deferred: https-catalogue-channels-ac17-gate)` — `_is_valid_source` currently rejects `catalogue+https://` URLs (the scheme gate at `source_defaults.py:96` returns `False` for any non-`git+https://` URL scheme). Until that spec ships, the AC17 test is written but marked `pytest.mark.xfail(reason="backlog:https-catalogue-channels-ac17-gate — spec/https-catalogue-channels AC1 not yet implemented", strict=True)` so it becomes a hard blocker once the dependency lands. The `[backlog].open` slug `https-catalogue-channels-ac17-gate` is registered in `workspace.toml`.
