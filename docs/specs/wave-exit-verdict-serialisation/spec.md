...
- A cohort-lock reclaim mid-hold is reported, and its meaning depends on which exit the body took. If the engine-state write had landed the transition is durable behind a non-zero exit, and the skipped outbox finalisation leaves an `events.pending` that `_recover_pending` completes on the next run; if the body had already refused before writing, nothing was committed and the transition should be re-run. The engine distinguishes the two. Bounding either absolutely needs a two-phase commit, which the registered entry forbids, so AC13 pins the reporting obligation on both paths.
