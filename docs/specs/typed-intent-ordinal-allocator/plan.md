...
**Manual verification — the canonical session inventory.** Seven sessions. Every other mention of manual QA in this plan refers to this table rather than repeating it, because a count restated in three places is a count that disagrees with itself.
...
| 5 | direct `intake-intent` | mapped altitude, request carries `<TYPE>-9999-` | written at `<slug>.md`, supplied value reported back not used | the artifact write | AC-0009 |
...
- `intake-intent`: two prose edits and no more. Procedure step 3 gains the never-block clause — it cannot allocate, its `## Boundaries` refuses a shell and this slice does not grant one, so on the direct path it writes the unprefixed intent and records that no ordinal was allocated, neither refusing nor using a prefix supplied with the request. And the update-in-place paragraph at `SKILL.md:61-67` states both destinations, because it currently cites an unconditional `docs/product/intents/<slug>.md` default while telling an agent not to rename to match it — true of an update, stale about creation. Its identity rule itself is untouched: nothing here renames.
