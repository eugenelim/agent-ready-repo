...
**Manual verification — the canonical session inventory.** Six sessions. Every other mention of manual QA in this plan refers to this table rather than repeating it, because a count restated in three places is a count that disagrees with itself.
...
| 6 | `work-intake` | an allocator stub returning `../escape` | § 6 refuses to compose a destination from it | the refusal, before any write | AC-0016 |
...
**Touches:** tests/roster/test_typed_ordinal_collision_equivalence.py, packs/core/.apm/skills/work-intake/SKILL.md, packs/core/.apm/skills/intake-intent/SKILL.md, packs/core/.apm/skills/work-intake/evals/, packs/core/.apm/skills/intake-intent/evals/, guides/core/reference/work-intake-routing-and-lifecycle.md, guides/core/how-to/start-the-work.md, guides/core/reference/workspace-toml-schema.md, packs/core/tests/skills/work-intake/test_work_intake.py, packs/core/tests/skills/intake-intent/test_intake_intent_manifest.py, packs/core/pack.toml, packs/core/.claude-plugin/plugin.json, docs/product/changelog.md, docs/specs/typed-intent-ordinal-allocator/notes/verification-ledger.md

## Rollout

Pack content only; adopters pick it up on the next install. New intents created through `work-intake` gain a typed prefix; existing files are untouched and existing references keep resolving. Two behaviour changes to flag in the changelog entry. An adopter whose intents carry no `Level`, or a level outside the parent's table, sees exactly today's behaviour — an unprefixed filename. An adopter who invokes `intake-intent` directly gets an unprefixed intent with a line in `## Unresolved questions` saying no ordinal was allocated — no refusal, and nothing they must do differently. An intent authored by `frame-intent` and then admitted is renamed to carry its ordinal, which is the one place an adopter sees a path change: the file they authored is not the file that ends up registered, and admission reports both paths.
...
- **A refusal becomes a block again.** Three review rounds drove this contract toward stopping, and each step looked locally right; only the brief's promise and the owner decision held the line. The likely regression is an implementer reading exit 1 as an error because that is what a non-zero status usually means. AC-0010's session is the guard that reaches it, and it asserts the intent exists — not that a message was printed.
- **A refusal becomes a block again.** Three review rounds drove this contract toward stopping, and each step looked locally right; only the brief's promise and the owner decision held the line. The likely regression is an implementer reading exit 1 as an error, because that is what a non-zero status usually means. Session 3 in the inventory is the guard that reaches it, and it asserts the intent exists — not that a message was printed.
