...
- **Not changing.** `next-ordinal.py` and its two suites; `intake-intent`'s `allowed-tools`, `## Boundaries` and every control in them; the existing corpus — the parent intent's guardrail makes adoption forward-only, so an intent carrying no ordinal today keeps none and no file is renumbered.
...
def test_the_three_filename_classes(tmp_path):
    for name, cls in (("FEAT-0001-x.md", "valid"),
                      ("FEAT-0001.md", "malformed"), ("FEAT-0001.txt", "malformed"),
        ...
        assert MODULE.classify(name) == cls                   # AC-0004, AC-0013
...
- One case per `Level` in the parent intent's table confirms the lookup returns that token, and three unmapped inputs — an intervening altitude, an absent field, a decorated `` `feature` `` — map to nothing (AC-0001, AC-0002).
...
- `token_for_level` is a dict lookup with an exact-match key, transcribing the parent intent's table rather than deciding it. The spec cites that owner; the script states the mapping directly because shipped pack content carries no internal-governance citations, and a comment names the transcription so a future edit knows where the decision lives.
- One mapping, one source of tokens. The introducer and the shape both derive their alternation from the mapping's values, so adding a row cannot leave the parser behind, and T2's bidirectional check is what catches a row added to the parser and not to the owner (AC-0014). It returns `None` for an absent, unmapped, or non-bare level — one backticked `feature` already exists in the corpus, and it is the live instance of an unmapped level, which admits unprefixed under AC-0002.
- The three filename classes are the load-bearing design choice, because most files in `docs/product/intents/` carry no typed prefix. Treating an unmatched name as an incomplete scan would make the live corpus unallocatable; treating a malformed typed name the same way would let it vanish from duplicate checking. Two patterns, not three, so the classes cannot leave a gap: the introducer `^<TOKEN>-` decides in-or-out of the namespace, and the end-anchored owner shape `^<TOKEN>-\d{4,}-[^/]+\.md$` decides valid-or-malformed inside it. Both are built from the mapping's own values, so the token set appears once in the module. Deriving malformed as "introducer and not shape" is what makes the partition exhaustive by construction; enumerating malformed shapes instead is how `FEAT-0001x.md` and `FEAT-0001` escaped an earlier draft. `EPIC-0001-x.md` is outside, not malformed, because `EPIC` is not in the table. `classify` returns the class rather than a boolean, so a fourth case cannot fall through to a default.
- The shape is end-anchored and requires a nonempty slug and a literal `.md`, which is the parent's `<TYPE>-NNNN-<slug>.md` contract and narrower than `next-ordinal.py`'s `[-.]` (AC-0013). Inheriting `[-.]` would count `FEAT-0001.md` and `FEAT-0001.txt`, letting a file the owner's contract does not admit raise the maximum. The roster test enumerates every typed file in `docs/product/intents/` and asserts each satisfies the anchored shape, so the narrowing is shown to reclassify nothing live and the check keeps working as the corpus grows. Verified before adopting the narrowing; the assertion is what keeps it true.
...
- The allocator's mapping and its parser's token namespace hold exactly the parent intent's table — keys and values, set equality in both directions (AC-0014). This is a roster test because it reads `docs/product/intents/FEAT-0001-intent-identity-and-registration.md`; a one-way check would pass an implementation that added `epic → EPIC` or kept a token the owner removed.
...
**Touches:** tests/roster/test_typed_ordinal_collision_equivalence.py, docs/product/intents/FEAT-0001-intent-identity-and-registration.md (read only)
...
- A construction check over `intake-intent/SKILL.md` frontmatter and its `## Boundaries` block rejects shell, network and any new tool (AC-0008). The existing suite cannot see the manifest, so it cannot carry this claim. That the slice's only edit there is one clause of Procedure step 3 is a verification choice this plan owns, not part of the criterion.
...
- Two existing-path cases, because an unprefixed mapped-level intent is the common shape on disk: an existing prefixed intent is updated in place unchanged, and an existing **unprefixed** intent at a mapped level is updated in place too — no allocation, no refusal, no renumber. The refusal covers creation only (AC-0009, AC-0010).
