...
The integration covers two entry paths with two mechanisms, because they have two capability sets — and **neither may stop an admission**, which is the owner decision of 2026-09-20. `work-intake` § 6 already "passes the confirmed repository destination, and authority mode to `intake-intent`" and already declares `Bash`, so it resolves the altitude against the closed table, allocates when the altitude maps, and supplies whichever destination results. The direct path at `SKILL.md:60` reaches `intake-intent` without a caller, and that skill's `## Boundaries` refuses a shell — so it cannot allocate and this slice does not grant it one. It admits at the unprefixed path instead and records in `## Unresolved questions` that no ordinal was allocated, which is what replaces refusing to write. A prefix supplied with the request is reported back rather than used, because the owner cannot check it against the working tree and `origin`; unverifiable is a reason not to trust it, not a reason to block. And an intent authored at the unprefixed path with no registry entry is not yet admitted, so admission allocates and renames it before writing that entry — the one case where the identity rule yields, because before admission there is no admitted identity to preserve.
