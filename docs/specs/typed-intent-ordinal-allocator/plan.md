...
- Procedure step 3 also states the rename carve-out: an intent at the unprefixed path with no registry entry is not yet admitted, so admission allocates and renames it before writing that entry, while an intent that already has one is preserved untouched (AC-0022). The carve-out names the one case rather than weakening the identity rule, and step 3 says the rename reports both paths and sweeps nothing (AC-0023).
