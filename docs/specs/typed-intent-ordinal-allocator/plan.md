...
- **This slice fires `work-loop`'s security-boundary trigger** — confinement, untrusted repository content, file validation, subprocess handling, and an output that selects a filesystem destination for a skill that writes. A spec-stage `security-reviewer` pass on 2026-09-20 discharged five findings as AC-0003 (tightened), AC-0016, AC-0017, AC-0018, AC-0019 and AC-0021, and reached Clean under the inlined boundary depth. **That pass predates the owner decisions**, which changed the direct path from refusing to writing an artifact and reporting a supplied untrusted prefix back to its requester — a new write on an untrusted-input path, which re-fires the trigger. A further spec-stage pass bound to the current revision is owed before implementation, and an implementation pass on the diff is owed at GATES. `AGENTS.md` forbids cutting a trust-boundary control, so each control stays a criterion rather than plan prose.
...
| 4 | direct `intake-intent` | mapped altitude | written at `<slug>.md`, no-allocation reason recorded, **not** registered | the artifact write | AC-0009 |
| 5 | direct `intake-intent` | a prefix bearing prompt-like Markdown, an ANSI escape, a newline and a secret-shaped string | written at `<slug>.md`; the artifact and the returned message contain none of those fragments, searched for by hand | the artifact write | AC-0009, AC-0024 |
...
- `intake-intent`'s Procedure step 3 states that it never blocks on an ordinal: lacking one it admits at the unprefixed path and records the reason in `## Unresolved questions`, it derives no ordinal itself, and it does not accept a prefix supplied with the request as proof of allocation — reporting the supplied value back instead of using it (AC-0009, AC-0010).
...
- The manual sessions are the canonical inventory in `## Construction tests`; T3 does not restate them or their count. Its completion condition is that every row there is recorded in the ledger with its stop boundary.
