...
- `test_either_promisor_designation_is_refused` (AC-0018) — deferred to EXECUTE; needs two `--filter=tree:0` clone fixtures with an unreachable remote, one carrying `remote.origin.promisor` and one carrying only `extensions.partialClone`
