...
- The manual sessions are the canonical inventory in `## Construction tests`; T3 does not restate them. Its completion condition is that every row there is recorded in the ledger with its stop boundary.
...
- `intake-intent`: one clause on Procedure step 3, and nothing else in the body. It cannot allocate — its `## Boundaries` refuses a shell and this slice does not grant one — so on the direct path it **writes the unprefixed intent and records that no ordinal was allocated**. It does not refuse, and it does not use a prefix supplied with the request: unverifiable against the working tree and `origin`, such a value is reported back and ignored. The cost is bounded and visible — a direct request yields an untyped intent with a note saying so — while an adopter never loses the ability to admit one, which is the property the owner decision protects.
