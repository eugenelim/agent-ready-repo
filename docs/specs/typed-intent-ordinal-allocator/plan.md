...
- The shape is end-anchored and requires a nonempty slug and a literal `.md`, which is the parent's `<TYPE>-NNNN-<slug>.md` contract and narrower than `next-ordinal.py`'s `[-.]` (AC-0013). Inheriting `[-.]` would count `FEAT-0001.md` and `FEAT-0001.txt`, letting a file the owner's contract does not admit raise the maximum. All 14 typed files in `docs/product/intents/` already satisfy the anchored shape, so the narrowing reclassifies nothing live — checked before adopting it.
...
- Two existing-path cases, because they are the live majority: an existing prefixed intent is updated in place unchanged, and an existing **unprefixed** intent at a mapped level is updated in place too — no allocation, no refusal, no renumber. The refusal covers creation only (AC-0009, AC-0010).
...
- 2026-09-20 — Round 8: the no-unprefixed-write invariant was universal and collided with forward-only adoption. 133 of the 141 live intents are unprefixed and most sit at a mapped level, so a universal invariant would have made every one of them either unwritable or a violation on its next edit. The invariant and AC-0010 are now scoped to creation, and the update-in-place case is stated and tested.
