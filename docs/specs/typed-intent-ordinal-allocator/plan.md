...
Refusal is the design centre, not an edge case — and it means one thing precisely: the allocator was asked and could not be sure of its answer. Where the untyped script's fallback is `0001`, the typed one returns nothing, for a malformed in-namespace filename, a scan it could not complete, a failed remote query, or a breached bound. An altitude the table has no token for is **not** a refusal: the allocator is never invoked, so there is nothing to refuse and nothing owed. Keeping those two apart is what lets a recorded reason mean something — it marks an ordinal that is owed, and an unmapped altitude owes none. `intake-intent` already has the receiving shape for both: `admit_repository_intent` takes `slug` and composes the target from it.

The integration covers two entry paths, and **neither stops an admission** — the owner decision of 2026-09-20, with the brief's promise at `:26` as its authority. `work-intake` § 6 already "passes the confirmed repository destination, and authority mode to `intake-intent`" and already declares `Bash`, so it resolves the altitude against the closed table, allocates when the altitude maps, and supplies whichever destination results; an allocator refusal changes the destination to the bare slug and adds a recorded reason, nothing more. The direct path at `SKILL.md:60` reaches `intake-intent` without a caller, and that skill declares no shell — so it admits at the unprefixed path and records that no ordinal was allocated. It derives none, and a prefix supplied with the request is **discarded, not echoed** — unverifiable against the working tree and `origin`, and a value that reaches a durable intent file must not be reproduced even correctly escaped. The ordinal-disposition text is fixed text from a closed set of outcomes (AC-0024). Unverifiable is a reason not to trust a value, not a reason to withhold an admission. Nothing renames: an intent already on disk keeps its filename, and the authoring case is deferred to `intent-renumber-and-reissue`.

## Assumption trio

**Written** — `packs/core/.apm/skills/work-intake/scripts/intent_ordinal.py` (new); `packs/core/.apm/skills/work-intake/scripts/file_safety.py` (new, byte-identical copy); `packs/core/.apm/skills/work-intake/SKILL.md` (§ 6); `packs/core/.apm/skills/intake-intent/SKILL.md` (one clause of Procedure step 3); both skills' `evals/evals.json` and `evals/eval_queries.json`; `packs/core/tests/skills/work-intake/test_intent_ordinal.py` (new); `packs/core/tests/skills/work-intake/test_work_intake.py`; `packs/core/tests/skills/intake-intent/test_intake_intent_manifest.py` (new); `tests/roster/test_typed_ordinal_collision_equivalence.py` (new); `tests/roster/test_close_work_extraction_and_immediate_disposition.py` (one added parity assertion); `.github/workflows/build-check.yml`; `tools/lint-ci-parity.py`; `guides/core/reference/work-intake-routing-and-lifecycle.md`; `guides/core/how-to/start-the-work.md`; `guides/core/reference/workspace-toml-schema.md`; `docs/product/README.md`; `packs/core/seeds/docs/product/README.md`; `packs/core/pack.toml`; `packs/core/.claude-plugin/plugin.json`; `docs/product/changelog.md`; `docs/specs/typed-intent-ordinal-allocator/notes/verification-ledger.md`.
...
**Done is demonstrated by** — the new pack suite and the two roster tests green, `test_intake_intent.py` green unamended, the three release gates named in the spec's `Release history` row passing, and every row of the canonical session inventory recorded in the verification ledger with its stop boundary.
...
- **This slice fires `work-loop`'s security-boundary trigger** — confinement, untrusted repository content, file validation, subprocess handling, and an output that selects a filesystem destination for a skill that writes. Two spec-stage `security-reviewer` passes have run. The first discharged five findings as AC-0003 (tightened), AC-0016, AC-0017, AC-0018, AC-0019 and AC-0021. The second was required because the owner decisions changed the direct path from refusing to writing an artifact, a new write on an untrusted-input path; it sustained one blocker, now closed by AC-0024. An **implementation pass on the diff is still owed at GATES**. `AGENTS.md` forbids cutting a trust-boundary control, so each control stays a criterion rather than plan prose.
...
| --- | --- | --- | --- | --- | --- |
| 1 | `work-intake` | mapped altitude, clean corpus | written at `<TYPE>-NNNN-<slug>.md`, registered | the registry entry | AC-0006 |
| 2 | `work-intake` | unmapped altitude | written at `<slug>.md`, registered, nothing added to `## Unresolved questions` | the registry entry, because AC-0007's outcome is *admitted and registered* | AC-0007 |
| 3 | `work-intake` | mapped altitude, a malformed in-namespace filename in the directory | written at `<slug>.md`, registered, refusal reason recorded | the registry entry | AC-0010 |
| 4 | direct `intake-intent` | mapped altitude | written at `<slug>.md`, fixed no-allocation text recorded, **not** registered | `workspace.toml` re-read after the write and shown unchanged — the claim is about registry state, so the endpoint has to be past it | AC-0009 |
| 5 | direct `intake-intent` | a prefix bearing prompt-like Markdown, an ANSI escape, a newline and a secret-shaped string | written at `<slug>.md`; neither the artifact nor the returned response contains any of those fragments | the returned response, captured and searched — an endpoint at the file would leave the response half unobserved | AC-0009, AC-0024 |
...
### T1: The allocator — one maximum per type, and no answer where it cannot be sure
...
- § 6 states that both non-allocating outcomes reach the same destination and differ only in what is recorded: an absent or unmapped level means the bare slug with nothing recorded (AC-0007), while an allocator refusal means the bare slug **plus** the reason passed through to the artifact (AC-0010). Neither stops. An earlier draft had the second arm stopping, which contradicted the brief's promise that a refused ordinal still leaves the intent admitted and registered; the owner settled it on 2026-09-20 in the brief's favour.
- `intake-intent`'s Procedure step 3 states that it never blocks on an ordinal: lacking one it admits at the unprefixed path and records fixed no-allocation text in `## Unresolved questions`, it derives no ordinal itself, and it **discards** a prefix supplied with the request rather than using or reproducing it (AC-0009, AC-0010, AC-0024).
- The marker written to `## Unresolved questions` and the message returned to the requester are both fixed text from a closed set of outcomes, asserted against the two skill bodies and observed once in session 5 with a hostile prefix (AC-0024).
- § 6 states that **nothing renames** (AC-0001): an intent already on disk when admission runs keeps its filename, whatever its altitude. No admission surface holds a move or delete, `intake_transaction.py`'s validated target is the only path the materializer may write, and `work-intake`'s `Bash` is declared for local Python validation and the `workspace-status` backend. Owner decision, 2026-09-20: the authoring case goes to `intent-renumber-and-reissue`, which needs a confined transactional rename for renumbering regardless.
- § 6 and the adopter-facing tables select a typed filename on **two** conditions, not one: the altitude maps *and* the request arrived through `work-intake`. A table keyed on altitude alone would promise a typed path to a direct `intake-intent` request, which AC-0009 writes unprefixed (AC-0001, AC-0009).
- `intake-intent`'s Procedure step 3 keeps its existing identity rule untouched — "its path is its identity; do not create a renamed copy" — because nothing in this slice asks it to rename. The earlier draft carved an exception into that rule; the carve-out is withdrawn with the rename.
...
- All seven admission prose surfaces state both intent destinations and the condition that selects them (AC-0006, AC-0007, AC-0014): `guides/core/reference/work-intake-routing-and-lifecycle.md`'s `Start routing` table, `guides/core/how-to/start-the-work.md`'s `intake-intent` walkthrough and its **Where it lands** line, `guides/core/reference/workspace-toml-schema.md`'s declared default path, `docs/product/README.md`'s file table, its seed at `packs/core/seeds/docs/product/README.md`, `packs/core/.apm/skills/work-intake/SKILL.md`'s default target shape, and `packs/core/.apm/skills/intake-intent/SKILL.md`'s update-in-place paragraph. The set came from an unrestricted sweep, not from the files a reviewer happened to name; the ledger records the command, the revision, and how the first narrower sweep produced a wrong claim.
- **No code changes outside the new allocator.** `intent_renderer.py:69` composes its target from the `slug` argument, so a caller passing `FEAT-0006-my-thing` yields the prefixed path with no edit at all — the seam was already there. `workspace_mcp.py:88-94` mentions the same path under its `"shape"` entry, whose `dispatch_skill` is `frame-intent`: authoring, not admission, and out of scope. An earlier ledger entry called both of these admission code surfaces; both classifications were wrong and are corrected there.
- The `guides/product-engineering/**` and `guides/_shared/` occurrences of the same path are asserted **unchanged**, because they describe `frame-intent` authoring rather than admission. Recording that is the point: a sweep that changed them would have moved a promise this slice does not make. Its current row promises `docs/product/intents/<slug>.md` for every admitted intent, which this slice makes false for a mapped level, and a published promise contradicting the shipped skill is the spec's one durable output. The replacement is **drafted here, before approval**, as the durable-output contract requires; the published file is edited in EXECUTE so an adopter never reads a promise the code does not yet keep:

  ```markdown
  | Input shape | Canonical artifact | Initial lifecycle | Processor |
  | --- | --- | --- | --- |
  | Minimal outcome routed through `work-intake`, `Level: product-vision` | Intent at `docs/product/intents/VISION-NNNN-<slug>.md`, the ordinal allocated at admission | Draft, non-dispatchable | `intake-intent` |
  | Minimal outcome routed through `work-intake`, `Level: product-strategy` | Intent at `docs/product/intents/STRAT-NNNN-<slug>.md`, the ordinal allocated at admission | Draft, non-dispatchable | `intake-intent` |
  | Minimal outcome routed through `work-intake`, `Level: capability` | Intent at `docs/product/intents/CAP-NNNN-<slug>.md`, the ordinal allocated at admission | Draft, non-dispatchable | `intake-intent` |
  | Minimal outcome routed through `work-intake`, `Level: feature` | Intent at `docs/product/intents/FEAT-NNNN-<slug>.md`, the ordinal allocated at admission | Draft, non-dispatchable | `intake-intent` |
  | Minimal outcome routed through `work-intake`, a mapped `Level`, allocation unavailable | Intent at `docs/product/intents/<slug>.md`, with a note that the ordinal is still owed | Draft, non-dispatchable | `intake-intent` |
  ...
  | Request naming `intake-intent` directly, any `Level` | Intent at `docs/product/intents/<slug>.md`, with a note that no ordinal was allocated | Draft, non-dispatchable | `intake-intent` |
  ```

  The exact mapping rather than a generic `<TYPE>` placeholder, because AC-0014 requires every maintained copy to hold the parent's keys and values and a placeholder holds neither. Six rows rather than a conditional footnote, because the destination is what an adopter looks up and a footnote is what they miss. Existing intents keep their current paths; the table describes admission, not the corpus.
...
- Both skills' `evals/` cover the new behaviour: `work-intake` a mapped-level admission, an unmapped-level one, and an allocator refusal that still admits; `intake-intent` the direct path's unprefixed write with its recorded reason, and the existing-path preservation. No eval asserts a refusal to admit, because none happens (`packs/AGENTS.md:60`).
- The manual sessions are the canonical inventory in `## Construction tests`. T3 restates neither their count nor their composition; its completion condition is that every row there is recorded in the ledger with its stop boundary.
...
- `intake-intent`: two prose edits and no more. Procedure step 3 gains the never-block clause — it cannot allocate, its `## Boundaries` refuses a shell and this slice does not grant one, so on the direct path it writes the unprefixed intent and records that no ordinal was allocated, neither refusing nor reproducing a prefix supplied with the request. The ordinal-disposition text — the marker and the part of the response that speaks about the ordinal — is **fixed text** from a closed set of outcomes, carrying no byte of the supplied prefix, the `Level`, or any other requester-controlled field (AC-0024). Fixed rather than sanitized, because an intent is a durable file a later agent reads and a filter only has to be wrong once. The intent's own content and the path receipt are untouched: minimizing those is `intake-intent`'s existing provenance contract. And the update-in-place paragraph at `SKILL.md:61-67` states both destinations, because it currently cites an unconditional `docs/product/intents/<slug>.md` default while telling an agent not to rename to match it — true of an update, stale about creation. Its identity rule itself is untouched: nothing here renames.
...
**Done when:** `python3 -m pytest tests/roster/test_typed_ordinal_collision_equivalence.py -q` is green including the prose-parity assertions, the three release gates named in the spec's `Release history` row pass, the prose and manifest assertions pass, `test_intake_intent.py` is green unamended, both eval harnesses cover the new behaviour, the published routing table and the skill bodies agree, `make lint-ruff lint-mypy` and `make build-self` pass on a clean tree, and every row of the canonical session inventory in `## Construction tests` is recorded in `notes/verification-ledger.md` with its stop boundary.

**Touches:** tests/roster/test_typed_ordinal_collision_equivalence.py, packs/core/.apm/skills/work-intake/SKILL.md, packs/core/.apm/skills/intake-intent/SKILL.md, packs/core/.apm/skills/work-intake/evals/, packs/core/.apm/skills/intake-intent/evals/, guides/core/reference/work-intake-routing-and-lifecycle.md, guides/core/how-to/start-the-work.md, guides/core/reference/workspace-toml-schema.md, docs/product/README.md, packs/core/seeds/docs/product/README.md, packs/core/tests/skills/work-intake/test_work_intake.py, packs/core/tests/skills/intake-intent/test_intake_intent_manifest.py, packs/core/pack.toml, packs/core/.claude-plugin/plugin.json, docs/product/changelog.md, docs/specs/typed-intent-ordinal-allocator/notes/verification-ledger.md

## Rollout

Pack content only; adopters pick it up on the next install. An intent that `work-intake` **creates** at a mapped altitude gains a typed prefix. Everything else an adopter has today is untouched: existing files keep their names, existing references keep resolving, an intent with no `Level` or an unmapped one lands unprefixed exactly as it does now, and nothing is renamed. The one visible addition is a line in `## Unresolved questions` when an ordinal was owed but could not be allocated — which tells an adopter the alias can be added later rather than that something failed. There is no path on which an adopter loses the ability to admit an intent, which is the property the owner decision of 2026-09-20 protects.
...
- **A refusal becomes a block again.** Three review rounds drove this contract toward stopping, and each step looked locally right; only the brief's promise and the owner decision held the line. The likely regression is an implementer reading exit 1 as an error, because that is what a non-zero status usually means. Session 3 of the canonical inventory is the guard that reaches it, and it asserts the intent exists rather than that a message was printed.
...
- 2026-09-20 — Spec and plan approved by eugenelim. The approval basis as recorded then: shaping review Clean, adversarial review `Clean — ready to commit.`, and the spec-stage security pass discharged into criteria.
- 2026-09-20 — The approval above was bound to revision `f6df9571b`, and the contract changed substantively after it: the refusal semantics reversed so admission never blocks, admission-time renaming was withdrawn on capability evidence with AC-0022 and AC-0023 retired, and AC-0021 and AC-0024 were added from two security passes. A prior revision's approval cannot authorize a revised scope.
- 2026-09-21 — **Re-approved at revision `87d6cced3` by eugenelim**, on the instruction to start implementing, given after the owner was shown: the two decisions above and their costs, that adversarial review had not reached Clean, and the three ways to spend the next effort. The authorization is to build this revision; it is not a claim that adversarial review returned Clean on it. Both spec-stage security passes are complete and their findings are criteria; the implementation pass on the diff is owed at GATES.
