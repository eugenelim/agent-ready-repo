...
- § 6 states the rename and owns it (AC-0022): an intent at the unprefixed path with no registry entry is not yet admitted, so `work-intake` allocates, renames, and then writes the entry at the final path. It is § 6's because `intake-intent:110` hands registration to the calling workflow and its `allowed-tools` carry no move or delete — the owner literally cannot rename without the capability widening AC-0008 forbids. § 6 also states that the rename reports both paths and sweeps nothing (AC-0023), and that the direct path never renames, having no caller to keep a registry entry in step with.
- `intake-intent`'s Procedure step 3 gains the matching read of the same rule: a destination it is handed may differ from a path already on disk when the caller renamed a not-yet-admitted intent, and that is not the renamed-copy its identity rule forbids (AC-0022). Naming the one case is what keeps the identity rule intact rather than weakened.
...
- All four admission surfaces state both intent destinations and the condition that selects them (AC-0006, AC-0007, AC-0014): `guides/core/reference/work-intake-routing-and-lifecycle.md`'s `Start routing` table, `guides/core/how-to/start-the-work.md`'s `intake-intent` walkthrough and its **Where it lands** line, `guides/core/reference/workspace-toml-schema.md`'s declared default path, and `packs/core/.apm/skills/work-intake/SKILL.md`'s default target shape. The set came from a repository-wide sweep for the literal path rather than from the two files review happened to name; `intake-intent`'s own update-in-place paragraph also gains the creation nuance, since it currently cites the unprefixed convention while telling an agent not to rename to match it.
...
- Nine recorded sessions (AC-0006, AC-0007, AC-0009, AC-0010, AC-0016, AC-0022, AC-0023). Through `work-intake`: a mapped level lands at `<TYPE>-NNNN-<slug>.md`; an unmapped level lands at `<slug>.md`, registered, with nothing added to `## Unresolved questions`; and an allocator refusal, induced by a malformed in-namespace name, lands at `<slug>.md`, registered, **with** the reason recorded. Direct to `intake-intent`: a mapped-level request lands unprefixed with the reason recorded; the same request carrying an arbitrary `<TYPE>-9999-` prefix also lands unprefixed, with the supplied value reported back rather than used. The rename: an intent authored at `<slug>.md` with no registry entry, admitted through `work-intake`, is renamed to `<TYPE>-NNNN-<slug>.md` before its entry is created, and `git status` shows exactly the rename plus the registry edit and no other file touched (AC-0023's non-sweep, asserted differentially). A ninth session takes the same authored intent through the *direct* path and shows it is **not** renamed, which is the arm that would otherwise be assumed rather than observed. Preserved: an intent that already has a registry entry is updated in place, prefixed or not. Plus the caller-side output-validation session for AC-0016. Through `work-intake`: (1) a mapped level lands at `docs/product/intents/<TYPE>-NNNN-<slug>.md`; (2) an unmapped level lands at `docs/product/intents/<slug>.md` **and is registered** — this session continues past the write through registration, because AC-0007's outcome is "admitted *and registered*" and stopping at the file cannot see it. Direct to `intake-intent`: (3) a new mapped-level request refuses; (4) the same request carrying an arbitrary `<TYPE>-9999-` prefix refuses on the same ground rather than being trusted; (5) an existing **unprefixed** mapped-level intent is updated in place, no allocation and no refusal; (6) an existing **prefixed** intent is updated in place with its ordinal unchanged. Each records its stop boundary: sessions 1 and 6 stop at the durable write, session 2 at the registered entry, sessions 3 and 4 at the refusal before any write, session 5 at the in-place edit.
...
- **The rename over-reaches.** AC-0022 applies to one case: unprefixed, mapped level, no registry entry. Applied to an intent that already has an entry it becomes an unsanctioned renumber with a citation sweep this slice does not own; applied at an unmapped level it invents a token. T3's preserved-path session and the unmapped session fail on those two errors respectively, and AC-0023's differential `git status` check catches a sweep that ran when it should not have.
