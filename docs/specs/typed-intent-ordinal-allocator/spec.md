...
| --- | --- | --- | --- | --- | --- |
| Adopter-facing admission-path promise | Applicable — four surfaces promise `docs/product/intents/<slug>.md` for an admitted intent, which becomes false for a mapped level. A repository-wide sweep for that path found the full set; the surfaces that describe `frame-intent` *authoring* rather than admission are listed as out of scope below, because this slice changes admission only | `guides/core/reference/work-intake-routing-and-lifecycle.md` (the `Start routing` table); `guides/core/how-to/start-the-work.md` (the `intake-intent` walkthrough and its **Where it lands**); `guides/core/reference/workspace-toml-schema.md` (the declared default path); `packs/core/.apm/skills/work-intake/SKILL.md` (the default target shape) | T3 | Each surface states both destinations and the condition that selects them, asserted against the parent's parsed table by the same roster check that covers § 6 | Every listed surface and the shipped skill bodies agree on where a mapped-level intent lands, and a repeat of the sweep finds no unlisted survivor |
