...
- [ ] **AC-0018.** Git access is local-ref only, and **no invocation may reach the network**. An argument vector free of `fetch` does not establish that: `git ls-tree` on a partial clone resolves a missing tree through the promisor remote, verified on git 2.50.1 against a `--filter=tree:0` clone with an unreachable remote, which attempted a transport and reported `could not fetch <oid> from promisor remote`. So the child environment carries `GIT_NO_LAZY_FETCH=1`, which on that same fixture failed closed with `not a tree object` and no transport; and because this repository declares no git version floor and that variable is ignored by a git older than 2.41, a configured `remote.<name>.promisor` is itself treated as a remote view the allocator refuses. `remote.<name>.promisor=false` is **not** a control — on the same fixture it still attempted the transport. Arguments are fixed and non-shell, stdin is closed, and repository and object-store redirect variables are removed from the child environment.
