...
- [ ] **AC-0021.** Four bounds hold, each with its measured origin, and every breach is an AC-0011 scan failure rather than a partial answer:
  ...
  | Entries consumed, local **or** remote | 65,536 | ~300× this repository's largest tracked directory, `tools` at 215 entries; one constant governs both halves, so neither half is bounded while the other is not |
