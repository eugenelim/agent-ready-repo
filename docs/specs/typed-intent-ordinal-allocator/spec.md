...
- [ ] **AC-0004.** The allocator classifies every filename it meets into one of three declared classes, and each has a stated outcome: a name outside the typed namespace is skipped, a typed-looking name it cannot fully parse makes the scan incomplete so no ordinal is returned and no clean duplicate check is reported, and a filesystem-read failure refuses the operation. No class falls through to a default.
