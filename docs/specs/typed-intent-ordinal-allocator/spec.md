...
- **VI-0007 — the invocation cannot become a command (AC-0016):** TDD at two artifacts in one item, because the criterion is one rule with two ends. The script's handling of a hostile `Level` is unit-level at the unit artifact; the caller's validation of what comes back is `packs/core/tests/skills/work-intake/test_work_intake.py` plus one recorded session, since no unit test of the script can reach the router that composes the path.
