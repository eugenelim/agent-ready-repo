...
- [ ] **AC-0009.** On the direct entry path, where a request names `intake-intent` and no caller has allocated, admission proceeds at the unprefixed path and records in `## Unresolved questions` that no ordinal was allocated and why. It does not refuse, and it does not accept a prefix supplied with the request as proof of allocation — unverifiable against the working tree and `origin`, such a prefix is neither trusted nor a blocker, so the destination is the bare slug and the supplied value is reported back rather than used. This path writes the artifact and does not register it, which is unchanged: `intake-intent:110` hands registration to "the calling intake workflow", and a direct invocation has none. This slice neither adds registration there nor removes it.
- [ ] **AC-0010.** When the allocator refuses — an unparseable name inside the namespace, an incomplete scan, a failed remote query, a breached bound — admission still writes and registers the intent at the unprefixed path, and records the refusal's reason in `## Unresolved questions`. A refusal changes the filename, never whether the intent exists. An operator sees no failure because none occurred: the intent was admitted.
