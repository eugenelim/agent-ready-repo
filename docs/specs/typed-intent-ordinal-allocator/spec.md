...
- **VI-0005 — refusal over plausibility, and what a refusal costs (AC-0010, AC-0011, AC-0015):** TDD, unit artifact. The distinction between an unknowably incomplete view and a complete-but-empty one is observable only at the call, because the two look identical from outside. AC-0010's half is that a refusal reaches the caller as *no ordinal*, never as a failure to admit — asserted at the unit boundary as the returned value, and at VI-0009's boundary as an intent that still exists.
...
- **An intent authored at the unprefixed path never acquires an ordinal from this slice.** `frame-intent` writes `<output_dir>/intents/<slug>.md`; admission preserves an existing path, and no admission surface can rename one. So the authoring route yields untyped intents until `intent-renumber-and-reissue` ships — the right home, because that slice must build a confined, locked, rollback-aware rename with registry lockstep for renumbering anyway, so a second one here would be the waste. Owner decision, 2026-09-20. Owner: eugenelim.
