...
> **Attribution.** Every verdict below was reached by this session reading the
> named artifact, not by a human reviewer. The spec calls for a named reviewer's
> recorded verdict; if that is meant to be a person, these eight rows need the
> owner's countersignature before the Gates criterion is satisfied. Recorded
> this way rather than under the owner's name, which would misstate who read what.
