...
> named artifact, and reviewed and accepted by **eugenelim on 2026-09-24**. The
...
| --- | --- | --- | --- | --- | --- |
| 1 | The four behavioural eval cases distinguish old behaviour from new — A and D pass, B and C fail, run against the pre-change skill | `evals/evals.json` against `SKILL.md` at `9748dd256` | **Pass — the cases discriminate.** Run against `SKILL.md` at `9748dd256`: **A passes** (no divergence generation exists pre-change, so no round runs; gate 2 forbids a second doc). **B fails** (nothing names or excludes the category default). **C fails** (the genre tier directs referents to named software products, the opposite of the criterion). **D passes** (gate 2 and the re-deriving-taste refusal together give amend-not-replace). Matches the spec's predicted A/D pass, B/C fail. | agent session (controller) | 2026-09-24 |
| 2 | Each operation's stability contract is complete and non-overlapping | `SKILL.md` for `frame`; `references/explore.md`, `visualize.md`, `converge.md`, `refine.md` for the other four | **Sufficient, with one noted overlap.** All five operations carry `May change:` and `Must remain stable:` — frame (brief components / nothing locked before it), explore (session candidates / the brief), visualize (compositional commitments / axis tokens), converge (the direction doc / the selection, since the agent does not choose), refine (named axes / ranked goals, dominant goal, referents, signature device, every unnamed axis). Overlap: visualize and converge both name the direction doc as changeable. Benign, because visualize writes nothing of its own and its commitments reach the file through converge's capture, but the wording is looser than the rest. | agent session (controller) | 2026-09-24 |
| 3 | The operation-selection rubric is decidable without loading a reference | `SKILL.md`, read without opening any reference | **Sufficient.** The rubric is five bullets keyed on the shape of the request — a plain-words change against an existing direction routes to refine; asking to see a candidate routes to visualize; choosing between candidates routes to converge; a new surface, section, component or feature routes through the route rule; probing the audience routes to frame. Every term it uses is defined in `SKILL.md`. No reference load is needed to reach an operation. | agent session (controller) | 2026-09-24 |
| 4 | The anti-pattern reference's era labelling is honest — a dated observation is distinguished from a durable one | `references/referents.md` | **Honest.** All 21 entries carry one of the three classifications — 4 quality invariant, 7 useful heuristic, 10 model-era anti-pattern — and all 10 model-era entries carry the period they describe (2020–2024, 2022–2024, 2023–2024). The three tiers are actually used rather than collapsing into one, which is what makes the labelling informative: a reader can tell a dated observation from a durable one. | agent session (controller) | 2026-09-24 |
| 5 | The guide is sufficient | `guides/experience-design/how-to/establish-design-intent.md` | **Sufficient, re-taken after a correction.** The guide names all three routes with their triggers and all five operations with their purposes, and a reader can act without opening `SKILL.md`. The first verdict was wrong on one point and is superseded: it asserted the guide stated `visualize` correctly, when the guide gated the whole operation on harness capability — telling a markdown-only adopter the operation does not run for them, which the Agent Rules forbid. The gate now falls on the rendered comp, matching `references/visualize.md`. The first verdict could not have caught this from the artifact it was scoped to; judgement 6's sweep is scoped to `SKILL.md` and `references/`, so the guide sat outside every manual-QA surface until the post-gates review reached it. | agent session (controller), accepted by eugenelim | 2026-09-24 |
| 6 | No sentence makes a visual artifact a precondition for writing the direction doc | `SKILL.md` and every file under `references/` | **Pass.** Swept `SKILL.md` and every file under `references/` for comp, render, image, mock and wireframe. No sentence makes writing the direction doc conditional on producing a visual. `visualize.md` states the inverse directly: a rendered comp is produced only when the harness can produce one and the route is originate, and its absence is a named skip, never a blocker. | agent session (controller) | 2026-09-24 |
| 7a | The four departing sections left `SKILL.md`, each with its method intact in the reference that now owns it and nothing paraphrased back into the body | post-change `SKILL.md` against `9748dd256` | **Pass, after two repairs during execution.** All four sections left `SKILL.md`, and all eighteen obligations from the departing procedure were traced to a current home. Two move tasks initially lost rules that every contracted grep still passed: converge.md dropped the push-back remedy for an ungrounded goal, the first-seven-are-structural rule, the cell-opens-with-tokens rule and the what-to-fill enumeration; referents.md dropped all three genre-tier prohibitions and refusal 7's platform-standard examples. All were restored before this verdict. Nothing is paraphrased back into the body. | agent session (controller) | 2026-09-24 |
| 7b | The three enumerations are exact and complete — three routes each with trigger and operations, five operations each with a one-line purpose, three representations | post-change `SKILL.md`, and `references/visualize.md` for the third | **Pass.** Route table: exactly 3 rows — inherit, extend, originate — each with a non-empty trigger and operations cell. Operations: exactly 5 headings. `visualize.md`: exactly 3 representations, each with its binding force stated. Read as rendered, not by pattern count. | agent session (controller) | 2026-09-24 |
