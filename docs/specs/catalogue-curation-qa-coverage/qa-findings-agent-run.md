...
**Run:** source = `llm-wiki-kit` (an external skill catalogue), target catalogue =
this repo (`agent-ready-repo`, HEAD `5b6deec4`).
**Method:** each AC was exercised by acting as the assimilating agent — following the
skill's `SKILL.md` + references against the AC's input (fixture, and real llm-wiki-kit
...
>
> **Human QA update (2026-08-02):** a subsequent live operator QA pass confirmed all four
> paths **PASS** — see the parent spec QA log for the full session records. The agent-run
> PARTIALs on AC4/AC6/AC7 reflect skill-documentation gaps (oracle knowledge not yet promoted
> into the skill itself), not incorrect behaviour. The findings below remain actionable as
> skill improvement work for a follow-up PR.
...
| AC | Skill / path | Agent-run verdict | Human QA verdict | Headline finding |
|---|---|---|---|---|
| AC4 | `assimilate-repo` re-sync routing | **PASS (logic) / PARTIAL (runnability)** | **PASS** | Routing tree is correct, but the path has no self-contained fixture and the Approver-sign-off gate lives only in the oracle, not the skill. |
| AC5 | `assimilate-primitive` anti-pattern steering | **PASS** | **PASS** | All three fixtures detected + steered correctly; no false positives on real llm-wiki-kit skills/scripts. |
| AC6 | `propose-catalogue-pack` | **PARTIAL** | **PASS** | Reject-on-duplication works, but the skill never encodes the CHARTER's **accelerator-pack** routing (exemption + 3 gates) it's expected to apply. |
| AC7 | `assimilate-primitive` hook-confirm | **PARTIAL** | **PASS** | Detection + confirm gate are correct; the **landing** path (git-hook mechanics, version/inventory bumps) and the **SAST tool list** exist only in the oracle, not the skill. |
...
**Ran:** the three routing forms against a llm-wiki-kit-as-source scenario — "a prior `assimilate-repo`
of llm-wiki-kit produced RFC-0001 in this catalogue; llm-wiki-kit has since grown, re-sync." Traced against
...
- 🟧 **No self-contained fixture — AC4 isn't runnable standalone.** AC5/AC7 ship fixtures; AC4 ships
  only a `notes/` oracle that references an *external prior artifact* (the adopter-repo RFC-0001 from
...
false-positive check against real llm-wiki-kit primitives (a terse llm-wiki-kit skill and
...
- **No false positives** on llm-wiki-kit: terse `name`+`description` skills and the deterministic
...
**Ran:** a real llm-wiki-kit-derived proposal — "add a `knowledge-work-deliverables` pack (ADRs, PRDs,
runbooks, decks, threat models, journey maps from llm-wiki-kit)" — through `propose-catalogue-pack/SKILL.md`
...
`product-strategy` (llm-wiki-kit's `prfaq`/`okr-cascade`/`strategy-analysis`), `experience-design`
...
elicitation also correctly surfaces the genuinely-additive slice (llm-wiki-kit's `runbook-generator` /
...
check against a real llm-wiki-kit script (`build_deck.py`) to test whether the code-confirm class keys on
...
- AST01–AST10 correctly scoped to **SKILL.md/behaviour-definition files, not raw scripts** (step 5),
  matching the oracle. ✓
- Real llm-wiki-kit `build_deck.py` also trips the code-confirm class → the gate keys on *executable code*,
