...
**Done when:** the five `test_exit_codes.py` suites green; ported resolution suite green; missing-`credbroker`→exit-1 (clean, no traceback) verified per CLI; grep shows zero `credentials_shim` imports in the five CLIs; `credbroker` in all five skill-root requirements files.
