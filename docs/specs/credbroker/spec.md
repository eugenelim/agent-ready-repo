...
- [ ] The reserved `3–9` exit band stays unclaimed: credbroker ships no `setup`/`check` CLI in v1, and no consumer's exit-code *constants* (`EXIT_OK`/`EXIT_ERROR`/`EXIT_USER_ACTION`) change.
- [ ] A missing `credbroker` install resolves to **exit 2** (`USER_ACTION`, "run pip install") through each CLI's existing import guard — not a traceback and not exit 1 — because the migrated resolver is now a pip dependency in the same class the guard already handles (PR #230). The `credentialed-cli-exit-code-contract` spec's "unprojected shim `ModuleNotFoundError` → exit 1" clause is amended (via a dated Changelog/Errata entry on that spec) to reflect that there is no projected shim for the five CLIs after migration; the clause's residual scope is the `sso-broker` companion path only.
