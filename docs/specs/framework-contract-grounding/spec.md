...
- Keep the change to **prose + routing** on the existing EXECUTE gate (`work-loop/SKILL.md`) and the existing `contract-acquisition` skill — edit the pack **source** under `packs/core/.apm/skills/…`, then project via `make build-self` (never hand-edit `.claude/skills/…`).
...
- Any change that would touch `contract-acquisition`'s **tiering structure** rather than adding the software surface to it (the tier shape is shared with infra).
...
- [x] The gate routes the software case to a **detect-and-recommend tier that mirrors `contract-acquisition`'s T2** — detect a framework-library skill (internal *or* published vendor) **or** a Context7-style retrieval surface **or** official versioned docs via `research`; consult-and-cite if present; recommend-and-surface-as-a-decision if absent.
