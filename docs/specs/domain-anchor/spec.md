...
- **Brief:** none
- **Contract:** none — the typed artifact is a prompt-only markdown schema carried in the `domain-anchor` skill body (Charter Principle 3); there is no `contracts/<type>/` file, no API surface.
- **Shape:** mixed
...
discovery loop, or standalone — runs the `domain-anchor` skill and gets a
**single typed artifact that grounds the product in the real-world activity it
serves before any screen, service, or architecture is drawn**. The artifact has
three load-bearing parts: a **real-world-activity half** (how the activity is
...
intuition); for a brownfield product a **current-system half** (how the existing
system already does it, reverse-engineered from code and docs via
`decision-archaeology` plus architecture extraction); and an **MVP out-of-scope
register** that names, explicitly, the tempting capabilities the appetite
excludes. Its purpose is correctness: it is the single biggest lever against the
two ways an autonomous upstream goes wrong — **hallucinating a domain it does not
know**, and **over-scoping past the MVP**. The artifact carries a stable marker —
a canonical filename and a frontmatter `type:` — so downstream lenses and tools
can find it regardless of where the adopter's layout puts it, and the skill
resolves its write path through the catalogue's standard three tiers
(config → designed default → discover-by-marker) so it is never hardcoded. Each
finding the agent cannot ground from evidence is surfaced as a named residual
assumption rather than silently asserted, so a human reviewing the MVP boundary
sees exactly what rests on grounding and what rests on a guess.
...
- **Wrap `research` applied mode** to produce the real-world-activity half — the
  applied-mode discipline (practitioner grey literature, the prior-art / best-
  practice / case-study / anti-pattern frames, the survivorship-bias and
  ...
  and shapes them into the typed artifact; it does not re-implement retrieval.
- **Reverse-engineer the brownfield current-system half via `decision-archaeology`**
  ...
  only when a current system exists.
- **Carry the MVP out-of-scope register** as a first-class section: the explicit
  list of tempting-but-excluded capabilities, each with the appetite reason it is
  out, so the scope-creep guard and the human at the MVP boundary have a referent.
- **Emit the stable marker** on every artifact: the canonical filename
  `domain-anchor.md` *and* a frontmatter `type: domain-anchor`, so the artifact is
...
- **Resolve the write path in three tiers** — (1) the adopter's chosen discovery
  ...
  the `domain-anchor.md` is a shared *discovery-loop* artifact, so its config home is
  the discovery layout key, not `product-engineering`'s file-per-slug
  `intents`/`rollups` table); (2) the designed default
  `docs/discovery/<initiative>/domain-anchor.md`; (3) discover-by-marker — and
...
- **Adding a fourth top-level component** to the artifact beyond the three this
  spec fixes (real-world-activity half, current-system half, out-of-scope
  register) — e.g. folding the persona in as a co-product (RFC-0048's artifact
  inventory lists it as a *separate* artifact; it is out of scope here).
- **Changing the canonical filename or the frontmatter `type:` value** — both are
  the stable contract other skills and the traceability lint resolve against;
  renaming either is a cross-artifact break.
...
- **Skill-body contract — wrapping, components, marker, resolution, degrade
  ...
  `decision-archaeology`, names the three required components and the residual-
  assumption rule, emits the canonical filename + frontmatter `type: domain-anchor`,
  ...
  *specifies* the degrade behaviour, backstopped by the `Never do` "no silent
  assertion" rule.
- **Discoverability by marker (AC6):** goal-based check — a grep for the frontmatter
  `type: domain-anchor` against an artifact written to a *non-default* path resolves
  it, proving discovery does not depend on the path.
- **One observable produced artifact (AC2 + AC6):** visual / manual QA — a real
  `domain-anchor` invocation against the RFC-0048 worked example
  (`example-assistant`) through its documented happy path produces a single
  `domain-anchor.md` carrying all three components (AC2) and the marker (AC6),
  recorded in the implementing PR. This is the end-to-end surface: the skill only
  proves out across the wrapped-research → shape → write flow.
...
- [ ] **AC1 — wraps research applied mode.** The `domain-anchor` skill body
  invokes `research` in `applied` mode to ground the real-world-activity half, and
  states that it *consumes and shapes* the applied-mode findings rather than
  re-implementing retrieval.
- [ ] **AC2 — three required components.** The artifact schema in the skill body
  defines exactly three top-level components: the real-world-activity half (how the
  activity is really done · best practice · naive-failure modes), the brownfield
  current-system half, and the MVP out-of-scope register.
- [ ] **AC3 — brownfield half via decision-archaeology.** The skill body
  reverse-engineers the current-system half via `decision-archaeology` plus
  architecture extraction, and states this half is produced **only when a current
  system exists** (greenfield omits it, and the artifact says so).
- [ ] **AC4 — out-of-scope register is first-class.** The schema requires the
  out-of-scope register to list each excluded capability *with its appetite
  reason*, not a bare list.
...
- [ ] **AC6 — stable marker.** Every produced artifact is named `domain-anchor.md`
  and carries frontmatter `type: domain-anchor`; given an artifact at a
  non-default path, a marker search (`type: domain-anchor`) resolves it.
- [ ] **AC7 — three-tier path resolution.** The skill body documents resolution in
  order: (1) the adopter's discovery base from `agentbundle-layout.toml` (RFC-0040 /
  ADR-0030 adopter-file mechanism — whatever discovery layout key the cross-cutting
  layout effort settles; currently unbound, see § Ask first, so resolution falls to
  the default and marker tiers until then; *not* `product-engineering`'s
  `intents`/`rollups` table), (2) designed default
  `docs/discovery/<initiative>/domain-anchor.md`, (3) discover-by-marker; it creates
  the directory lazily on first write and surfaces ambiguity rather than guessing.
...
- Technical: the primitive ships as a new prompt-only skill `domain-anchor` in the
  `product-engineering` pack (source: `packs/product-engineering/.apm/skills/`;
  RFC-0048 worked-example table row 3 "domain-anchor (PE)").
- Technical: `research` applied mode emits `<topic-slug>-survey.md` and does not
  emit the domain-anchor for free, so the skill wraps and shapes it (source:
...
- Product: the artifact's three components are exactly the real-world-activity
  half, the brownfield current-system half, and the MVP out-of-scope register; the
  persona is a separate artifact, excluded here (source: RFC-0048 Decision 4 + task
  direction; `docs/rfc/0048-notes/04-artifact-inventory.md`).
