...
- **D1:** The Python port of `msg-to-markdown` reads `.msg` with `olefile`
  (BSD-2-Clause, pure-Python) for OLE2/CFBF container access, pinned `>=0.47`.
- **D2:** The MAPI property layer is hand-rolled first-party code in the skill
  (`scripts/mapi.py`), not a third-party wrapper.
- **D3:** The skill reads and decodes the `__substg1.0_<id><type>` property
  streams itself, covering PtypString (`001F`), PtypString8 (`001E`), integers
  (`0003`), FILETIME (`0040`), and binary (`0102`/`000D`).
- **D4:** `olefile` is resolved pip-on-demand through the skill's `--check` verb
  and is never auto-installed or bundled/vendored.
- **D5:** Copyleft dependencies are excluded on license regardless of technical
  merit, so `extract-msg` cannot be adopted.
- **D6:** The decision is scoped to the `converters` pack's `msg-to-markdown`
  skill only.
...
**Revisit if:** `olefile` ships a new major version or a release is yanked, which is the record's stated re-review trigger for the `>=0.47` minimum pin (D1, D4); or `olefile` is abandoned, which forces vendoring or re-implementing the thin CFBF container layer; or a permissive, maintained, Python-3-correct `.msg` library reaches the ecosystem and collapses the hand-rolled MAPI layer (D2).
