...
The adapter catalogue projects one canonical set of pack primitives (`.apm/skills`, `.apm/agents`, `.apm/hooks`, `.apm/hook-wiring`, `.apm/commands`) into each tool's native discovery layout. Gemini CLI was classified in `guides/_shared/reference/adapter-support.md` as a **"Universal layer" / `AGENTS.md` reader only** — no skills, subagents, commands, or hooks.
