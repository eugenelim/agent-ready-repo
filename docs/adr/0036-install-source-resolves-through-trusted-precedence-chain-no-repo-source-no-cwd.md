...
**Revisit if:** real usage shows the auto-detected clone is more often the
intended source than the persisted one, reopening D1's layer-2-over-layer-3
order; a planted-marker hijack is observed against D2's accident-guard; or the
deferred layer-4 integrity pin lands and changes D5's asymmetric posture.

## Confirmation

- **Mode:** review-time plus the existing test suite — no new CI gate, matching
  Principle 3.
- **Signal:** the implementing spec's acceptance criteria below; the ~13 adapter
  default-resolution tests and `tests/unit/test_resolve_user_scope_target_adapter.py`
  staying green; and the mandatory `security-reviewer` plus `adversarial-reviewer`
  passes confirming no repo-scoped source or cwd fall-back creeps back in.
- **Owner:** eugenelim
