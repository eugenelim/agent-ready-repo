...
| Full changelog and `/now/` inputs | Yes | Both consumers use one parsed model so content and ordering cannot drift. |
...
| `/now/` purpose and editorial authority | [`/now/` surface specification](../specs/site-now-surface/spec.md) |
...
| Site changelog and `/now/` source relationship | Modified | Render both views from the same model | [Documentation/site publishing](../../ARCHITECTURE.md#systems) |
...
| Atomic Highlights | New | [`/now/` content contract](../specs/site-now-surface/spec.md) | Each fragment's Highlights remains a distinct update group in `/now/` | Shared model exposes Highlights by fragment identity | Every source Highlights item appears exactly once and byte-for-byte in `/now/` data |
...
| Stable links | New | Current generated heading slugs | Historical anchors remain unchanged; a new update anchor is `change-` plus the UUID's 32 lowercase hexadecimal digits | Renderer emits `<a id="change-<uuidhex>"></a>` immediately before the update heading and `/now/` links to that ID | Rendered-DOM and local-Markdown tests cover repeated package, version, and date labels |
...
| Human editorial authority | Narrowed | [`/now/` authoring boundary](../specs/site-now-surface/spec.md) | Drafting assistance remains optional inside a pull request; only reviewed fragment text enters the model | Normal code review and required Highlights validation | Offline build and dependency check prove no model or network dependency |
...
| Build performance | Existing `make site-build` duration | Many small files increase scan and parse cost | Compare five-run medians and require less than 10% site-build regression at ten times the current release-entry count |
...
| `/now/` adapter | Modified | `tools/build-site.py` and `web/src/pages/now/index.astro` | Documentation builder and web page | Documentation site | Exact per-fragment Highlights correspondence test |
