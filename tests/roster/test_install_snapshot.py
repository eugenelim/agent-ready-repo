...
leak:
  ...
  2. **No catalogue-string leaks.** Each scaffolded file is scanned
     against the same blocklist `lint.py (_PackRules._check_seeds)` uses — no
...
Goldens live at `tests/fixtures/install_snapshot/<pack>.paths.txt`
...
# Same single-line sentinel lint.py (_PackRules._check_seeds) honours. Catalogue-attribution
