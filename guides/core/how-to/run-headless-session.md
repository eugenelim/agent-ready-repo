...
> **`CODE-HUMAN-GATE` — wait for actual merge before responding.** When `gate` is `CODE-HUMAN-GATE`, the work-loop skill opens the PR and then calls `elicit()` with a gate question that includes the PR URL. Parse the PR URL from the elicitation `message` field, surface it to the human reviewer, and poll your VCS API until the PR is merged before sending the elicitation response. Responding before the actual merge marks the work-loop run DONE while the PR remains open. **Stage 1 note:** a structured `pr_url` field in the `workspace_status()` response and a dedicated `gate-pr-ready` notification are in the design but not yet implemented — the elicitation message text is the only contracted source of the PR URL in Stage 1.
