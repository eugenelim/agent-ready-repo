...
- **Contract:** <!-- contracts/<type>/<name> this spec defines or touches (see new-spec step 4b / the `new-spec` skill's spec-and-plan contract reference Contracts), or "none" for a non-API feature. A contract surface is not just a synchronous REST API — an event interface or a backend-for-frontend (BFF) boundary is a contract too; name it here and author it under contracts/<type>/. -->
