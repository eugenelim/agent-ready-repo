...
This page explains the edge, why a **brief** sits above the spec to cover it, and why the brief looks the way it does. For how to use the brief, see [Receive a product brief and decompose it into specs](../how-to/receive-a-product-brief-and-decompose-it-into-specs.md); for the field list, see [Product brief fields](../reference/product-brief-fields.md).
...
- **Cram it into one oversized spec.** It breaks the sizing rule, and the build loop has nothing coherent to chew on.
- **Fire `new-spec` N times by hand.** Nothing records *why* the work exists, *how* it was decomposed, or *whether the whole thing is delivered*.
