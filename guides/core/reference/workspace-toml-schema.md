...
| Field | Meaning |
| --- | --- |
| `parent` | Local parent artifact such as a brief or intent. Declare it explicitly on a spec entry before that spec cools: the value is **unestablished once the spec has cooled** if the key is absent or names no registered brief, and reconciliation then refuses every `kind = "brief"` dependency until it is declared. A literal `none` is a valid declaration meaning the spec has no parent brief. |
...
| `cooled_child_scope_unknown` | A cooled spec entry's parent scope is not established: `source.parent` is absent, or names no registered brief. | Declare `source.parent` on the named entry — a brief path that resolves to a registered membership, or `none` only when that spec has no parent brief. Both repairs clear the refusal; guessing does not, because the value is trusted rather than verified once the spec has cooled. |
