...
The `/desk-research-project-start` skill fires (the phrasing *"start a research
...
`/desk-research-project-digest` reads `sources/*.md` and builds the **middle layer**
...
`/desk-research-project-check` reads the matrix and memos **by eye** and reports a
...
`/desk-research-project-synthesize` reads the digest and writes **two** files into
the folder:
...
- Want the full catalogue of the four project skills, their phases, and the
  `agentbundle-layout.toml` `[research]` keys? See the [reference](../reference/desk-research-pack.md).
- Wondering when a project is overkill and a one-off `/desk-research` would do?
