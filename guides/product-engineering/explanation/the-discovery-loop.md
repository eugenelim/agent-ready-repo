...
the bounded handoff is submitted through `work-intake`, which keeps the existing
`new-spec` or `author-delivery-brief continue` gates. An explicitly compatible
Core invocation admits it as the machine-readable handoff object.

Core is optional. If it is absent, unknown, or predates the handoff capability,
the loop renders the same bounded content for portable use, submits that through
`work-intake` too, and still completes shaping. This seam moves context, not authority: external locators stay opaque,
