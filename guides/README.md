...
**An alternative to P2, not a step after it.** P2 is the fastest route from an
idea to a build-ready bet. This one is wider: it adds strategy and design, and
follows the order the packs themselves declare, so each step hands a named
artifact to the next. Take P2 when the question is already framed; take this
when it is not.
