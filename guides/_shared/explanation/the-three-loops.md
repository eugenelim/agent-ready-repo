...
- **Risk-scaled modes.** Eligible low-risk work runs direct-light from the current request with adversarial review and no persisted spec. Full mode uses a durable spec and plan when a risk trigger fires — a design you cannot predict, new dependency, compliance surface, multi-person work, destructive operation. The mode is chosen by the work's risk profile, not by file count.
