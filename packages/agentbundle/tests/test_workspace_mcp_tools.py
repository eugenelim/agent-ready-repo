...
class TestWorkspaceStatusSlugSafety:
    ...
    def test_unsafe_slug_dot_filtered(self) -> None:
        assert False  # STUB: AC10 — an entry with slug="." must not appear in the output

    def test_unsafe_slug_dotdot_filtered(self) -> None:
        assert False  # STUB: AC10 — slug=".." filtered

    def test_unsafe_slug_leading_dash_filtered(self) -> None:
        assert False  # STUB: AC10 — slug="-bad" filtered

    def test_safe_slug_passes(self) -> None:
        assert False  # STUB: AC10 — normal slug "my-feature.v2" passes
...
class TestPackPresenceFilter:
    ...
    def test_skill_found_in_repo_claude_root(self, tmp_path: Path) -> None:
        assert False  # STUB: AC9 — create SKILL.md under .claude/skills/{skill}/; dispatch_skill advertised as available

    def test_skill_not_in_any_root_marks_unavailable(self, tmp_path: Path) -> None:
        assert False  # STUB: AC9 — no SKILL.md anywhere → available=False + required_pack present

    def test_skill_found_in_user_scope(self, tmp_path: Path) -> None:
        assert False  # STUB: AC9 — skill exists in ~/.agents/skills/{skill}/SKILL.md
...
class TestFSMStateMerge:
    ...
    def test_fsm_fields_present_in_result(self) -> None:
        assert False  # STUB: AC8 — result must contain current_state, gate_pending, gate, gate_question, review_findings

    def test_gate_pending_true_when_bridge_says_gate(self) -> None:
        assert False  # STUB: AC7/AC8 — bridge.get_fsm_state() gate_pending=True → result["gate_pending"] True
