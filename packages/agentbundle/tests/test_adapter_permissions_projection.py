...
class TestPermissionsAllowProjection:
    ...
    def test_install_core_adds_six_permission_entries(self) -> None:
        pytest.skip("STUB: AC17 (deferred) — install core on clean tmp repo; parse .claude/settings.json; assert all 6 ids in permissions.allow")

    def test_install_core_preserves_existing_entries(self) -> None:
        pytest.skip("STUB: AC18 (deferred) — pre-populate permissions.allow; install core; assert existing entries still present")

    def test_install_core_does_not_duplicate_entries(self) -> None:
        pytest.skip("STUB: AC18 (deferred) — install core twice; assert no duplicate entries in permissions.allow")

    def test_install_core_does_not_clobber_other_keys(self) -> None:
        pytest.skip("STUB: AC18 (deferred) — pre-populate unrelated settings.json key; install core; assert key preserved")
