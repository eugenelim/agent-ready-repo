...
class TestEventBridgePoll:
    ...
    def test_bridge_reads_new_event_within_poll_interval(self, tmp_path: Path) -> None:
        pytest.skip("STUB: write an event to events.jsonl; confirm bridge picks it up within 2×200ms")

    def test_bridge_byte_offset_tracks_position(self, tmp_path: Path) -> None:
        pytest.skip("STUB: write two events; bridge must read only new bytes on second poll")

    def test_bridge_seq_dedup_skips_lower_seq(self, tmp_path: Path) -> None:
        pytest.skip("STUB: write seq=2 then replay seq=1; second event must not update state")

    def test_bridge_gate_pending_set_on_human_gate_state(self, tmp_path: Path) -> None:
        pytest.skip('STUB: to field ending "-HUMAN-GATE" → gate_pending=True')


class TestEventBridgeInodeReset:
    """_EventBridge resets byte offset and state when inode changes."""

    def test_bridge_resets_on_inode_change(self, tmp_path: Path) -> None:
        pytest.skip("STUB: delete + recreate events.jsonl; bridge must reset to offset=0 and re-read")

    def test_bridge_resets_on_truncation(self, tmp_path: Path) -> None:
        pytest.skip("STUB (variant): truncate existing file (st_size < offset); bridge must reset")
...
class TestEventBridgeStop:
    ...
    def test_bridge_stop_exits_thread(self, tmp_path: Path) -> None:
        pytest.skip("STUB: call stop(); confirm thread.is_alive() is False within 1 s")
