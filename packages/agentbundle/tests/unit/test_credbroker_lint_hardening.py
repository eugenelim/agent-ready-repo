...
class TestIsCanonicalShimPathAnchor:
    ...
    def _canonical_bytes(self, tmp_path):
        self._source_dir = tmp_path / "source"
        self._source_dir.mkdir()
        self._bytes = b"# canonical fixture shim\n"
        (self._source_dir / "credentials_shim.py").write_bytes(self._bytes)
        self._tmp = tmp_path
    ...
    def test_non_canonical_parent_returns_false(self):
        ...
        assert _cs_is_canonical_shim(shim, self._source_dir) is False

    def test_scripts_parent_returns_true(self):
        """Canonical bytes at a scripts/ parent → True."""
        shim = self._write_shim("scripts")
        assert _cs_is_canonical_shim(shim, self._source_dir) is True
