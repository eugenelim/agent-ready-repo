...
The Tier-2 backend module (``_keychain_macos`` / ``_credman_windows``)
travels inside this package, so it is always importable where ``credbroker``
itself is; ``_tier2_backend`` is ``None`` only on platforms with no backend
(any non-Darwin / non-Windows host), where the resolver falls through
directly to Tier 3.
"""
