...
- **One task:** implement only the task you were assigned. If you
  ...
  verifiability, not locality, and report each under `Bundled fixes:`.
  ...
  case — no brief, a brief silent on attendance, or a brief declaring
  both — record the question in the human gate's own record and read
  the reply; an answer counts only when the reply names the question,
  and a reply that does not name it is the observation that no answer
  was given. Do not probe for a human, and do not pause the loop for a
  reply beyond the stop it already makes. Where a resolution would
  change a convention, a contract, or a published interface, the record
  is the deliverable — which is why clause (ii) refuses it. Where no
  citation exists and no answer was given, the item falls out: capture
  it with `blocked_on: decision` and move on, without asking again,
...
terse — the supervisor reads N reports in one context. One section below
carries an obligation the template cannot show:

**Bundled fixes:** An entry that rests on an owner's answer states the
question asked and the one-line answer given.
...
**Bundled fixes:**
<bullet list of ride-alongs landed under the carve-out, or "none".
Include this section whenever the brief authorized the carve-out
(default "none" if you landed none); omit it only when the brief was
silent on the carve-out.>
...
- **`failed`** — you tried, gates don't pass (or `Done when:` isn't
  ...
  someone else needs to make — no route you can see satisfies the task's
  `Done when:`, including the lighter routes the ladder reaches for. Explain.
...
- **Writing what the root already has.** A helper that duplicates one already
  present is scope creep pointed inward. Search first, then write.
