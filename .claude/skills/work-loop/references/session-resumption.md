...
5. Route by `last_event` to pick up where the session left off:
   ...
   | `reviewers-clean` | `CODE-HUMAN-GATE` | Wait for human signal. **Approved (merge confirmed):** fire `done`. **Changes requested:** surface the `review record` audit risk first — every clean-recording form is non-idempotent and its outcome is unknown, so a replay may double-increment `review_round_count` and overwrite one level of fingerprint audit history. Explicit human authorization is required before any replay. If authorized, read `last_review_clean_source` from `state.json` and replay that form: `"direct-clean"` → `--direct-clean-file <raw-path>`; `"structural-clean"` → `--structural-clean-file <raw-path>` (which re-classifies it); `"report"` → `--report <adjudication-path> --adjudication`; `null` means Surface rather than guess. Then fire `blocker-applied` → apply fix → `wave-complete` → GATES → REVIEW. |
