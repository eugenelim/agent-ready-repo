...
def test_near_duplicate_flags() -> None:
    ...
    new = (
        "Use to summarize a long email thread into a brief digest"
        " of the decisions and action items."
    )
...
def test_distinct_description_no_collision() -> None:
    existing = {
        "summarize-thread": (
            "Use to summarize a long email thread into a short digest of decisions."
        ),
    }
