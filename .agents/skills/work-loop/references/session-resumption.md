...
5. Route by `last_event` to pick up where the session left off:
   ...
   | `wave-passed` | `CODE-IMPLEMENTATION` | Re-issue `python '<skill-dir>/scripts/loop-cohort.py' wave advance docs/specs/<feature> --from-index <last_event_context.completed_wave_index> --expect-run-id <run_id>`; resume EXECUTE. Replay is safe only where the advance already landed — that branch is a no-op. If it did not land, this is the advancing branch, which refuses a wave whose tasks are not accounted for: every task in the completed wave needs a `dispatch-receipt` record, receipt or decline. Record the missing ones, then re-issue. |
   ...
   | `findings-remain` | `CODE-IMPLEMENTATION` | Read `last_review_record_operation_id` from `state.json` and compare it with `<run_id>:<transition_sequence>` from step 3. Equal → the round is recorded and re-issuing the identical payload under that id is a no-op; continue. Absent or different → the round was not recorded under that id, so a re-issue writes a new round and increments both counters; stale fingerprint baseline and possible under-count; do NOT auto-reissue. A round recorded before this field existed also reads as absent. **Surface to human**. |
   | `blocker-applied` | `CODE-IMPLEMENTATION` | Resume implementation directly (Status: Shipped stays; do not rewrite) |
   | `reviewers-clean` | `CODE-HUMAN-GATE` | Wait for human signal. **Approved (merge confirmed):** fire `done`. **Changes requested:** surface `review record --direct-clean-file/--structural-clean-file/--report` audit risk first — without a matching `--operation-id` both are non-idempotent, and a replay may double-increment `review_round_count` and overwrite one level of fingerprint audit history; explicit human authorization required before any such replay; if authorized, read `last_review_clean_source` from `state.json` and replay that form — `"direct-clean"` → `--direct-clean-file <raw-path>`, `"structural-clean"` → `--structural-clean-file <raw-path>`, `"report"` → `--report <adjudication-path> --adjudication` — passing `--operation-id <run_id>:<transition_sequence>`. That makes the replay a no-op only when `last_review_record_operation_id` already equals that id and the artifact the form names is still on disk; a clean round recorded flaglessly, or one whose gitignored artifact has been evicted, still refuses or still counts a round, so the authorization above is not optional; `null` means no clean round was recorded, so Surface rather than guess; then run `loop-cohort wave reopen <spec-dir> --expect-run-id <run_id>` then fire `blocker-applied` → apply fix → run `loop-cohort check <spec-dir> --phase wave-exit` → fire `wave-complete` → re-run GATES → REVIEW (adversarial first) |
   ...
   | any — when `state.json.amendment_pending` is set | any | A controlled contract amendment was in flight when the session ended, so the approval and schedule state it clears has not been rebuilt. Finish that amendment before anything else: `approve-plan`, then (code mode) `schedule`, then `plan-locked`. Exact replay of the same amendment completes the crash window without a second history entry. Until `schedule` persists a partition again, `schedule_waves` is empty and the wave-exit check hard-refuses, naming this route. Do not read that refusal as a reason to hand-write cohort state. |
