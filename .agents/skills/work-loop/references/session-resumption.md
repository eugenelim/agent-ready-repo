...
5. Route by `last_event` to pick up where the session left off:
   ...
   | `findings-remain` | `CODE-IMPLEMENTATION` | Read `last_review_record_operation_id` from `state.json` and compare it with `<run_id>:<transition_sequence>` from step 3. Equal → the round is recorded; continue. Absent or different → `review record --fingerprint` may not have run; stale fingerprint baseline and possible under-count; do NOT auto-reissue without the round's adjudication artifact. Re-issuing the identical payload under that same operation id is a no-op, so a replay you can reconstruct is safe. **Surface to human** when you cannot reconstruct it. |
   | `blocker-applied` | `CODE-IMPLEMENTATION` | Resume implementation directly (Status: Shipped stays; do not rewrite) |
   | `reviewers-clean` | `CODE-HUMAN-GATE` | Wait for human signal. **Approved (merge confirmed):** fire `done`. **Changes requested:** surface `review record --direct-clean-file/--report` audit risk first — without a matching `--operation-id` both are non-idempotent, and a replay may double-increment `review_round_count` and overwrite one level of fingerprint audit history; explicit human authorization required before any such replay; if authorized, read `last_review_clean_source` from `state.json` and replay that form — `"direct-clean"` → `--direct-clean-file <raw-path>`, `"report"` → `--report <adjudication-path> --adjudication` — passing `--operation-id <run_id>:<transition_sequence>`, which makes an identical replay a no-op; `null` means no clean round was recorded, so Surface rather than guess; then fire `blocker-applied` → apply fix → fire `wave-complete` → re-run GATES → REVIEW (adversarial first) |
