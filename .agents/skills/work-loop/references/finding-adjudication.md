...
| `matches_previous_round=true` | Surface it, and continue the round sequence; this never stops a loop. Read it as a weak signal: it is equality over a fingerprint whose preimage carries the finding's position, so a repair that shifts a line gives an otherwise-identical finding a new fingerprint. The flag therefore reads false through most real recurrence. It fires only when the sustained set is exactly the previous round's, which a round that changed nothing makes possible but does not guarantee — the set is parsed from the reviewer's report, not from the tree. A false reading is not evidence of progress. Full mode only — light mode holds no prior-round fingerprints. |
