...
| `approved_spec_hash` | `sha256(spec.md bytes)` at the time of `approve-plan`. |
| `approved_plan_hash` | `sha256(canonical(plan.md))` at the time of `approve-plan`. Canonical = CRLF→LF + strip trailing whitespace per line. |
| `plan_hash` | `sha256(canonical(plan.md))` at the time of `schedule`. `schedule check-current` verifies plan.md hasn't changed since scheduling. |
