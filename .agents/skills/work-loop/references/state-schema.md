...
| `finding_fingerprints` | `sha256("<file>\|<line>\|<title>")` per finding in the last findings round. Written by `review record --fingerprint`; compared across rounds by `review inspect`. The line and ordinal in the preimage move with any repair, so an equal set means an unchanged round rather than a stalled one. |
...
the flag returned false on all 302 recorded evaluations. ADR-0104 records the
measurement and retires the halt it used to carry.
