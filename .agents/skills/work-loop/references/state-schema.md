...
| `approved_spec_hash` | `sha256(canonical_contract(spec.md))` at the time of `approve-plan`. **Canonical form** = CRLF→LF + strip trailing whitespace per line + normalize the preamble `**Status:**` *token* + normalize checkbox bracket contents (`[x]`→`[ ]`). |
| `approved_plan_hash` | `sha256(canonical_contract(plan.md))` at the time of `approve-plan`. **Canonical form** = CRLF→LF + strip trailing whitespace per line + normalize the preamble `**Status:**` *token* + normalize checkbox bracket contents (`[x]`→`[ ]`). |
| `plan_hash` | `sha256(canonical_contract(plan.md))` at the time of `schedule`. **Canonical form** = CRLF→LF + strip trailing whitespace per line + normalize the preamble `**Status:**` *token* + normalize checkbox bracket contents (`[x]`→`[ ]`). `schedule check-current` verifies plan.md hasn't changed since scheduling. |
...
approved*, not the loop's own **bookkeeping**. The preamble status token and
