...
1. **Fetch the source, SSRF-guarded** — same allowlist as `assimilate-primitive`
...
2. **Open (or resume) the ledger.** State lives at
...
3. **Inventory candidates and assign a verdict each**, iteratively (one
...
4. **Emit an RFC** capturing the inventory + verdicts — a reviewable proposal,
...
5. **Incremental re-sync.** Re-pointing at a source assimilated before is a
