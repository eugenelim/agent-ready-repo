...
4. **For credentialed skills** (calls an authenticated external API on the user's behalf), use the [`add-credentialed-skill`](packs/core/.apm/skills/add-credentialed-skill/) skill as the scaffold — security rules around credential handling are non-negotiable and the reference is in [`packs/core/.apm/skills/example-credentialed-skill/`](packs/core/.apm/skills/example-credentialed-skill/).
