...
Core is a complete build system for coding agents. Non-trivial work runs through `work-loop`: surface assumptions → plan → implement → lint, typecheck, and test → cold independent review → fix or finish. The adversarial reviewer is the hard review gate; security and quality lenses join when the diff warrants them. Findings are fingerprinted, so a round that repeats itself exactly is surfaced rather than passing unremarked.

The loop scales its ceremony with risk, but it never self-certifies. It stops at plan approval, unresolved boundaries, the iteration cap, and PR merge. Who answers those gates—a person or a control harness—is up to you.
