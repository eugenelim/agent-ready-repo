...
- [ ] interaction/behavior section enriched   [ ] design-review clean
```

## Genre-specific notes
<!-- Uncomment and populate ONLY the sub-section that matches the surface-genre
     declared in the frontmatter. Leave the others commented out. Genre is
     orthogonal to platform — a marketplace surface on iOS is both `marketplace`
     AND `iOS`. -->

<!-- ### If marketing
     Hero approach: <which of the 5 hero types applies — statement / problem-agitation / social-proof / demo-first / narrative>
     Above-fold contract: headline (≤10 words) + sub + primary CTA + secondary CTA + proof signal + friction microcopy
     Scroll story: <7-zone structure — which zone is this screen?>
     IC-first check: does copy lead with reader's recognized problem before naming the product?
-->

<!-- ### If documentation
     Diátaxis type: <tutorial | how-to | reference | explanation> — shapes density and navigation target
     TTFV target: <what is the reader's first value moment on this screen?>
     Navigation strategy: <hub / progressive / reference-table — based on page count in this doc set>
     Machine-readability: <code blocks, API params, structured output fields — note design decisions>
-->

<!-- ### If informational
     Typography as primary tool: line length 45–75 chars, line height 1.4–1.6×, heading scale contrast
     F/Z-pattern calibration: <which scan pattern dominates — F (information-dense) or Z (conversational/marketing)?>
     Editorial grid: <column structure — symmetric or asymmetric>
     "What's next" chain: <which of the 4 category types — related topic / deeper dive / action / discovery>
-->

<!-- ### If analytical
     Business questions this screen answers (3–5 explicit):
     Widget hierarchy tier: <primary KPI (≤9) | secondary | tertiary>
     Spatial layout grammar: top=state-signals / left=worklist / centre=primary-diagnostic / right=context+filter
     Domain model anchor: <objects / attributes / relationships / actions driving widget placement>
     Role-based view: <which role(s) is this screen designed for>
-->

<!-- ### If marketplace
     Listing card IA: <hierarchy — primary identifier / key attribute / social proof / price / CTA>
     Filter/facet architecture: <taxonomy used / chip vs sidebar / instant vs apply-button>
     Browse-first vs search-first: <which routing pattern dominates>
     Comparison affordances: <does this screen support comparison — if so, which mechanism>
-->

<!-- ### If workspace
     Session arc stage: <arrive | orient | work | persist | collaborate>
     Context-persistence pattern: <last-location | returning-session re-orientation | breadcrumb+recents+activity>
     Attention zone: <ambient (background awareness) | focal (primary task)>
     Agentic patterns (if applicable): <task queue | agent status | HITL confirmation | output review | multi-agent visibility>
     Collaboration state: <presence | live-editing | following mode — if applicable>
-->

<!-- ### If transactional-journey
     Transaction bridge: <wizard pattern type — linear stepper | save-and-resume | conditional disclosure>
     Destructive-action tier (if applicable): <inline | toast+undo | modal | typed-confirmation | 2FA>
     Save-state indicator: <autosave indicator state — saving | saved | error — or manual save>
     Error recovery path: <what happens on payment/submission failure — which screen/state>
-->

## How it fits the flow

- `user-flow` sequences the screens (the flow + the inventory spine) and
...
- The action→service column is the live tie to `service-blueprint`; the outcome
  trace is the tie up to `journey-mapping` — so a screen can be built alone
