...
the shared quality floor (`../design-review/references/quality-floor.md`) —
...
2. **Text-only steel thread (the floor).** When no such tool is present, script
   ...
   - **every action has a backing service** — each action names a service from
     the blueprint (or a textually-named service when `service-blueprint` is
     absent).
...
(down to `service-blueprint`) and **every screen → a journey step** (up to
`journey-mapping`). The cross-brief consistency pass and the
