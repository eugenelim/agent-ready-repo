...
in here would duplicate the `desk-research` pack's job. The method travels; the
...
`desk-research` skill, whose job is exactly to survey current practice. So this file
...
1. **Detect novelty.** When the concept stage finds that **no shipped
   ...
   reference for it."
2. **Compose with `desk-research` when available.** If the `desk-research` skill is
   ...
   the current state of the art. Composition with `desk-research` is **optional** —
...
5. **Degrade when `desk-research` is absent.** With no `desk-research` skill installed, do
