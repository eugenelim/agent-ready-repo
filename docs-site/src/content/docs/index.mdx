...
New here? Follow the [install-to-ship walkthrough](/agent-ready-repo/docs/guides/#the-install-to-ship-walkthrough) — one ordered route from installing the catalogue through a governed, reported release, with an explicit next step at every stage.

Or start with the supervised build loop, or choose the outcome closest to the work in front of you.
