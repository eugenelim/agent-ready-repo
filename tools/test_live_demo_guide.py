    ...
    def test_track_map_pins_real_pack_paths(self) -> None:
        ...
        self.assert_contains_all(
            ...
            (
                ...
                "**Enterprise**",
                "`receive-brief` → selected slice → `new-spec`",
                "Source brief `Ready`",
                "**Non-technical**",
                "Product Engineering at user scope, then Core at repo scope",
                "`frame-intent` → `de-risk-intent` → `decompose-intent` → `receive-brief` → `new-spec`",
                "Draft intent, Ready leaf brief",
    ...
    def test_enterprise_contract_uses_unqueued_ready_brief(self) -> None:
        ...
        self.assert_contains_all(
            ...
            (
                ...
                "skips `author-brief`",
                "Use Core's receive-brief skill",
    ...
    def test_nontechnical_contract_crosses_user_to_repo_scope(self) -> None:
        ...
        self.assert_contains_all(
            ...
            (
                ...
                "validation hook",
                "Outcome, Appetite, at least one Rabbit hole, and a one-row Spec",
                "before marking the brief `Ready`",
                "`to-validate`",
                "source → corrected intent → Ready brief → Draft acceptance-criterion",
