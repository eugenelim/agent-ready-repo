...
5. **Hand off.** Once the content brief is written:
   ...
   - `conversion-design` is the hand-off for an acquisition surface that also needs a structural specification. It admits acquisition surfaces only, so it runs its editorial quality gate on every structural specification it produces, rather than branching on the declared mode.
