...
- **`output_dir` is a base, not the leaf.** The `creative-direction` skill writes
  to `<output_dir>/direction/<slug>.md`, where `<slug>` is a short kebab-case
  name for the surface (e.g. `marketing-site`, `mobile-app`, `checkout-flow`).
  The `direction/` directory is created lazily on first write — you do not need
  to pre-create it.
