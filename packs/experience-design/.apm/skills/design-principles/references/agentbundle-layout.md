...
- **`output_dir` is a base, not the leaf.** The `design-principles` skill writes
  ...
  `analytics-dashboard`, `mobile-app`). The `principles/` directory is created
...
Every design-principles doc written by this skill includes the following
frontmatter:
...
The `type: design-principles` field is the discover-by-marker key. Do not omit
it; without it the artifact cannot be found by Tier 3 resolution.
