...
- **`output_dir` is a base, not the leaf.** The `design-system` skill writes
  to `<output_dir>/tokens/<slug>.md`, where `<slug>` is a short kebab-case
  name for the product or surface (e.g. `marketing-site`, `mobile-app`,
  `analytics-dashboard`). The `tokens/` directory is created lazily on first
  write — you do not need to pre-create it.
