...
- **`output_dir` is a base, not the leaf.** The `copy-direction` skill writes
  to `<output_dir>/copy/<surface-slug>.md` with frontmatter `type: copy-direction`,
  where `<surface-slug>` is a short kebab-case name for the specific surface
  (e.g. `landing-page`, `pricing-page`, `onboarding-hero`, `campaign-q3`).
  The `copy/` directory is created lazily on first write — you do not need to
  pre-create it.
...
2. **User-profile config** — read `~/.agentbundle/agentbundle-layout.toml`
   ...
   the fallback — useful for a personal vault (e.g. a notes app) or a default output
...
## Coexistence with `tone-of-voice`

Both `copy-direction` and `tone-of-voice` write to `<output_dir>/copy/`. They do not collide:

- `tone-of-voice` writes a single brand-level doc at `copy/brand-register.md`.
- `copy-direction` writes one doc per surface: `copy/<surface-slug>.md`.

An adopter may have both types in the same `copy/` directory. The `type:` frontmatter field disambiguates them.
