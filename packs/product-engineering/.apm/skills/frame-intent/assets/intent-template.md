...
- **Status:** `<Draft | Accepted | Fulfilled | Withdrawn | Cancelled>` <!-- required; or `Superseded by <slug>` naming the live intent that replaced this bet. Withdrawn and Cancelled are peers of Fulfilled, not flavours of it — a rollup that counts them as delivered is the failure this vocabulary prevents -->
