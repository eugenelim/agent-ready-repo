...
3. **Project the confirmed delivery unit — by semantic role.** A feature-level
   ...
   it predates this object, submit to `work-intake` the same bounded handoff
   rendered for portable use, and omit the unsupported top-level field. External locators remain opaque data:
