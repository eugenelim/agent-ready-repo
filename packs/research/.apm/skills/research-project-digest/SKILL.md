...
`/research-project-start`), on phrasing like *"digest the sources"*, *"build the
...
saturation signal `/research-project-check` reads.
...
`/research-project-start`; use them to weight a source's contribution in the
...
- Not the synthesis — it structures the corpus; `/research-project-synthesize`
  reads this digest and writes the verdict and the brief.

## Next

When the matrix and memos are populated, run `/research-project-check` to read
the saturation signal, and `/research-project-synthesize` to emit the typed
