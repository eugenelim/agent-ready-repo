#!/usr/bin/env python3
"""Fail-closed policy and normalized-envelope checks for Jira Align intake."""
...
def trusted_source(acquired: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    """Derive provenance only from the trusted Jira Align response envelope."""
    item_id = acquired.get("id")
    object_type = acquired.get("type")
    revision = acquired.get("modifiedDate")
    if not isinstance(item_id, str) or not item_id.isdigit() or int(item_id) < 1:
        raise IntakePolicyError("trusted Jira Align response lacks a stable item ID")
    if not isinstance(object_type, str) or not isinstance(revision, str) or not revision:
        raise IntakePolicyError("trusted Jira Align response lacks revision or object type")
    ...
    return {
        "mode": "tracker-origin",
        "locator": f"jira-align://{kind}/{item_id}",
        "revision": revision,
