...
1. **User-scope config** — read `~/.agentbundle/agentbundle-layout.toml` `[research]
   output_dir` if the file exists and the key is present. User-scope takes priority
   so that a personal vault (e.g. Obsidian) or a default output path applies
   regardless of which repo is active.

2. **Repo-scope config** — read `./agentbundle-layout.toml` `[research] output_dir`
   if the file exists and the key is present. Repo-scope is the fallback — useful
   for team-visible research committed to the repo when no personal config is set.
