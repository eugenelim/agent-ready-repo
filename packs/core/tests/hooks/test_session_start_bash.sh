...
#
# These test core-pack content, not the agentbundle package, so they live
# with the pack rather than in packages/agentbundle/tests/ — renaming a
# helper in the core pack must not break the published package's suite.
#
# The pytest siblings are the wired parity net (`make test`, and Gate A in
# catalogue-tooling-ci-gates.yml). This bash runner is the second oracle,
# run by hand; `test_session_start_projection.sh` (same directory) is the projection oracle CI invokes.
#
...
if [[ ! -f "$HOOK" ]]; then
  echo "$(basename "$0"): hook not executable at $HOOK" >&2
  exit 1
fi
