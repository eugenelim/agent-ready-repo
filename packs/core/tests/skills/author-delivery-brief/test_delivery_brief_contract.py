...
def test_ready_has_one_canonical_six_field_semantic_gate() -> None:
    body = _BODIES["author-delivery-brief"]
    ...
    trailing = " ".join(body.split("## DoR gate", 1)[1].split())
    assert "defined only in continue stage 1" in trailing
    assert "**Outcome**" not in trailing


def test_spec_and_plan_descriptions_and_warranted_spec_invocation() -> None:
    receive_brief = " ".join(_BODIES["author-delivery-brief"].split())
    new_spec = " ".join(_BODIES["new-spec"].split())

    assert "a spec is the durable behavior contract for one delivery slice; and the plan is the implementation and verification strategy." in receive_brief.lower()
