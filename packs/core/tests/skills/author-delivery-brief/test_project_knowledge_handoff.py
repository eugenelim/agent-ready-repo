...
def _gate_section() -> str:
    ...
    return " ".join(
        text.split("## Project-knowledge gate: `brief-ready`", 1)[1].split()
    )
...
def test_brief_ready_gate_follows_the_complete_write_back() -> None:
    ...
    assert "canonical Ready gate above" in section
...
def test_brief_ready_constructs_the_published_request() -> None:
    ...
    assert "`producer.workflow: author-delivery-brief`" in section
...
def test_brief_ready_preserves_authority_and_receipt_scope() -> None:
    ...
    assert "at most one refinement" in section
