...
def test_author_brief_never_stamps_ready() -> None:
    normalized = " ".join(SKILL.read_text(encoding="utf-8").split())

    assert "does not set `Status: Ready`" in normalized
    assert "is the only skill that sets `Status: Ready`" in normalized
...
def test_single_direct_light_change_does_not_create_a_brief() -> None:
    ...
    assert "Creating a brief for a single direct-light change." in normalized
    assert "coherent multi-slice or cross-repository outcome; route a single change" in normalized
