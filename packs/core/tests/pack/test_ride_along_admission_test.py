...
C4 = (
    ...
    "ride-along eligible | Dispatched in-session at DECIDE, not "
    "captured | | Specific, real, ready now, not ride-along eligible | "
    "The session's next independently reviewed unit, decided at DECIDE "
    "| | Specific, failing the razor | Refused, non-silently | Take the "
    "first row that applies and stop: a defect blocked on a decision, "
    ...
    "existing artifact already covers, or one no capture criterion "
    "admits — is refused, non-silently, rather than discarded. What a "
    "`work-item` capture must carry, what the razor checks, and what a "
    "refusal tells the author are in [Close-time work-item "
