...
creates no fallback file. A successful result with no eligible topic supplies
zero candidate checks; a consequential match whose owning source cannot be
verified must retain `abstained: true`. Existing privacy refusal, committed-only
