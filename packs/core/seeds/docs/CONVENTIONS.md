...
failing loud on a cycle, warning on a forward-reference, and refusing any
`Depends on:` entry that names no task in the plan is refused (non-zero exit,
every offending task→dep pair named, nothing persisted). Parallel
