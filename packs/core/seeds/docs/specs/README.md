...
| [`wire-session-start-hook/`](wire-session-start-hook/spec.md) | Approved | RFC-0001, RFC-0004 | Ships hook-wiring for the core pack's `session-start.py` hook body so `agentbundle install core` auto-writes the Claude Code `SessionStart` binding into `.claude/settings.local.json` — no more hand-paste from `tools/hooks/README.md`. Core stays at adapter-contract v0.2; no contract, schema, or build-code changes. Wiring TOML uses Claude Code's documented nested SessionStart schema. Bundles a legacy-fixture rewrite of three stale `pre-commit.toml` upgrade-catalogue fixtures from `[hook] name/trigger/matcher` shape to live `[[hooks.<Event>]]` shape with a static stub command. Kiro support deferred to a separate spec (Kiro's session model needs the `steering` primitive we don't model yet). 10 ACs, 7 tasks; one PR. |
