...
- **`parent` is a base, not the leaf.** Each output file is written directly
  ...
  | `map-screen-flow` | `<parent>/screens/<slug>-flow.md` + `<parent>/screens/<slug>/<screen>.md` |
  | `map-internal-process` | `<parent>/processes/<slug>.md` |
...
| --- | --- |
| `type: customer-journey` | `map-customer-journey` |
| `type: service-blueprint` | `blueprint-service` |
| `type: screen-flow` | `map-screen-flow` |
| `type: process-flow` | `map-internal-process` |
...
`blueprint-service` writes `<parent>/blueprints/<slug>.md` with frontmatter
`type: service-blueprint`. The `slug` is a short kebab-case name for the
journey or product flow being blueprinted (e.g. `checkout-flow`,
`account-registration`). The output path is surfaced to the adopter before the
first write; the `blueprints/` directory is created lazily.
