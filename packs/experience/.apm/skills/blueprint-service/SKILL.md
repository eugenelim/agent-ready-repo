...
description: Use when you need to map the backing services that fulfil a customer journey — building a service blueprint with four rows (frontstage / line-of-visibility / backstage / support) that ties every screen action to the service or system behind it. Triggers on "service blueprint", "what backs this screen", "map the backstage", "what services support this journey", "blueprint the service". Do NOT use to map the customer journey itself (use `map-customer-journey`), to sequence screens and their transitions (use `map-screen-flow`), or to design system components (use `design-system-foundations`).
---

# Skill: blueprint-service

Produces a **service blueprint** — a four-row, column-by-column map that ties
every customer action and touchpoint to the employee and system actions that
back it and the internal support that enables those. The backstage column is the
**slicing instrument**: each backstage service is a candidate component; its
hand-off to `architect` and `contracts` is by-reference (a named service), never
an import. The method is grounded in the NN/g definition of service blueprinting;
see `references/service-blueprint.md`.

**Inputs (declared):** a customer journey map or journey stages (from
`map-customer-journey` or elicited inline); a screen flow or screen inventory
(from `map-screen-flow` or described inline). Both are elicited inline when no
...
2. **You are mapping the screen↔service tie, not the journey itself** — if the
   journey hasn't been mapped yet, offer to run `map-customer-journey` first,
...
2. **Elicit or confirm the journey and touchpoints.** If a `map-customer-journey`
...
3. **Build the four rows.** For each journey column, populate all four rows.
...
5. **Check the line of visibility is honest.** Walk each column: every item on
   the customer side that has no backstage entry is a gap — either a service is
   missing or the frontstage action is unsupported. Name every gap explicitly
...
- **Blocking when upstream artifacts are absent.** Elicit the journey inline;
  never refuse to proceed because `map-customer-journey` hasn't run.
