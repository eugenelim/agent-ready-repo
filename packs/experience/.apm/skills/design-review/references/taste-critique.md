...
The taste mode is the third pass `design-review` runs — after the quality-floor
checklist and the heuristic evaluation — when a **grounded aesthetic reference**
from `creative-direction` is present. It checks the screen against two things:
...
`creative-direction` records a set of named aesthetic goals, each grounded in:
...
- **Introduces fresh referents.** If a referent was not recorded in the current
  session's `creative-direction` output, it is not a valid warrant for a taste
