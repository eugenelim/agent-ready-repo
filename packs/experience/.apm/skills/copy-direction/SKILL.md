---
name: copy-direction
description: Use when a designer, copywriter, or builder has a felt "copy vibe" but no named direction — turning a vague register sense into ranked copy goals grounded in stable referents (persona language, copy precedents, persuasion standards), and recording copy arbitration rules the rest of the build references. Triggers on "what voice should our copy have", "write a copy-direction doc", "how do we sound different from our competitors", "what should our hero headline feel like", "copy vibe check". Do NOT use for product UI copy states (error messages, empty states, button labels, form labels) — use `voice-and-microcopy` in the `product-engineering` pack for those. Do NOT use for SEO keyword targeting, advertising copy templates, or brand identity documentation.
---

# Skill: copy-direction

Turns a vague "copy vibe" into a small set of **named, ranked copy goals**, each grounded in a stable referent, and records them — along with arbitration rules — in a copy-direction doc the rest of the build references. The doc is the durable artifact: every later copy choice points back to a goal and its referent rather than relitigating voice on each surface. This skill is the copy twin of `aesthetic-direction`: same interrogation rhythm applied to what the product *says* rather than how it *looks*.
...
2. **The direction isn't already named** — no current copy-direction doc owns this surface. If one exists, you are amending it, not starting fresh.
...
6. **Capture the doc.** Copy `assets/copy-direction-template.md` to `docs/design/copy/<slug>.md`. Fill: reader map (reader types, JTBD sentences, rank), named copy goals (each with what it means, what would violate it, and its referents), dominant goal, copy arbitration rules, plain-language floor notes, and open questions. The artifact path follows the three-tier layout contract (config → default → discover-by-marker) per RFC-0050 D6; the default is `docs/design/copy/<slug>.md` with frontmatter `type: copy-direction`. The `copy-direction` marker extends the pack's discover-by-marker set alongside `content-brief`. Load `references/agentbundle-layout.md` for the full path resolution.
