...
3. **You are deciding direction, not writing final copy** — the moment the ask is "write the headline," this skill has done its job and hands off to `copy-direction` for voice and `voice-and-microcopy` for UI strings.
4. **You know or can elicit the target audience** — either `map-customer-journey` output is available, or you can elicit persona and outcome inline before routing to a sub-path.
...
2. **Elicit or confirm persona and outcome.** If `map-customer-journey` output is available, consume it — the journey's audience definition, awareness level, and key moments are direct inputs. If not, elicit inline:
...
5. **Hand off.** Once the content brief is written:
   - Name `copy-direction` as the next step for copy voice and register grounding — the brief names what to say; copy-direction names how to say it.
   - Name `map-screen-flow` as the next step for screen sequencing — the scroll sections and content hierarchy in the brief feed the screen-flow's copy slots directly.
...
- **Substituting a content brief for a screen flow.** The brief names what each section must accomplish; it does not sequence screens or define interaction states — that is `map-screen-flow`'s job.
