...
**Three state homes — the load-bearing carve.** The shared quality floor (at `../design-critique/references/quality-floor.md`) owns the state *set* — the enumeration of which states must be designed (empty, loading, error, success, partial, disabled). `map-screen-flow` owns *cross-screen* routing — which screens follow which, and the macro-level error/edge paths across the journey. This skill owns the *in-component* state machine — the transitions and guards *within* a single screen or component — plus the motion, feedback timing, and input flow that animate those transitions. Macro-flow vs. micro-behavior is the line; never re-enumerate the state set or route across screens here.
...
2. **The state set is already known** — the shared quality floor's handle-all-states section (empty/loading/error/success/partial/disabled) has been consulted for this surface. If it hasn't, load the floor first (`../design-critique/references/quality-floor.md`).
3. **You are designing within a screen, not across screens** — if the ask is "which screen comes after this one on error," that is `map-screen-flow`'s territory. Bring the cross-screen routing already decided and design the in-component behavior on top of it.
...
5. **Design motion with purpose.** For every animation or transition, name what state change or spatial relationship it communicates; if it communicates nothing, cut it. Point to platform motion guidance (`references/interaction-pillars.md`). Honor the quality floor's reduced-motion rule: every animated state change must have a still or instant-transition alternative that preserves the information the motion carried. Express this as design intent; name no duration or easing value.
...
7. **Check cognitive-law fit.** Review the design against the cognitive laws in `references/interaction-pillars.md` (Fitts's, Hick's, Miller's, Doherty, Jakob's) and the usability heuristics in `../design-critique/references/heuristics.md`. Note where the design follows and where it requires a deliberate trade-off.
...
- **Routing across screens.** "On error, go to the retry screen" is `map-screen-flow`'s territory. Name the cross-screen destination from the screen flow and design the in-component behavior that reaches it.
...
- **Prescribing a layout.** Where things sit is `layout-and-information-architecture`'s domain. How moving through them behaves is this skill's. They are complementary; do not conflate them.
