...
2. **Consult available knowledge surfaces.** Before shaping the concept,
   ...
   **lower the confidence and flag** any claim you could not ground. **Never
   ...
   <!-- knowledge-provider-handoff:start — stable anchor for the bounded
        provider-consumer instructions below. Tests locate this block by
        these markers, not by neighbouring headings or step numbers, so
        reordering the procedure cannot silently move the boundary. -->
