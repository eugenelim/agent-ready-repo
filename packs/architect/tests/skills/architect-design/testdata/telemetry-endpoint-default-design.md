<!-- Corpus baseline: this document is the authored reference corpus for
architect-design's document-architecture gates (DA1 through DA10). It backs
the DA3/DA10 clean-corpus check and the seven-precheck clean walk recorded in
docs/specs/architect-design-document-gates/notes/verification-ledger.md. This
file is a baseline: any edit to its content, including a wording fix,
obliges a fresh re-walk of all seven prechecks and a fresh record of that
walk before the edit lands, because the only reason this document exists is
to be the corpus that walk was measured against. -->
...
that point is the sender's already-shipped resolution chain. Extending this
subsystem into send-time resolution would duplicate a chain that already
covers five layers and already ranks the user scope correctly. This
...
sequenceDiagram
    ...
    Note over Installer,Config: Normal path — fresh install · Question: does a fresh install get the enterprise default with no user action? · Zoom: component
...
<!-- rationale -->
Placement matches the structural model in section 2, so no separate topology
diagram is drawn here; a second diagram would only repeat the two-component
picture already shown there.
...
| Merge Projector | AgentBundle distribution maintainers | `packages/agentbundle/agentbundle/install.py` | The `agentbundle install` CLI | Integration test asserting an existing key survives and a missing key is added |
