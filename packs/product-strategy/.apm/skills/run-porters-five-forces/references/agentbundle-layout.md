...
**This skill** writes `<parent>/competitive-landscape.md` with frontmatter `type: competitive-landscape`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: competitive-landscape` frontmatter in an existing file; use that file's parent directory as `<parent>`.
