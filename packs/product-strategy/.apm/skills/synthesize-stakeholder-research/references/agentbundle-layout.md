...
**This skill** writes `<parent>/stakeholder-synthesis.md` with frontmatter `type: stakeholder-synthesis`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: stakeholder-synthesis` frontmatter in an existing file; use that file's parent directory as `<parent>`.
