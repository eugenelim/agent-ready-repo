...
**This skill** writes `<parent>/okr-cascade.md` with frontmatter `type: okr-cascade`. It also writes `{type = "strategy"}` gap entries to `workspace.toml` (see `references/cross-pack-routing.md`).
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: okr-cascade` frontmatter in an existing file; use that file's parent directory as `<parent>`.
