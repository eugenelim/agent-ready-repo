...
**This skill** writes `<parent>/portfolio-position.md` with frontmatter `type: portfolio-position`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: portfolio-position` frontmatter in an existing file; use that file's parent directory as `<parent>`.
