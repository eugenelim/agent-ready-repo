...
**This skill** writes `<parent>/content-strategy.md` with frontmatter `type: content-strategy`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: content-strategy` frontmatter in an existing file; use that file's parent directory as `<parent>`.
