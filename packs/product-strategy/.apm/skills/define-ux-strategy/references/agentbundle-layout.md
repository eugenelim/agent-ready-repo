...
**This skill** writes `<parent>/ux-strategy.md` with frontmatter `type: ux-strategy`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: ux-strategy` frontmatter in an existing file; use that file's parent directory as `<parent>`.
