...
2. **Apply the Levy four-tenets quality check.** Rate each tenet for current strength: (a) Is the business strategy coherent and the market opportunity validated? (b) Is there a genuine value innovation — something the experience does that competitors do not or cannot? (c) Is the user research validated — observational or behavioral, not assumed? (d) Is the experience frictionless across every touchpoint the customer actually crosses, online and offline — or does it hold only inside one screen flow? Flag any tenet where the current strength is weak — these are the gaps the UX strategy must address.
