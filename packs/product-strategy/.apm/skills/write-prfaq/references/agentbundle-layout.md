...
**This skill** writes `<parent>/prfaq.md` with frontmatter `type: prfaq`.
...
3. **Discover-by-marker** — if neither config nor default resolves to an existing directory, search the workspace for `type: prfaq` frontmatter in an existing file; use that file's parent directory as `<parent>`.
