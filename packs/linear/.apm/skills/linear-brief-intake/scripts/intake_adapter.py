#!/usr/bin/env python3
"""Fail-closed policy and normalized-envelope checks for Linear intake."""
...
def trusted_source(acquired: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    """Derive provenance only from the trusted Linear response envelope."""
    raw_id = acquired.get("id")
    object_type = acquired.get("type")
    revision = acquired.get("updatedAt")
    if not isinstance(raw_id, str) or not raw_id.startswith("lin-"):
        raise IntakePolicyError("trusted Linear response lacks a stable item ID")
    item_id = raw_id.removeprefix("lin-")
    if not item_id.isdigit() or int(item_id) < 1:
        raise IntakePolicyError("trusted Linear response lacks a stable item ID")
    if not isinstance(object_type, str) or not isinstance(revision, str) or not revision:
        raise IntakePolicyError("trusted Linear response lacks revision or object type")
    if object_type == "Project":
        kind = "containers"
    elif object_type == "Cycle":
        kind = "collections"
    else:
        kind = "items"
    return {
        "mode": "tracker-origin",
        "locator": f"linear://{kind}/{item_id}",
        "revision": revision,
