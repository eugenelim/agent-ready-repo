...
`/desk-research` standard and deep modes enumerate retrievers from three
...
`packs/desk-research/.apm/skills/desk-research/scripts/<name>-retriever.py`. The
...
3. Declares its auth shape via `metadata.auth` in a module-level
   ...
   the `credential-brokers` pack which `desk-research` does not depend on
...
`citations` array is a contract violation — `/desk-research` refuses
un-citable answers.

## Invocation from the main session

`/desk-research` enumerates available scripts in standard or deep mode by
listing `packs/desk-research/.apm/skills/desk-research/scripts/*-retriever.py`,
...
```bash
python packs/desk-research/.apm/skills/desk-research/scripts/arxiv-retriever.py \
  "query text here"
```
...
6. `/desk-research` picks the new retriever up automatically the next time
